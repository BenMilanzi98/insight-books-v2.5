...
export async function deriveSetupWizardFacts(db, tenantId) {
  ...
  const [
    ...
    db.inventoryTransaction.count({
      where: { tenantId, type: { in: ['Stock In', 'stock_in', 'opening_stock', 'import_receipt'] } },
    }),
    ...
    typeof db.acctV2OpeningBalanceBatch?.count === 'function'
  ...
  return {
    ...
    hasOpeningBalancesReview: openingBalanceTxCount > 0 || openingBalanceBatchCount > 0,
    openingBalanceTxCount,
    openingBalanceBatchCount,
    tenantName: tenantRow?.name || '',
    businessPhone: tenantSettingsRow?.businessPhone || '',
    businessAddress: tenantSettingsRow?.businessAddress || '',
    businessEmail: tenantSettingsRow?.businessEmail || '',
    accountSettingsComplete: !!(tenantRow?.name && String(tenantRow.name).trim()),
...
function derivedStepComplete(stepId, facts) {
  switch (stepId) {
    case 'accountSettings':
      return facts.accountSettingsComplete;
    case 'inventory':
    case 'openingStock':
...
export function mergeWizardStep(currentRaw, action, stepId) {
  ...
  const canonical = aliases[stepId] || stepId;
  if (!STEP_IDS.has(canonical)) {
