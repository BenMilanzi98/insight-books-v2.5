...
export async function calculateProductProfitability(tenantId, startDate, endDate) {
  ...
  const profitability = products.map(product => {
    // Calculate revenue
    const saleRevenue = product.saleItems.reduce((sum, item) => {
      return sum + (item.price * item.quantity);
    }, 0);

    const invoiceRevenue = product.invoiceItems.reduce((sum, item) => {
      return sum + (item.price * item.quantity);
    }, 0);
...
export async function calculateCustomerProfitability(tenantId, startDate, endDate) {
  ...
  const profitability = customers.map(customer => {
    ...
    const saleRevenue = customer.sales.reduce((sum, sale) => sum + sale.total, 0);

    // Calculate revenue from invoices
    const invoiceRevenue = customer.invoices.reduce((sum, invoice) => sum + invoice.total, 0);
    ...
    for (const sale of customer.sales) {
      for (const item of sale.items) {
        if (item.product && !item.product.isService) {
          const cost = parseFloat(item.product.averageCost || item.product.cost || 0);
  ...
  return {
    ...
    totalRevenue: profitability.reduce((sum, c) => sum + c.revenue, 0),
    totalCOGS: profitability.reduce((sum, c) => sum + c.cogs, 0),
    totalGrossProfit: profitability.reduce((sum, c) => sum + c.grossProfit, 0),
    averageProfitMargin: profitability.length > 0
