...
export function resolvePaymentMethodLabel(paymentMethod, paymentAccountById = new Map()) {
  ...
  if (LEGACY_METHOD_LABELS[key]) {
    return LEGACY_METHOD_LABELS[key];
  }

  const spaceKey = raw.toLowerCase();
  if (spaceKey === 'stock in') return LEGACY_METHOD_LABELS.stock_in;
  if (spaceKey === 'stock out') return LEGACY_METHOD_LABELS.stock_out;
