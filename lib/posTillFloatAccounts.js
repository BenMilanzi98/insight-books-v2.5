  ...
  try {
    return await db.account.create({
      data: {
        ...
        postingAllowed: true,
        // Internal POS float only — never show in COA or account pickers (use 1110 for cash).
...
async function ensureTillFloatCoaAccount(tenantId, db) {
  ...
  if (existing.visibleInChart !== false) patch.visibleInChart = false;
...
function buildTillFloatPaymentAccountData(tenantId, tillCoaId) {
  return {
    ...
    isSystem: true,
    isActive: true,
    coaAccount: { connect: { id: tillCoaId } },
  };
}
...
export function isTillFloatLeafAccount(account) {
  ...
  if (normalizedGlCode(account) === POS_TILL_FLOAT_GL_CODE) return true;
  const purpose = String(account.systemPurpose ?? account.controlAccountPurpose ?? '').trim();
  return purpose === POS_TILL_FLOAT_REFERENCE;
}

/** PaymentAccount rows that must never appear in cash / payment pickers. */
export function isPosTillFloatPaymentAccount(paymentAccount) {
  if (!paymentAccount) return false;
  const reference = String(paymentAccount.reference ?? '').trim();
  if (reference === POS_TILL_FLOAT_REFERENCE) return true;
  const name = String(paymentAccount.name ?? '').trim().toLowerCase();
  return name === POS_TILL_FLOAT_PA_NAME.toLowerCase() && paymentAccount.isSystem === true;
}

/**
 * Filter till-float out of payment-account UI lists.
 * Prefer JS filtering over Prisma `NOT` — SQL `NOT (nullable = x)` drops null rows.
 */
export function withoutPosTillFloatPaymentAccounts(paymentAccounts = []) {
  return (paymentAccounts || []).filter((p) => !isPosTillFloatPaymentAccount(p));
}
