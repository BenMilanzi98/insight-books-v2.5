...
export async function generateStockMovementReport(tenantId, startDate, endDate, productId = null, branchId = null) {
  ...
  for (const pid of productIds) {
    ...
    if (!data) {
      ...
      const closingBal = useActualStockForClosing ? currentStock : openBal;
      const openBalDisplay = useActualStockForClosing ? closingBal : openBal;
    ...
    if (useActualStockForClosing && data.product) {
      const currentStock = Number(data.product.stockLevel) || 0;
      closing = currentStock;
      opening = closing - net;
      totals.balance = closing;
      // Recompute each row's running balance so it flows from reconciled opening to closing
      let running = opening;
