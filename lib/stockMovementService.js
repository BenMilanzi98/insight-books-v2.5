...
export async function getQuantityOnHandAsOfDate(prisma, tenantId, productId, asOfDate, _legacyBranchId = null) {
  ...
  const baseline = Number(product?.stockLevel || 0) - allTimeDelta;
...
async function getProductBaselineQuantity(tenantId, productId, currentStockLevel) {
  const fullLedgerDelta = await getInventoryTransactionDelta(tenantId, productId);
  return Number(currentStockLevel || 0) - fullLedgerDelta;
}
...
export async function generateStockMovementReport(tenantId, startDate, endDate, productId = null, branchId = null) {
  ...
  // 3) For each product: reconcile opening from authoritative stockLevel + transaction ledger.
  const baselineByProduct = {};
  const openingByProduct = {};
  await Promise.all(
    productIds.map(async (pid) => {
      const product = productById.get(pid);
      baselineByProduct[pid] = await getProductBaselineQuantity(
        tenantId,
        pid,
        product?.stockLevel || 0
      );
      openingByProduct[pid] = await getOpeningBalanceForProduct(
        tenantId,
        pid,
        start,
        baselineByProduct[pid]
      );
    })
  );
  ...
  for (const pid of productIds) {
    ...
    if (!data) {
      ...
      const closingBal = nonNegativeQuantity(
        (baselineByProduct[pid] || 0) +
          (await getInventoryTransactionDelta(tenantId, pid, { lte: end }))
      );
