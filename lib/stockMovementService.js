...
export async function generateStockMovementReport(tenantId, startDate, endDate, productId = null, branchId = null) {
  ...
  if (!productIds.length) {
    productIds = [...new Set(periodTransactions.map(t => t.productId))];
  ...
  for (const t of periodTransactions) {
    ...
    const row = {
      ...
      qtyIn: qtyInNum,
      qtyOut: qtyOutNum,
      balance: productMap[t.productId].runningBalance,
  ...
  for (const pid of productIds) {
    ...
    if (!data) {
      ...
      productMovements.push({
        ...
        openingBalance: openingByProduct[pid] ?? 0,
        movements: [],
        totals: { qtyIn: 0, qtyOut: 0, net: 0, balance: openingByProduct[pid] ?? 0 },
        closingBalance: openingByProduct[pid] ?? 0
      });
      continue;
    }
    ...
    totals.net = totals.qtyIn - totals.qtyOut;
    totals.balance = data.movements.length ? data.movements[data.movements.length - 1].balance : data.openingBalance;
    productMovements.push({
      product: data.product,
      openingBalance: data.openingBalance,
      movements: data.movements,
      totals,
      closingBalance: data.movements.length ? data.movements[data.movements.length - 1].balance : data.openingBalance
    });
  }
