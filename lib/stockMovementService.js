...
export async function generateStockMovementReport(tenantId, startDate, endDate, productId = null, branchId = null) {
  ...
  for (const pid of productIds) {
    ...
    if (!data) {
      ...
      if (!product) continue;
      // Use accurate opening from transactions (no clamp) so report matches history
      ...
      productMovements.push({
        ...
        movements: [],
        totals: { qtyIn: 0, qtyOut: 0, net: 0, netDisplay: 0, netDirection: 'in', balance: openBal },
    ...
    totals.net = net;
    // Positive number for display: magnitude of change (in or out)
