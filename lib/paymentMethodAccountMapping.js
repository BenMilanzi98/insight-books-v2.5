...
export const DEFAULT_PAYMENT_METHOD_MAP = {
  ...
  credit: {
    accountCodes: ['1100', '1200'],
    accountNameKeywords: ['receivable', 'ar', 'a/r'],
    defaultAccountCode: '1100',
...
export async function getAccountForPaymentMethod(tenantId, paymentMethod, tx = prisma) {
  ...
  if (typeof paymentMethod === 'string' && paymentMethod.length > 20 && /^[a-z0-9]+$/i.test(paymentMethod)) {
    ...
    if (paymentAccountById) {
      // Convert PaymentAccount to Account-like structure if needed
      // For now, try to find the corresponding Account by coaAccountId
  ...
  // Last resort: find any active asset account
  ...
  if (fallbackAccount) {
    console.warn(`⚠️ Using fallback account ${fallbackAccount.accountName} for payment method ${paymentMethod}`);
