 ...
 * or the Bank - Primary (1130) dropdown, not as separate top-level rows.
 */
...
export const PHINDU_COA_STRUCTURE = [
  {
    code: '1000',
    name: 'Assets',
    children: [
      {
        ...
        children: [
          ...
          { code: '1120', name: 'Cash - Petty Cash' },
          {
            ...
            name: 'Bank - Primary',
            children: [
              { code: '1130-01', name: 'National Bank' },
              { code: '1130-02', name: 'Standard Bank' },
            ],
          },
  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        ...
        name: 'Salaries & Wages',
        children: [
          { code: '5201', name: 'Admin & Management Salaries' },
          { code: '5202', name: 'Sales & Distribution Wages' },
          { code: '5203', name: 'Production & Operations Wages' },
          { code: '5210', name: 'Employer PAYE & Contributions' },
        ],
      },
      ...
      { code: '5500', name: 'Bank Charges & Interest' },
      {
        code: '5900',
...
export function flattenStructureCodes(nodes = PHINDU_COA_STRUCTURE, /** @type {Set<string>} */ out = new Set()) {
...
export const PHINDU_STRUCTURE_CODES = flattenStructureCodes();
 ...
 * @param {string} structureNodeName — display name from PHINDU_COA_STRUCTURE
 ...
 * @param {{ h1130?: unknown[], c1999?: unknown[], c2999?: unknown[], c3999?: unknown[], c4900?: unknown[], c5900?: unknown[], cap3100?: unknown[] }} buckets — same shape as PhinduLedgerCoaTable `dropdownBuckets`
 */
...
/**
 * Display balance for a PHINDU structure row: same-code ledger rows + dropdown bucket (range catch-alls, 1130 extras, 3101–3199).
...
export function structureRowDisplayBalance(matches, nodeCode, buckets = {}) {
  return sumLedgerBalances(matches || []) + catchAllBucketExtraForStructureCode(nodeCode, buckets);
}
...
export function accountsForCatchAllDropdown(accounts, catchCode) {
  const flat = PHINDU_STRUCTURE_CODES;
  return accounts.filter((a) => {
    const c = normAccountCode(a.accountCode || a.code);
