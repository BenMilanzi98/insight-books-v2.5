...
/** Human label for a period column in budget/forecast grids. */
export function formatPeriodLabel(period, frequency) {
  const f = String(frequency || 'MONTHLY').toUpperCase();
  const y = period.periodStart.getUTCFullYear();
  if (f === 'QUARTERLY') {
    return `Q${period.quarterNumber} ${String(y).slice(2)}`;
  }
  if (f === 'ANNUAL' || f === 'YEARLY') {
    return String(y);
  }
  const m = period.monthNumber || period.periodStart.getUTCMonth() + 1;
  return `${MONTH_SHORT[m - 1]} ${String(y).slice(2)}`;
}

/** Period columns for a budget grid (key, label, dates). */
export function buildBudgetPeriodColumns(frequency, startDate, endDate) {
  return buildPeriods(frequency, startDate, endDate).map((p) => ({
    ...p,
    label: formatPeriodLabel(p, frequency),
  }));
}
