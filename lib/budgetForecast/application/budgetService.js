...
import { buildPeriods, buildMonthlyPeriods, parsePeriodKey, spreadEvenly } from '../domain/periods.js';
import {
...
export async function saveBudgetLines(tenantId, budgetId, linesInput = [], userId = null, { allowEmpty = false } = {}) {
  ...
  const periods = buildPeriods(budget.frequency, budget.startDate, budget.endDate);
    ...
    for (const row of linesInput) {
      ...
      await tx.budgetLine.create({
        data: {
          ...
          calculationMethod: row.calculationMethod || row.growthMode || 'MANUAL',
...
export async function getBudgetDashboard(tenantId) {
  ...
  for (const b of budgets) {
    ...
    for (const line of b.lines || []) {
      ...
      else if (t.includes('expense') || t.includes('cost')) plannedExpense += amt;
    }
  }
  const active =
    budgets.find((b) => b.status === BUDGET_STATUS.ACTIVE) ||
    budgets.find((b) => b.status === BUDGET_STATUS.APPROVED) ||
    null;
  const awaiting = budgets.filter((b) => ['READY_FOR_REVIEW', 'IN_REVIEW'].includes(b.status));
  const drafts = budgets.filter((b) =>
    ['DRAFT', 'IN_PREPARATION', 'CHANGES_REQUESTED'].includes(b.status)
  );

  let actualRevenue = 0;
  let actualExpense = 0;
  let actualsPeriodLabel = null;
  const varianceCallouts = [];

  if (active?.startDate && active?.endDate) {
    const ytdEnd = new Date();
    const periodEnd = ytdEnd < new Date(active.endDate) ? ytdEnd : new Date(active.endDate);
    const periodStart = new Date(active.startDate);
    actualsPeriodLabel = `${String(periodStart.toISOString()).slice(0, 10)} → ${String(periodEnd.toISOString()).slice(0, 10)}`;
    ...
    try {
      const actuals = await resolveBudgetActuals({
        ...
        accountIds: (active.lines || []).map((l) => l.accountId).filter(Boolean),
      });
      for (const a of actuals.accounts) {
        if (a.kind === 'REVENUE' || a.kind === 'OTHER_INCOME') actualRevenue += a.actualMinor;
        else if (a.kind === 'EXPENSE' || a.kind === 'COST_OF_SALES' || a.kind === 'OTHER_EXPENSE') {
          actualExpense += a.actualMinor;
        }
      }

      const full = await loadBudget(tenantId, active.id);
      const { computeVariance, classifyAccountKind } = await import('../domain/variance.js');
      const { describeVariance } = await import('../domain/varianceCopy.js');
      const scored = [];
      for (const line of full.lines || []) {
        ...
        for (const p of line.periodAmounts || []) {
          if (p.periodEnd < periodStart) continue;
          if (p.periodStart > periodEnd) continue;
          budgetMinor += minorToNumber(p.plannedAmountMinor);
        }
        if (!budgetMinor) budgetMinor = minorToNumber(line.annualAmountMinor);
        ...
        if (variance.status === 'ON_TRACK' || variance.status === 'NO_ACTUAL') continue;
        scored.push({
          line,
          kind,
          variance,
          absFav: Math.abs(Number(variance.favourableVarianceMinor) || 0),
        });
      }
      scored.sort((a, b) => b.absFav - a.absFav);
      for (const row of scored.slice(0, 4)) {
        varianceCallouts.push({
          key: row.line.accountId,
          budgetId: active.id,
          isFavourable: row.variance.isFavourable,
          message: describeVariance({
            accountName: row.line.accountNameSnapshot || row.line.accountCodeSnapshot,
            kind: row.kind,
            status: row.variance.status,
            favourableVarianceMinor: row.variance.favourableVarianceMinor,
            variancePercent: row.variance.variancePercent,
            currency: active.currency || 'MWK',
          }),
        });
      }
    } catch {
      // Dashboard still returns planned figures if actuals fail.
    }
  }
  ...
  return {
    cards: {
      ...
      annualBudget: (plannedRevenue + plannedExpense) / 100,
      actualRevenue: actualRevenue / 100,
      actualExpense: actualExpense / 100,
      actualProfit: (actualRevenue - actualExpense) / 100,
      activeStatus: active?.status || 'NONE',
      activeBudgetName: active?.name || null,
      actualsPeriodLabel,
