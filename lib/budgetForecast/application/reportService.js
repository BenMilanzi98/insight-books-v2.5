...
import { buildPnlVarianceLayout, buildVarianceInsight } from '../domain/pnlVarianceLayout.js';
import { buildPnlGroupedForBudgetPlan } from '../domain/pnlBudgetLayout.js';
...
export async function reportBudgetVsActual(tenantId, { budgetId, startDate, endDate, branchId } = {}) {
  ...
  const enrichedLines = withVarianceMessages(lines, { currency: budget.currency || 'MWK' });
  const pnlGrouped = buildPnlVarianceLayout({ varianceLines: enrichedLines, showAdvanced: false });
  ...
  return {
    ...
    lines: enrichedLines,
    pnlGrouped,
    insight: buildVarianceInsight(pnlGrouped.summary),
...
export async function reportBudgetPlan(tenantId, { budgetId, startDate, endDate } = {}) {
  ...
  const pnlGrouped = buildPnlGroupedForBudgetPlan(budget, lines, {
    startDate: from,
    endDate: to,
    showAdvanced: false,
  });
  const currency = budget.currency || 'MWK';
  const fmt = (n) =>
  ...
  return {
    ...
    lines,
    pnlGrouped,
    insight: `Planned net profit: ${fmt(pnlGrouped.summary.netProfit)}`,
    totals: {
      revenue: pnlGrouped.summary.revenue,
      grossProfit: pnlGrouped.summary.grossProfit,
      netProfit: pnlGrouped.summary.netProfit,
    },
