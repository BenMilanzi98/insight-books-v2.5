 ...
 *
 * Do not use Prisma `NOT: { field: value }` for nullable fields — SQL three-valued logic
 * drops rows where the field is NULL (that blanked the entire chart after till-float hide).
 */
...
/**
 * Drop internal POS till-float GL (1112) from chart/picker results in JS.
 * Safe with nullable systemPurpose/code fields (unlike Prisma NOT equality).
 */
export function withoutPosTillFloatCoaAccounts(accounts = []) {
  const CODE = '1112';
  const PURPOSE = 'POS_TILL_FLOAT';
  return (accounts || []).filter((a) => {
    const code = String(a?.accountCode ?? a?.code ?? '').trim();
    if (code === CODE) return false;
    const purpose = String(a?.systemPurpose ?? a?.controlAccountPurpose ?? '').trim();
    if (purpose === PURPOSE) return false;
    return true;
  });
}
