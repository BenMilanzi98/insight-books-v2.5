...
export async function buildPayeSummaryReport(params) {
  ...
  const where = {
    tenantId: tenantWhereIn(tenantIds),
  };
  ...
  if (payrollIds.length) {
    const txs = await db.transaction.findMany({
      ...
      select: { sourceId: true, sourceType: true, isReversal: true },
    });
    for (const tx of txs) {
      if (!tx.sourceId) continue;
      if (!journalByPayrollId.has(tx.sourceId)) {
        journalByPayrollId.set(tx.sourceId, { posted: true, reversed: false });
      }
    }
    const revs = await db.transaction.findMany({
      where: {
        tenantId: tenantWhereIn(tenantIds),
        sourceId: { in: payrollIds },
        isReversal: true,
    ...
    for (const r of revs) {
      if (r.sourceId && journalByPayrollId.has(r.sourceId)) {
        journalByPayrollId.get(r.sourceId).reversed = true;
      }
    }
  }
