...
/**
 * Same rules as chart GET /api/chart-of-accounts inventory aggregate.
 * @param {import('@prisma/client').PrismaClient} prisma
 * @param {string} tenantId
 * @param {string|null|undefined} branchId
 */
export async function computeInventoryStockAggregate(prisma, tenantId, branchId) {
  const invWhere = {
    tenantId,
    isService: false,
    isDeleted: false,
  ...
  for (const product of inventoryProducts) {
    try {
      ...
      const productValue =
        stored != null && !Number.isNaN(stored) && stored > 0 ? stored : stockLevel * cost;
      total += productValue;
    } catch {
      // skip
    }
  ...
  return { total, productCount: inventoryProducts.length, productsSampled: inventoryProducts.length };
}
 ...
 * @param {{ branchId?: string|null, maxInvoiceDetailLines?: number }} [opts]
 */
export async function computeCoaAccountBalanceBreakdown(
  ...
  if (!hasChildren && isInventoryLedger && !hasPostedGlActivity) {
    inventoryDetail = await computeInventoryStockAggregate(prisma, tenantId, branchId);
  }
  ...
  if (accountCode === '1300' && (accountType === 'ASSET' || accountType === 'Asset')) {
    notes.push(
      'Canonical 1300 inventory: after load, the chart reallocates the stock aggregate across the 1300 subtree so the section matches Stock Management.'
    );
  }
