...
export async function computeCoaAccountBalanceBreakdown(
  ...
  if (hasChildren) {
    ...
    if (!hasPostedGlActivity && legacyBalance !== 0) {
      notes.push(
        'Stored Account.balance was ignored for this parent/header because it is not an auditable direct posting and would duplicate child rollups.'
      );
    }
  }
  ...
  return {
    ...
    legacyStoredBalanceIgnored: hasChildren && !hasPostedGlActivity ? legacyBalance : 0,
    components,
    notes,
  };
}
