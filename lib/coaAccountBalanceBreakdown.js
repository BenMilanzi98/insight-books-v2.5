 ...
 */
export async function computeArSubledgerFromInvoices(prisma, tenantId, { maxLines = 100, asOfDate = null } = {}) {
  const allInvoices = await prisma.invoice.findMany({
    where: {
      ...
      ...(asOfDate
        ? {
            issueDate: {
              lte: asOfDate instanceof Date ? asOfDate : new Date(asOfDate),
            },
          }
        : {}),
    },
 ...
 * @param {{ asOfDate?: Date|null }} [valuationOpts]
 */
export async function computeInventoryStockAggregate(
  prisma,
  tenantId,
  user,
  searchParams,
  ...
  const r = await computePhysicalInventoryValuationTotal(
    prisma,
    tenantId,
    user,
    searchParams,
    valuationOpts.asOfDate ? { asOfDate: valuationOpts.asOfDate } : {}
  );
  return {
    total: r.total,
    productCount: r.productCount,
    productsSampled: r.productCount,
 ...
 * @param {{ inventoryUser?: object|null, inventorySearchParams?: URLSearchParams, maxInvoiceDetailLines?: number, dateRange?: { from?: Date|null, to?: Date|null, invalid?: boolean }, glBranchFilter?: Record<string, string>, fiscalYearStartMonth?: number }} [opts]
 */
export async function computeCoaAccountBalanceBreakdown(
  ...
  const dateRange = opts.dateRange ?? { from: null, to: null, invalid: false };
  const glBranchFilter = opts.glBranchFilter ?? {};
  const fiscalYearStartMonth =
    opts.fiscalYearStartMonth ?? (await getTenantFiscalYearStartMonth(tenantId, prisma));
  ...
  const j = {
    totalDebit: ja.debit,
    totalCredit: ja.credit,
    lineCount: ja.lineCount,
  };
  const t = {
    totalDebit: tx.debit,
  ...
  const childCount = await prisma.account.count({ where: { tenantId, parentAccountId: account.id } });
  ...
  let arDetail = null;
  const arAsOf = dateRange?.to || dateRange?.from || null;
  if (!hasChildren && isAccountsReceivableLeaf && !hasPostedGlActivity) {
    arDetail = await computeArSubledgerFromInvoices(prisma, tenantId, {
      ...
      ...(arAsOf ? { asOfDate: arAsOf } : {}),
    });
  }

  let inventoryDetail = null;
  if (!hasChildren && isInventoryLedger && !hasPostedGlActivity && inventoryUser) {
    const alignedInv = alignInventorySearchParamsWithGlBranch(
      inventorySearchParams instanceof URLSearchParams
  ...
  // Phase 4: Account.balance is not financial SoT — expose for diagnostics only.
  ...
  if (isAccountsReceivableLeaf || isInventoryLedger) {
    ...
    // Prefer 0 over stored Account.balance for statement figures.
    finalBalance = 0;
  }
  ...
  if (
    ...
    !hasPostedGlActivity &&
    legacyBalance !== 0 &&
    !isAccountsReceivableLeaf &&
    !isInventoryLedger
  ) {
    components.push({
      id: 'legacy_account_balance_field',
      label: 'Stored Account.balance (legacy, ignored)',
      amount: legacyBalance,
      note: 'Diagnostic only — not used for displayed balance (Phase 4 / V2 ledger cutover).',
    });
  }
  ...
  if (hasChildren) {
    ...
    if (!hasPostedGlActivity && legacyBalance !== 0) {
      notes.push(
        'Stored Account.balance was ignored (Phase 4): Account.balance is not financial SoT; balances come from posted GL / V2 ledger.'
      );
    }
  } else if (!hasPostedGlActivity && legacyBalance !== 0 && !isAccountsReceivableLeaf && !isInventoryLedger) {
    notes.push(
      'Stored Account.balance was ignored (Phase 4); displayed balance falls back to 0 when there is no posted activity.'
    );
  }
