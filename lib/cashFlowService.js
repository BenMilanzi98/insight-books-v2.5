...
export async function generateCashFlowFromAccounts(
  ...
  for (const account of cashAccounts) {
    const openingBalance = await getAccountBalanceDetails(account.id, tenantId, startDateMinusOne, prisma);
    const closingBalance = await getAccountBalanceDetails(account.id, tenantId, end, prisma);
    openingCashBalance += openingBalance.balance;
    closingCashBalance += closingBalance.balance;
  }

  // Get all transactions in the period that affect cash accounts
