...
export async function generateCashFlowFromAccounts(
  ...
  const customerPayments = await prisma.payment.findMany({
    ...
    include: {
      ...
      sale: { select: { saleNumber: true, client: { select: { name: true } } } }
    }
  });
  ...
  const capitalTypes = ['Capital', 'Owner Contribution', 'Investment', 'Owner capital', 'capital injection'];
  const otherTypes = ['Income', 'Loan', 'Other', 'Refund', 'Asset Sale', 'Miscellaneous', 'Cash received'];
  ...
  const payrollPaid = await prisma.payroll.findMany({
    where: {
      tenantId,
      paymentDate: dateFilter,
      status: { in: ['Processed', 'Paid', 'Posted'] }
    },
  ...
  const previousPeriodEndStr = startMinusOne.toISOString().split('T')[0];
