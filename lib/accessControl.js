...
export async function requirePremiumAccess(request) {
  try {
    ...
    const hasPaid = await hasPremiumAccess(user.tenantId);
    if (hasPaid) {
      return null;
    }

    // Owner / Admin / Master admin on an active trial: same access as paid for gated "premium" routes
    // (e.g. add-business) while the tenant trial is still valid.
    if (isFullAccessTenantRole(user)) {
      const standard = await hasStandardAccess(user.tenantId);
      if (standard) {
        return null;
      }
      const trialOk = await isTenantTrialActive(user.tenantId);
      if (trialOk) {
        return null;
      }
    }
