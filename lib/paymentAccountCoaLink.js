...
/**
 * When true, also auto-create missing cash/bank *parent* shells (1110 / 1130 / channel parents)
 * if the tenant CoA is incomplete. Payment *child* leaves under an existing parent (e.g. 1132-01)
 * are always created when adding a payment account — that is required for posting.
 */
const ALLOW_COA_AUTO_CREATE = process.env.ALLOW_COA_AUTO_CREATE !== 'false';
...
export async function ensurePaymentTypeMainGlAccounts(tenantId, tx = prisma) {
  ...
  await ensurePaymentChannelParentGlAccounts(tenantId, group1130?.id || parentId, tx);
  await ensurePaymentGlParentRollupFlags(tenantId, tx);
}

/**
 * Ensure bank/mobile channel parents (1131–1138, 1140, 1141) exist as rollup rows.
 * Required so payment accounts can auto-create posting children (e.g. 1132-01).
 */
async function ensurePaymentChannelParentGlAccounts(tenantId, defaultParentId, tx = prisma) {
  const group1130 = await findAccountByTenantCode(tenantId, BANK_GROUP_CODE, tx);
  const parentId = group1130?.id || defaultParentId;
  if (!parentId) return;

  for (const channel of PAYMENT_GL_CHANNELS) {
    const existing = await findAccountByTenantCode(tenantId, channel.code, tx);
    if (existing?.id) {
      const patch = {};
      if (existing.parentAccountId == null) patch.parentAccountId = parentId;
      if (existing.acceptsNewTransactions !== false) patch.acceptsNewTransactions = false;
      if (existing.accountSubtype !== 'Group') patch.accountSubtype = 'Group';
      if (Object.keys(patch).length) {
        await tx.account.update({ where: { id: existing.id }, data: patch });
      }
      continue;
    }
    if (!ALLOW_COA_AUTO_CREATE) continue;
    try {
      await tx.account.create({
        data: {
          ...
          acceptsNewTransactions: false,
          visibleInChart: true,
          description: `Payment channel parent (${channel.accountType}) — post to child accounts only`,
          balance: 0,
        },
      });
    } catch (error) {
...
export async function allocateNextPaymentChildCode(tenantId, parentCode, tx = prisma) {
  ...
  for (let slot = 1; slot <= MAX_PAYMENT_CHILD_SUFFIX; slot += 1) {
    if (!used.has(slot)) {
      return `${parentCode}-${String(slot).padStart(2, '0')}`;
    }
  }
  return null;
}
...
async function createPaymentChildGlAccount(tenantId, parentAccount, paymentAccount, tx) {
  ...
  try {
    ...
    if (error?.code === 'P2002') {
      // Race: another request took this slot — allocate again once.
      const retryCode = await allocateNextPaymentChildCode(tenantId, parentCode, tx);
      if (!retryCode || retryCode === codeStr) return null;
  ...
  if (strategy === 'bank_tree') {
    ...
    if (!parent?.id) {
      throw new PaymentGlSlotsExhaustedError(
        parentCode
          ? `GL parent account ${parentCode} is not set up in your chart of accounts. Run CoA setup or import accounts 1131–1138, 1140, and 1141 first.`
    ...
    if (!child) {
      throw new PaymentGlSlotsExhaustedError(
        `The maximum number of accounts under ${normalizedGlCode(parent)} (${MAX_PAYMENT_CHILD_SUFFIX} slots) has been reached. Archive unused payment GL children or use another bank channel.`,
        paymentAccount.accountType
      );
    }
