 ...
 */
export async function buildTrialBalance({ tenantId, branchId, startDate, endDate, includeZero = false }) {
  const { start, end } = toDateRange(startDate, endDate);

  // Query JournalEntry records
  const journalWhere = {
    tenantId,
    status: 'Posted',
  ...
  // IMPORTANT: Avoid relation filters inside groupBy (can fail depending on Prisma/DB).
  // Do a two-step query: get journalEntryIds, then groupBy lines by journalEntryId list.
  ...
  // Query Transaction records (from payment processing)
  ...
  const transactionGrouped = transactionIds.length
    ? await prisma.transactionLine.groupBy({
        by: ['accountId'],
  ...
  // Combine both grouped results
  const accountBalanceMap = new Map();

  // Add JournalEntry line totals
  journalGrouped.forEach((g) => {
    const existing = accountBalanceMap.get(g.accountId) || { debitAmount: 0, creditAmount: 0 };
    accountBalanceMap.set(g.accountId, {
      debitAmount: existing.debitAmount + asNumber(g._sum?.debitAmount),
      creditAmount: existing.creditAmount + asNumber(g._sum?.creditAmount),
    });
  });

  // Add Transaction line totals
  transactionGrouped.forEach((g) => {
    const existing = accountBalanceMap.get(g.accountId) || { debitAmount: 0, creditAmount: 0 };
    accountBalanceMap.set(g.accountId, {
      debitAmount: existing.debitAmount + asNumber(g._sum?.debitAmount),
      creditAmount: existing.creditAmount + asNumber(g._sum?.creditAmount),
  ...
  // Pull account metadata (code/name/type/normal balance)
  ...
  const rowsRaw = grouped.map((g) => {
    ...
    // If account is missing/inactive, still emit a placeholder so totals remain traceable
    ...
    const normal = (acc?.normalBalance || (type === 'Asset' || type === 'Expense' ? 'Debit' : 'Credit')).toString();

    // For trial balance display, show net balance in either debit or credit column
    let debit = 0;
    let credit = 0;
  ...
  // Order: Assets, Liabilities, Equity, Income, Expenses, then by account code
  rows.sort((a, b) => {
    const ta = TYPE_ORDER.indexOf(a.type);
    const tb = TYPE_ORDER.indexOf(b.type);
    if (ta !== tb) return (ta === -1 ? 999 : ta) - (tb === -1 ? 999 : tb);
    // numeric-ish code sort fallback to lexicographic
