 ...
 */
async function findAccountByTenantCode(tenantId, code, tx = prisma) {
  return tx.account.findFirst({
    where: {
      tenantId,
      OR: [{ code }, { accountCode: code }].filter(Boolean),
    },
  });
}
...
async function getOrCreateCOGSAccounts(tenantId, tx = prisma) {
  ...
  // Define required accounts (codes align with common CoA; COGS must exist for summaries / manual COGS API)
  ...
  for (const accountData of requiredAccounts) {
    let account = await findAccountByTenantCode(tenantId, accountData.code, tx);
  ...
  // POS/GL often use 5100 as "Cost of Sales" — treat as COGS for lookups
  if (!accounts.costofgoodssold) {
    ...
    if (fallback) accounts.costofgoodssold = fallback;
  }
