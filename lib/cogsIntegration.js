...
export async function recordPurchaseFromSupplier(purchaseData) {
  ...
  const transaction = await prisma.$transaction(async (tx) => {
    ...
    const glTransaction = await postGlEntry({
      tenantId,
      userId,
      entryDate,
      ...
      if (product) {
        ...
        await tx.product.update({
          ...
          data: {
            ...
            cost: item.cost // Update Order Price
          }
        });
...
export async function getCOGSTransactionStats(tenantId, startDate, endDate, branchId = null, branchIdsIn = null) {
  ...
  const saleWhere = validSaleReportWhere(tenantId, 'saleDate', startDate, endDate);
  ...
  const invoiceWhere = validInvoiceReportWhere(tenantId, 'issueDate', startDate, endDate);
  if (branchId) {
