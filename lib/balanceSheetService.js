...
async function getAccountBalanceAsOfDate(accountId, tenantId, asOfDate, accountCode = null, branchId = null) {
  const isCashAccount = accountCode && isCashAccountCodeForBalanceSheet(accountCode);
...
export async function generateBalanceSheetFromAccounts(tenantId, asOfDate, companyName = 'Company', logoUrl = null, branchId = null) {
  ...
  const cashAccountCodes = LEGACY_CASH_BALANCE_CODES.filter((c) => c !== '1000');
  ...
  for (const account of categorized.assets.current) {
    ...
    // Cash: canonical 1110, bank/mobile 1130-xx, legacy 1010–1050
    const isCashAccount =
      isCashAccountCodeForBalanceSheet(accountCode) ||
      subtype.includes('cash') ||
      accountName.includes('cash') ||
      accountName.includes('bank') ||
      accountName.includes('airtel') ||
      accountName.includes('mpamba') ||
      accountName.includes('paychangu');
  ...
  assets.currentAssets.lineItems = [
    ...assets.currentAssets.cashAccounts
      ...
      .map(a => ({
        ...
        value: a.balance || 0,
        drillDown: { type: 'Account', items: [toAccountDrillItem(a)] }
      })),
    {
  ...
  // Only show accounts with a non-zero balance on the report.
  assets.currentAssets.lineItems = assets.currentAssets.lineItems.filter((li) =>
    isNonZeroMoneyAmount(li.value)
  );
  ...
  ].filter((li) => isNonZeroMoneyAmount(li.value));
  ...
  liabilities.currentLiabilities.lineItems = [
    ...
    ...(liabilities.currentLiabilities.shortTermLoans ? [{
      ...
      drillDown: liabilities.currentLiabilities.shortTermLoanAccounts.length
        ? { type: 'Account', items: liabilities.currentLiabilities.shortTermLoanAccounts.map(toAccountDrillItem) }
        : undefined
    }] : []),
    ...(liabilities.currentLiabilities.taxPayable ? [{
      key: 'tax-payable',
      ...
      drillDown: liabilities.currentLiabilities.taxAccounts.length
        ? { type: 'Account', items: liabilities.currentLiabilities.taxAccounts.map(toAccountDrillItem) }
        : undefined
    }] : []),
  ...
  ].filter((li) => li && typeof li === 'object' && isNonZeroMoneyAmount(li.value));
  ...
  liabilities.nonCurrentLiabilities.lineItems = [
    ...(liabilities.nonCurrentLiabilities.longTermLoans ? [{
      ...
      drillDown: liabilities.nonCurrentLiabilities.longTermLoanAccounts.length
        ? { type: 'Account', items: liabilities.nonCurrentLiabilities.longTermLoanAccounts.map(toAccountDrillItem) }
        : undefined
    }] : []),
  ...
  liabilities.lineItems = [
    ...liabilities.currentLiabilities.lineItems.filter((li) => isNonZeroMoneyAmount(li.value)),
    ...liabilities.nonCurrentLiabilities.lineItems.filter((li) => isNonZeroMoneyAmount(li.value))
  ];
  ...
  equity.lineItems = [
      ...
      })),
    {
      ...
      drillDown: equity.retainedAccounts.length
        ? { type: 'Account', items: equity.retainedAccounts.map(toAccountDrillItem) }
        : undefined
    },
    {
