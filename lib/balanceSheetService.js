...
export async function generateBalanceSheetFromAccounts(tenantId, asOfDate, companyName = 'Company', logoUrl = null, branchId = null) {
  ...
  // Get Accounts Receivable directly from unpaid invoices (more accurate than account balances)
  ...
  unpaidInvoices.forEach(invoice => {
    const actualTotalPaid = invoice.payments.reduce((sum, p) => sum + (parseFloat(p.amount) || 0), 0);
    ...
    if (isUnpaid) {
      assets.currentAssets.accountsReceivable.total += actualRemaining;
  ...
  for (const account of categorized.assets.current) {
    ...
    if (isCashAccount) {
      ...
      // Include all inventory accounts, even with zero balance
  ...
  assets.currentAssets.lineItems = [
      ...
      })),
    {
      ...
      value: assets.currentAssets.prepaidExpenses || 0
    },
  ...
  assets.currentAssets.lineItems = assets.currentAssets.lineItems.filter(li => {
    // Always show accounts receivable, inventory, and prepaid expenses even if zero
    if (li.key === 'accounts-receivable' || li.key === 'prepaid-expenses' || li.key?.includes('inventory-')) {
      return true;
    }
    // Show other items if they have a balance or if they're explicitly in otherItems
    return Math.abs(li.value || 0) > 0.000001 || li.key?.includes('other-current-');
  });
  ...
  for (const account of categorized.assets.fixed) {
    ...
    if (subtype.includes('depreciation') || accountName.includes('depreciation')) {
      ...
      // Include all PP&E accounts, even with zero balance
  ...
  for (const account of categorized.assets.other) {
    ...
    if (subtype.includes('intangible')) {
      ...
      // Include all other non-current asset accounts, even with zero balance
  ...
  // Build dynamic non-current asset line items (for UI)
  // Include all accounts even with zero balances for complete balance sheet
  assets.nonCurrentAssets.lineItems = [
    ...
    // Always show intangible assets line item even if zero
    {
    ...
    ...assets.nonCurrentAssets.otherItems
      .sort((a, b) => (a.accountCode || '').localeCompare(b.accountCode || ''))
      .map(a => ({
        key: `other-noncurrent-${a.accountId}`,
        label: a.accountName || 'Other Non-Current Asset',
        value: a.balance || 0,
        drillDown: { type: 'Account', items: [{ accountCode: a.accountCode, accountName: a.accountName, balance: a.balance }] }
      }))
  ];
  // Only filter out truly zero items if they're not explicitly needed
  // But show all accounts that have been categorized
  assets.nonCurrentAssets.lineItems = assets.nonCurrentAssets.lineItems.filter(li => {
    // Always show PP&E and intangible assets even if zero
    if (li.key === 'ppe-net' || li.key === 'intangible-assets') {
      return true;
    }
    // Show other items if they have a balance or if they're explicitly in otherItems
    return Math.abs(li.value || 0) > 0.000001 || li.key?.includes('other-noncurrent-');
  });
  ...
  unpaidExpenses.forEach(expense => {
    ...
    const actualPaid = expense.payments.reduce((sum, p) => sum + (parseFloat(p.amount) || 0), 0);
    const balanceDue = Math.max(0, amount - actualPaid);
  ...
  for (const account of categorized.liabilities.current) {
    ...
    if (isPayableAccount) {
      ...
      if (balance !== 0) {
        liabilities.currentLiabilities.accruedAccounts.push({
          accountId: account.id,
          accountCode: account.accountCode,
          accountName: account.accountName,
          balance
  ...
  // Build dynamic current liability line items (for UI)
  liabilities.currentLiabilities.lineItems = [
    ...
    ...(liabilities.currentLiabilities.accruedExpenses ? [{
      ...
      value: liabilities.currentLiabilities.accruedExpenses || 0,
      drillDown: liabilities.currentLiabilities.accruedAccounts.length
        ? { type: 'Account', items: liabilities.currentLiabilities.accruedAccounts.map(a => ({ accountCode: a.accountCode, accountName: a.accountName, balance: a.balance })) }
        : undefined
    }] : []),
  ...
  for (const account of categorized.liabilities.longTerm) {
    ...
    if (subtype.includes('loan') || accountName.includes('loan')) {
      ...
      liabilities.longTermLiabilities.bondsPayable += balance; // Alias
      if (balance !== 0) {
        liabilities.nonCurrentLiabilities.bondAccounts.push({
          accountId: account.id,
          accountCode: account.accountCode,
          accountName: account.accountName,
          balance
      ...
      liabilities.nonCurrentLiabilities.otherNonCurrentLiabilities += balance; // Alias
      liabilities.longTermLiabilities.otherLongTermLiabilities += balance; // Alias
      if (balance !== 0) {
  ...
  for (const account of categorized.liabilities.other) {
    ...
    if (subtype.includes('long') || subtype.includes('term') || accountName.includes('long') || accountName.includes('term')) {
      if (subtype.includes('bond') || accountName.includes('bond')) {
        liabilities.nonCurrentLiabilities.bondsPayable += balance;
        liabilities.longTermLiabilities.bondsPayable += balance; // Alias
        if (balance !== 0) {
          liabilities.nonCurrentLiabilities.bondAccounts.push({
  ...
  liabilities.longTermLiabilities.total = 
    ...
    liabilities.longTermLiabilities.bondsPayable +
  ...
  // Build dynamic non-current liability line items (for UI)
  liabilities.nonCurrentLiabilities.lineItems = [
    ...
    ...(liabilities.nonCurrentLiabilities.bondsPayable ? [{
      key: 'bonds-payable',
      label: 'Bonds Payable',
      value: liabilities.nonCurrentLiabilities.bondsPayable || 0,
      drillDown: liabilities.nonCurrentLiabilities.bondAccounts.length
        ? { type: 'Account', items: liabilities.nonCurrentLiabilities.bondAccounts.map(a => ({ accountCode: a.accountCode, accountName: a.accountName, balance: a.balance })) }
        : undefined
    }] : []),
    ...liabilities.nonCurrentLiabilities.otherItems
      .sort((a, b) => (a.accountCode || '').localeCompare(b.accountCode || ''))
      .map(a => ({
        key: `other-noncurrent-liability-${a.accountId}`,
        label: a.accountName || 'Other Non-Current Liability',
        value: a.balance || 0,
        drillDown: { type: 'Account', items: [{ accountCode: a.accountCode, accountName: a.accountName, balance: a.balance }] }
      }))
  ].filter(li => Math.abs(li.value || 0) > 0.000001);
  ...
  equity.currentYearProfitLoss = totalRevenue - totalExpenses;
  equity.currentYearProfit = equity.currentYearProfitLoss; // Alias
  
  // Debug: Log equity components
  console.log('Equity Calculation:', {
    ownersCapital: equity.ownersCapital,
    capitalStock: equity.capitalStock,
    retainedEarnings: equity.retainedEarnings,
    currentYearProfitLoss: equity.currentYearProfitLoss,
  ...
  equity.total = equity.ownersCapital + equity.retainedEarnings + equity.currentYearProfitLoss + equity.otherEquity;
  ...
  return {
    ...
    balanceDifference: finalDifference,
