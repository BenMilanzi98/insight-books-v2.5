...
export function calculateNPS(grossSalary, employeeRatePercent, employerRatePercent) {
  ...
  const employeeAmount = percentOfMoney(salary, Number.isFinite(empPct) ? empPct : 0);
  const employerAmount = percentOfMoney(salary, Number.isFinite(erPct) ? erPct : 0);
  const totalAmount = addMoney(employeeAmount, employerAmount);
...
export function calculateCustomDeductions(grossSalary, deductions = []) {
  ...
  deductions.forEach(deduction => {
    ...
    if (pct != null && !Number.isNaN(pct) && pct > 0) {
      amount = percentOfMoney(salary, pct);
      deductionType = 'percentage';
    } else if (fix != null && !Number.isNaN(fix)) {
      amount = roundMoney(fix);
      deductionType = 'fixed';
    }

    totalDeductions = addMoney(totalDeductions, amount);
...
// parseMoney re-export for callers migrating off float parse
export { parseMoney };
...
export function calculatePayroll(grossSalary, selectedDeductions = [], options = {}) {
  ...
  const totalDeductions = addMoney(
    payeCalculation.payeAmount,
    npsCalculation.employeeAmount,
    customDeductionsCalculation.totalAmount
  );
  const netPay = subtractMoney(salary, totalDeductions);
