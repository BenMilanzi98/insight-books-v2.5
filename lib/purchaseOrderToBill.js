/**
 * Creates a SupplierBill from an approved service (or mixed) Purchase Order
 * so the PO automatically appears in Bills/Payables for payment processing.
...
/**
 * For an approved service/mixed PO, create one SupplierBill linked to the PO
...
export async function createBillFromApprovedServicePO(purchaseOrderId, tenantId, userId = null, tx = prisma) {
  ...
  if (!po || po.status !== 'Approved') return { bill: null, created: false };
  ...
  // Idempotent: if a bill already linked to this PO, return it
