...
import { getCurrentDateInAfricaBlantyre } from './dateUtils';
...
/**
 * Ensure an open monthly accounting period exists for the current calendar month.
 * Idempotent — skips creation when an overlapping monthly period already exists.
 *
 * @param {string} tenantId
 * @param {import('@prisma/client').PrismaClient | import('@prisma/client').Prisma.TransactionClient} [tx]
 * @returns {Promise<{ created: boolean, periodId?: string, periodName?: string }>}
 */
export async function ensureDefaultAccountingPeriodsForTenant(tenantId, tx = prisma) {
  if (!tenantId) {
    console.warn('ensureDefaultAccountingPeriodsForTenant: missing tenantId');
  ...
  if (existingOverlap) {
    return { created: false, periodId: existingOverlap.id, periodName: existingOverlap.name };
  }
  ...
  return { created: true, periodId: created.id, periodName: created.name };
}
...
export async function assertPeriodOpen(tenantId, entryDate, tx = prisma) {
  ...
  try {
    const periodCount = await tx.accountingPeriod.count({ where: { tenantId } });
    if (periodCount === 0) {
      console.warn(
        `[accountingPeriod] Tenant ${tenantId} has no accounting periods configured; allowing posting (legacy compat). ` +
          'Run ensureDefaultAccountingPeriodsForTenant or create periods in Accounting Periods.'
      );
      return;
    }
    ...
    if (!openPeriod) {
      const dateLabel =
        entryDate instanceof Date ? entryDate.toISOString().slice(0, 10) : String(entryDate);
      const error = new Error(
        `No open accounting period covers entry date ${dateLabel}. Create or reopen a period before posting.`
      );
      error.code = 'PERIOD_NOT_OPEN';
      throw error;
    }
  } catch (error) {
    if (error.code === 'PERIOD_NOT_OPEN' || error.code === 'PERIOD_LOCKED') {
      throw error;
    }
    console.warn('[accountingPeriod] Period open check failed (non-fatal):', error?.message || error);
  }
}
