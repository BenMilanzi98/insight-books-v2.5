...
/** Transaction rows sometimes store status as `posted` or `Posted` depending on code path / legacy data. */
...
export async function reverseSaleGlForRefundInTx({
  ...
  const journalOrBranches = [];
  if (saleSourceIdIn.length === 1) {
    journalOrBranches.push({ ...postedSaleJournalClause, sourceId: saleSourceIdIn[0] });
  } else if (saleSourceIdIn.length > 1) {
    journalOrBranches.push({ ...postedSaleJournalClause, sourceId: { in: saleSourceIdIn } });
  } else if (saleIdStr) {
    journalOrBranches.push({ ...postedSaleJournalClause, sourceId: saleIdStr });
  }
  // Legacy / mismatch: COGS row must still reverse if sourceId did not match sale id but description does
  if (saleNumberTrim) {
    journalOrBranches.push({
  ...
  const originalJournalEntriesRaw =
    journalOrBranches.length === 0
      ? []
      : await tx.transaction.findMany({
          where: { OR: journalOrBranches },
          include: {
            lines: true,
          },
        });
