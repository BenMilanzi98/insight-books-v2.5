...
async function validateReversalEligibility({ transactionId, transactionType, tenantId }) {
  ...
  if (transactionType === 'Invoice') {
    ...
    // Reject reversal while any completed non-reversal payment value remains on the invoice.
    const totalPaid = parseFloat(transaction.totalPaid || 0);
    if (totalPaid > 0) {
      return { isValid: false, error: 'Cannot reverse invoice with recorded payments; reverse/refund payments first.' };
    }
  }
...
async function createPaymentReversal({
  ...
  const v2 = await reverseSourceJournals({
    ...
    userId,
    reason: reasonValidation.reason,
    sourceTypes: ['Payment', 'Invoice-Revenue'],
