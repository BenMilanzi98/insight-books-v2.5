...
async function reversePayrollSideEffects(tx, { payrollId, reversalTransactionId, userId, tenantId, reason }) {
  ...
  // Match by same employee, period end date, and description/notes indicating payroll for this run
  ...
  // Operating-expense row created on process (date = paymentDate, notes contain payrollDashboardExpense:<payrollId>)
  const dashboardPayrollExpenses = await tx.expense.findMany({
    where: {
      tenantId,
      employeeId: payroll.employeeId,
      isReversal: false,
  ...
  for (const e of [...payrollExpenses, ...dashboardPayrollExpenses]) {
    if (e?.id) mergedById.set(e.id, e);
  }
  const payrollExpensesMerged = [...mergedById.values()];
  // Dedupe by id and filter to those that look like this payroll (description contains employee or known prefix)
  const seenIds = new Set();
  const payrollExpensesFiltered = payrollExpensesMerged.filter(e => {
    if (seenIds.has(e.id)) return false;
    seenIds.add(e.id);
    const notesRaw = e.notes || '';
    if (notesRaw.includes(dashboardExpenseMarker)) return true;
    const desc = (e.description || '').toLowerCase();
    const notes = notesRaw.toLowerCase();
    const hasPrefix = payrollExpenseDescriptions.some(p => desc.includes(p.toLowerCase()));
    const hasEmployee = employeeName && (desc.includes(employeeName.toLowerCase()) || notes.includes(employeeName.toLowerCase()));
    const hasPeriod = periodStr && notes.includes(periodStr.toLowerCase());
    return hasPrefix || (hasEmployee && (hasPeriod || ['salary', 'tax', 'paye', 'nps', 'pension', 'net pay', 'overtime'].some(k => desc.includes(k))));
  });
  const expenseIds = payrollExpensesFiltered.map(e => e.id);
...
async function createTransactionReversal({ 
  ...
  if (originalTransaction.sourceId && originalTransaction.sourceType && !isSaleCogsSubJournal) {
    ...
    if (['Invoice', 'Sale', 'Expense', 'Payroll'].includes(sourceType)) {
  ...
  const reversalTransaction = await prisma.$transaction(async (tx) => {
    ...
    const reversal = await tx.transaction.create({
      data: {
        ...
        createdById: userId,
        entryType: 'Reversal',
        notes: isSaleCogsSubJournal
          ? `COGS journal reversal only (sale revenue/tax unchanged). Original: ${originalTransaction.reference || originalTransaction.id}`
          : `Reversal of transaction: ${originalTransaction.reference || originalTransaction.id}`,
        postedById: userId,
        postedDate: new Date(),
    ...
    // Create opposite journal entries for main transaction (numeric coercion — avoids NaN / Decimal quirks)
    ...
    for (const taxTx of taxTransactions) {
      ...
      for (const taxLine of taxTx.lines) {
        const tDebit = parseFloat(taxLine.debitAmount ?? 0) || 0;
        const tCredit = parseFloat(taxLine.creditAmount ?? 0) || 0;
...
export async function reverseSaleGlForRefundInTx({
  ...
  const saleSourceIdIn = [
    ...new Set(
      [saleIdStr, saleId != null ? String(saleId) : '']
