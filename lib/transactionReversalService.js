  ...
  const reversalTransaction = await prisma.$transaction(async (tx) => {
    ...
    const reversal = await tx.transaction.create({
      data: {
        ...
        postedById: userId,
        postedDate: effectiveReversalDate,
    ...
    for (const line of originalLines) {
      ...
      const reversalLine = await tx.transactionLine.create({
        data: {
          ...
          createdAt: effectiveReversalDate,
          updatedAt: effectiveReversalDate
        }
      });
...
export async function reverseSaleGlForRefundInTx({
  ...
  const journalBySourceBranches = [];
  if (saleSourceIdIn.length === 1) {
    journalBySourceBranches.push({ ...postedSaleJournalClause, sourceId: saleSourceIdIn[0] });
  } else if (saleSourceIdIn.length > 1) {
    journalBySourceBranches.push({ ...postedSaleJournalClause, sourceId: { in: saleSourceIdIn } });
  } else if (saleIdStr) {
    journalBySourceBranches.push({ ...postedSaleJournalClause, sourceId: saleIdStr });
  }

  let originalJournalEntriesRaw =
    journalBySourceBranches.length === 0
      ? []
      : await tx.transaction.findMany({
          where: { OR: journalBySourceBranches },
          include: {
            lines: true,
          },
        });
