...
export async function createFifoBatch({
  ...
  const currentQtyAfterBatch = toNumber(product.stockLevel);
  const newQty = currentQtyAfterBatch + qty;
  const wac = newQty > 0 ? totalStockValue / newQty : cost;
  ...
  const updatedProduct = await tx.product.update({
    ...
    data: {
      ...
      // Weighted-average cost matches FIFO valuation (Order Price on /stock)
      cost: toDecimal(wac),
      averageCost: toDecimal(wac),
  ...
  const newQtyActual = toNumber(updatedProduct.stockLevel);
  console.log(`[FIFO] createFifoBatch: Product ${productId}, After increment: ${newQtyActual} (should be ${currentQty + qty})`);
  
  if (newQtyActual !== currentQty + qty) {
    console.error(`[FIFO] ⚠️ STOCK MISMATCH! Product ${productId}: Expected ${currentQty + qty}, Got ${newQtyActual}, Difference: ${newQtyActual - (currentQty + qty)}`);
  }

  return { batchId: batch.id, quantityOnHand: newQtyActual, totalStockValue };
}
...
export async function consumeFifoForSale({
  ...
  saleItemId = null,
  tx = prisma,
  /** When false, skips incrementing product.totalSoldQty (e.g. stock transfers). */
  updateSoldQty = true,
}) {
  ...
  await tx.product.update({
    ...
    data: {
      totalStockValue: toDecimal(totalStockValue),
      ...(updateSoldQty ? { totalSoldQty: { increment: toDecimal(qtyToSell) } } : {}),
    },
  });
