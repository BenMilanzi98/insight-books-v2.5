...
export async function createFifoBatch({
  ...
  if (sourceType && sourceId) {
    ...
    if (existingBatch) {
      ...
      return { quantityOnHand: toNumber(product.stockLevel), totalStockValue };
    }
  }
...
export async function consumeFifoForSale({
  ...
  const findBatches = async (branchIdToUse) => {
  ...
  let batches = await findBatches(branchId ? branchId : undefined);
  ...
  if (batches.length === 0 && branchId) {
    ...
    batches = await findBatches(null);
  }
