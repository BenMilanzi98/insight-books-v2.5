...
/**
 * Gross expense amount (base + tax) — matches AP / partial-payment totalDue and journal credit when tax is split.
 */
export function expenseGrossAmount(expense) {
  const base = Number(expense?.amount) || 0;
  const taxRaw = expense?.taxAmount;
  const tax = taxRaw != null && taxRaw !== '' ? Number(taxRaw) : 0;
  return base + (Number.isFinite(tax) ? tax : 0);
}

/**
 * Outstanding payable for a supplier-linked expense row (gross minus recorded paidAmount).
 */
export function expenseOutstandingPayable(expense) {
  const gross = expenseGrossAmount(expense);
  const paidRaw = expense?.paidAmount;
  const paid = paidRaw != null && paidRaw !== '' ? Number(paidRaw) : 0;
  const p = Number.isFinite(paid) ? paid : 0;
  return gross - p;
}

/** Supplier bills that still carry a payable (PO-from-receipt expense bills use status Unpaid). */
const SUPPLIER_OPEN_PAYABLE_STATUSES = [
  'Unpaid',
  'Overdue',
  'Approved',
  'Finalized',
...
export async function getSupplierFinancialSummary(supplierId, tenantId) {
  ...
  const pendingBills = await prisma.supplierBill.findMany({
    where: {
      supplierId,
      status: { in: SUPPLIER_OPEN_PAYABLE_STATUSES },
...
export async function updateSupplierBalance(supplierId, tenantId) {
  ...
  // Outstanding from supplier expenses: use gross (amount + taxAmount) minus paidAmount so tax is not
  // double-counted vs paidAmount (which applies to the full invoice including tax).
  const pendingSupplierExpenses = await prisma.expense.findMany({
    where: {
      supplierId,
      tenantId,
      isDeleted: false,
    ...
    select: { amount: true, taxAmount: true, paidAmount: true }
  });

  const expensesBalance = pendingSupplierExpenses.reduce(
    (sum, e) => sum + expenseOutstandingPayable(e),
    0
  );
...
export async function getSupplierExpenseSummary(supplierId, tenantId, options = {}) {
  ...
  const [expenseRowsForTotals, expensesCount] = await Promise.all([
  ...
  const totalAmount = expenseRowsForTotals.reduce((s, e) => s + expenseGrossAmount(e), 0);
