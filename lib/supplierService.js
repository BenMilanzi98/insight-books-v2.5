...
export async function updateSupplierBalance(supplierId, tenantId) {
  ...
  // IMPORTANT: Exclude pure tax expenses from supplier balance so "tax paid" does not inflate supplier currentBalance.
  const expensesResult = await prisma.expense.aggregate({
    where: {
      supplierId,
      tenantId,
      isDeleted: false,
      // Tax-category expenses (e.g. PAYE/VAT to MRA) should not be treated as amounts owed to the supplier.
      // This keeps supplier management focused on trade payables, not tax authorities.
      NOT: {
        category: 'Tax'
      },
