...
export async function loadCoaBulkGlAggregates(prisma, params) {
  ...
  if (unionIds.length) {
    ...
    const loadTxnLinesForReversalFlag = async (isReversal) => {
      if (!hasDateFilter) {
    ...
    const [txnRegular, txnReversals] = await Promise.all([
      loadTxnLinesForReversalFlag(false),
      loadTxnLinesForReversalFlag(true),
    ]);
    txnLines = [...txnRegular, ...txnReversals];
  }
