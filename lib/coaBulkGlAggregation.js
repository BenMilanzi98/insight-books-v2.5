 ...
 * merge survivor rollup, mirror-journal exclusion (transactionId null). Posted transaction lines include
 * reversals (isReversal true) so net GL on each account matches posted activity.
 */
import {
  accountClass,
...
function transactionWhereChunk({ tenantId, glBranchFilter, hasDateFilter, bounds, mode }) {
  const base = {
    tenantId,
...
export async function loadCoaBulkGlAggregates(prisma, params) {
  ...
  if (unionIds.length) {
      ...
      creditAmount: true,
      transaction: {
        select: { date: true, status: true, isReversal: true },
      },
    };
    if (!hasDateFilter) {
      txnLines = await prisma.transactionLine.findMany({
        where: {
          accountId: { in: unionIds },
          transaction: {
            tenantId,
      ...
      if (bsIds.size) {
        tchunks.push(
          prisma.transactionLine.findMany({
            where: {
              accountId: { in: [...bsIds] },
              transaction: transactionWhereChunk({
                tenantId,
                glBranchFilter,
                hasDateFilter,
                bounds,
      ...
      const tparts = await Promise.all(tchunks);
      const seenT = new Set();
