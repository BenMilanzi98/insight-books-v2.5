...
function journalEntryWhereChunk({ tenantId, glBranchFilter, hasDateFilter, bounds, mode }) {
  const hasBranch = glBranchFilter && Object.keys(glBranchFilter).length > 0;
  const base = {
    tenantId,
    architectureVersion: 'ACCOUNTING_V2',
  };
  if (!hasDateFilter || !bounds?.end) {
    return hasBranch ? { ...base, ...glBranchFilter } : base;
  }
  ...
  if (mode === 'BS') {
    ...
    if (!start) return hasBranch ? { ...base, ...glBranchFilter } : base;
    dateOr = [
  ...
  // Date OR must not clobber branch OR (branch + unassigned null postings).
  if (!hasBranch) {
    return { ...base, OR: dateOr };
  }
  return {
    ...base,
    AND: [glBranchFilter, { OR: dateOr }],
  };
}
