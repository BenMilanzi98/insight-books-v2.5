...
/**
 * Post-blueprint: reparent statutory + AP under 2100; salary detail accounts (5201–5215) under expense root 5000; **1130-** rows under 1130 group;
...
export async function runCanonicalCoaMigrationsPhase2(tenantId, tx = prisma) {
  ...
  const expense5000 = await tx.account.findFirst({
  ...
  if (expense5000?.id && salaries5200 && /salaries|wages/i.test(salaries5200.accountName || '')) {
    for (const code of ['5201', '5202', '5203', '5210', '5215']) {
      ...
      if (child && child.parentAccountId !== expense5000.id) {
        await tx.account.update({
          where: { id: child.id },
          data: { parentAccountId: expense5000.id },
        });
      }
    }
