 ...
 * block postings to consolidation rows (structural roots, parents with active children, acceptsNewTransactions false).
 */
...
/**
 * @param {Array<{ accountId: string }>} lines
 * @param {Array<object>} accounts - Prisma rows with code, acceptsNewTransactions, _count.childAccounts (active children)
 * @returns {{ ok: true } | { ok: false, error: string, details?: string }}
 */
export function validateNoPostingToStructuralCoaRoots(lines, accounts) {
  ...
  return validateLineAccountsAllowDirectPosting(orderedAccounts);
}
