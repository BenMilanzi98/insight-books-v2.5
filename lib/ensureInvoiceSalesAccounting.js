    ...
    for (const item of invoice.items || []) {
      ...
      try {
        ...
        if (
          ...
          // Payment path must not block cash application when stock was already
          // moved (or never reserved) on a Partial/open invoice. Issue path still fail-closed.
          if (force) {
            console.warn(
              `ensureInvoiceSalesAccounting: insufficient stock for "${product.name}" on payment force path — continuing without stock decrement`,
            );
            const cogsData = await calculateCOGS({
              ...
              console.warn('COGS calc skipped on force path:', e?.message || e);
              return { cogsAmount: 0 };
            });
            totalCOGS = addMoney(totalCOGS, cogsData.cogsAmount || 0);
            continue;
          }
          throw new Error(
