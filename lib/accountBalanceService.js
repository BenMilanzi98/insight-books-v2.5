 ...
 */
export async function updateAccountBalanceOnTransaction(accountId, debitAmount, creditAmount, tx = prisma) {
  const account = await tx.account.findUnique({
    where: { id: accountId },
    select: {
      id: true,
  ...
  // Do not select `acceptsNewTransactions` here — older generated Prisma clients (pre-column) reject it and break POS checkout.
  if (isCoaStructuralRootCode(glCode)) {
    throw new Error(
      `Cannot update balance on structural chart header ${glCode || accountId}. Post to a detail account under Assets, Liabilities, Equity, Revenue, or Expenses.`
    );
  }
...
export async function getAccountBalanceDetails(accountId, tenantId, asOfDate = null, tx = prisma, branchId = null) {
  ...
  return {
    ...
    recentTransactions: transactionLines.slice(0, 10).map(line => ({
