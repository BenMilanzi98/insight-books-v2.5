...
import { resolveOrEnsureStockOnHandGlAccount } from './inventoryGlAccount.js';
...
export async function updateAccountBalanceOnTransaction(
  ...
  if (block.blocked && !options.allowBlockedAccountForReversal) {
    ...
    const redirectDepth = Number(options._inventoryGlRedirectDepth || 0);
    if (glCode === '1300' && account.tenantId && redirectDepth < 5) {
      try {
        const leaf = await resolveOrEnsureStockOnHandGlAccount(account.tenantId, tx);
        if (leaf?.id && leaf.id !== accountId) {
          return updateAccountBalanceOnTransaction(leaf.id, debitAmount, creditAmount, tx, {
            ...options,
            _inventoryGlRedirectDepth: redirectDepth + 1,
          });
        }
      } catch {
