...
import { isFullAccessTenantRole } from './tenantRoleAccess';
...
export async function getUserFromSession(request) {
  try {
    ...
    try {
      ...
      // Resolve effective role per active tenant (multi-business RBAC).
      // Fail-safe: if the membership table doesn't exist yet (or query fails), fall back to the user's global role.
      if (user.tenantId) {
        try {
...
export function hasPermission(user, permission) {
  ...
  // Master admin and tenant Owner/Admin: full access (trial or paid), aligned with `/api/auth/me` Owner/Admin backfill.
  if (user.role.name === 'MASTER_ADMIN' || isFullAccessTenantRole(user)) {
    return true;
  }
  ...
  for (const p of permissionsToCheck(permission)) {
    ...
    if (category && action && perms?.[category]?.[action] === true) {
      return true;
    }
  }
  return false;
