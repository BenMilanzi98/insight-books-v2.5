...
export async function getUserFromSession(request) {
  try {
    ...
    try {
      ...
      // Get user data from database (include userBranches for branch separation when schema supports it)
      let user;
      try {
        ...
        user = await prisma.user.findUnique({
          ...
          select: {
            ...
            tenant: { select: { ownerUserId: true, defaultBranchId: true } }
          }
        });
      } catch (schemaError) {
        console.error('getUserFromSession lookup (new schema) failed, falling back to legacy select:', schemaError?.message || schemaError);
        // Legacy schema without branch separation: select only safe fields.
        user = await prisma.user.findUnique({
          where: { id: sessionData.userId },
          select: {
            id: true,
      ...
      try {
        ...
        // null = all branches (owner); [] = no access; non-empty = only those branches
        user.allowedBranchIds = isOwner ? null : branchIds;
