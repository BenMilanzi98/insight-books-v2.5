...
import { applyHiddenPrimaryBranchToUser } from './hiddenPrimaryBranch';
...
export async function getUserFromSession(request) {
  if (isDesktopRuntime()) {
    const { getDesktopSessionUser } = await import('./desktop/local/session');
    return getDesktopSessionUser(request);
  }
  ...
  try {
    ...
    try {
      ...
      // Revoked / expired tracked sessions (Phase 15) — fail closed when row exists
      if (sessionData.sessionId && typeof prisma.secV2UserSession?.findUnique === 'function') {
        try {
          const tracked = await prisma.secV2UserSession.findUnique({
            where: { id: sessionData.sessionId },
            select: { status: true, expiresAt: true },
          });
          if (tracked) {
            if (tracked.status !== 'ACTIVE') return null;
            if (tracked.expiresAt && tracked.expiresAt.getTime() < Date.now()) return null;
          }
        } catch {
          /* table may not exist pre-migration — allow legacy sessions */
        }
      }
      ...
      user.sessionId = sessionData.sessionId || null;

      await applyTenantMembershipRole(user, user.tenantId);

      // Suspended / revoked membership cannot access business data
      try {
        if (user.tenantId) {
          const membership = await prisma.tenantMembership.findUnique({
          ...
          if (membership) {
            ...
            user.membershipStatus = membership.status;
            const st = String(membership.status || '').toLowerCase();
            if (st && st !== 'active' && st !== 'invited') {
              return null;
            }
          }
        }
