...
function resolveBalanceForAccount(account, balanceByKey, balanceByNormalized, balanceByCode) {
  const id = account.id;
...
export async function fetchPaymentAccountChannels(tenantId, db) {
  ...
  const enriched = paymentAccounts.map((pa) => {
    ...
    return {
      ...
      coaCode,
      parentGlCode,
      balance: resolveBalanceForAccount(pa, balanceByKey, balanceByNormalized, balanceByCode),
    };
  });
