...
async function syncPaymentAccounts(tenantId, db) {
  ...
  const needsLeafMigration = withoutPosTillFloatPaymentAccounts(
    await findPaymentAccountsNeedingLeafCoaMigration(tenantId, db)
  );
  ...
  for (const p of toCoaFix.slice(0, 80)) {
    if (isPosTillFloatPaymentAccount(p)) continue;
    try {
...
export async function fetchPaymentAccountChannels(tenantId, db) {
  ...
  // Prefer posted 1110 once — do not sum multiple Cash payment rows that share the same GL.
  const cashGlBalance = balanceByCode.get(PAYMENT_CASH_GL_CODE);
  const cashUniqueCoaBalances = new Map();
  for (const a of cashAccounts) {
    if (a.coaAccountId && glByCoaId.has(a.coaAccountId)) {
      cashUniqueCoaBalances.set(a.coaAccountId, glByCoaId.get(a.coaAccountId));
    }
  }
  let cashTotal;
  if (cashUniqueCoaBalances.size) {
    cashTotal = [...cashUniqueCoaBalances.values()].reduce((s, n) => s + (Number(n) || 0), 0);
  } else if (cashGlBalance !== undefined && cashGlBalance !== null) {
    cashTotal = Number(cashGlBalance) || 0;
  } else {
    cashTotal = cashAccounts.reduce((s, a) => s + (Number(a.balance) || 0), 0);
  }
  ...
  const otherAccounts = enriched.filter(
    (a) =>
      a.accountType !== 'Cash' &&
      !isPosTillFloatPaymentAccount(a) &&
