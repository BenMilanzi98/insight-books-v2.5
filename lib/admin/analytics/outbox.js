...
export async function appendAnalyticsOutbox(db, input = {}) {
  ...
  try {
    ...
    // Table missing on partially-migrated DBs — do not throw into request path.
    if (e?.code === 'P2021' || /AnalyticsOutbox.*does not exist/i.test(e?.message || '')) {
      return { ok: false, skipped: true, reason: 'analytics_table_missing' };
    }
