...
export async function captureInvoiceAsPDF(elementRef, filename = 'invoice.pdf') {
  ...
  try {
    ...
    // Multi-page canvas stamps slice mid-element (totals). Prefer server jsPDF which keeps totals together.
    if (imgHeight > pageHeight + 1) {
      const idMatch = filename.match(/(?:invoice|quotation)-([^.]+)\.pdf/);
      if (idMatch) {
        const id = idMatch[1];
        const isQuotation = filename.includes('quotation-');
        const url = isQuotation
          ? `/api/quotations/${encodeURIComponent(id)}/download/pdf`
          : `/api/invoices/${encodeURIComponent(id)}/download/pdf`;
        try {
          ...
          if (res.ok) {
            ...
            console.log('✅ Multi-page PDF served from server (avoids mid-totals slice)');
            return true;
          }
        } catch (serverPdfErr) {
          console.warn('Server PDF fallback failed, using canvas pages:', serverPdfErr);
        }
      }
    }
