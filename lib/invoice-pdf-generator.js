...
export function generateInvoicePdf(invoice, template, branding) {
  ...
  // Footer: phone and bank (document override or default), then email footer
  if (showFooter) {
    ...
    let fy = pageHeight - 16;
    if (footerPhone.trim()) {
      doc.text(`Tel: ${footerPhone.trim()}`, pageWidth / 2, fy, { align: 'center' });
      fy -= 4;
    }
    if (footerBankDetails.trim()) {
      const bankLines = doc.splitTextToSize(footerBankDetails.trim(), pageWidth - margin * 2);
      bankLines.forEach((line) => {
        doc.text(line, pageWidth / 2, fy, { align: 'center' });
        fy -= 4;
      });
    }
    const footerText = branding?.emailFooter || `${branding?.companyName || 'Your Company'}`;
    doc.text(footerText, pageWidth / 2, pageHeight - 10, { align: 'center', maxWidth: pageWidth - margin * 2 });
  }
