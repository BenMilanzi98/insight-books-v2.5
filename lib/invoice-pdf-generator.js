...
import { tDocument } from './i18n/documentTranslate';
...
function hexToRgb(hex) {
  ...
  if (!hex || typeof hex !== 'string' || !hex.startsWith('#')) {
    return { r: 0, g: 117, b: 190 }; // Default color #0075be
  }
 ...
 */
export function generateInvoicePdf(invoice, template, branding, options = {}) {
  const docLocale = options.locale || branding?.locale || 'en';
  const totalLabel = `${tDocument('documents.total', { locale: docLocale })}:`;
  ...
  const { 
    style = 'standard',
    primaryColor = branding?.primaryColor || '#0075be',
    showLogo = true,
    showFooter = true
  } = content;
  ...
  const tableColumns = [
    ...
    { header: 'Selling Price (MWK)', dataKey: 'unitPrice' },
    ...(showLineTax ? [{ header: 'Tax Rate', dataKey: 'taxRate' }] : []),
  ...
  const tableRows = invoice.items.map(item => ({
    ...
    ...(showLineTax ? { taxRate: `${item.taxRate}%` } : {}),
  ...
  const totalsBoxHeight = showDocumentTax ? 25 : 18;
  ...
  let totalY = y + 12;
  if (showDocumentTax) {
    doc.text('Tax:', totalsX + 5, totalY);
    doc.text(formatCurrencyForExport(invoice.taxAmount), totalsX + totalsWidth - 5, totalY, { align: 'right' });
    totalY += 7;
  } else {
    totalY = y + 10;
  }
  ...
  doc.line(totalsX + 5, totalY + 3, totalsX + totalsWidth - 5, totalY + 3);
  
  // Total
  doc.setTextColor(colorRGB.r, colorRGB.g, colorRGB.b);
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.text(totalLabel, totalsX + 5, totalY + 10);
  doc.text(formatCurrencyForExport(invoice.total), totalsX + totalsWidth - 5, totalY + 10, { align: 'right' });
  ...
  if (invoice.notes) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
    const noteLines = doc.splitTextToSize(String(invoice.notes), contentWidth);
    let ny = y + 5;
