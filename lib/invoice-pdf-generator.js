...
export function generateInvoicePdf(invoice, template, branding) {
  ...
  const tableRows = invoice.items.map(item => ({
    ...
    unitPrice: formatCurrency(item.unitPrice, '', false),
    taxRate: `${item.taxRate}%`,
    amount: formatCurrency(item.quantity * item.unitPrice, '', false)
  }));
  ...
  doc.text(formatCurrency(invoice.subtotal), totalsX + totalsWidth - 5, y + 5, { align: 'right' });
  
  // Tax
  doc.text('Tax:', totalsX + 5, y + 12);
  doc.text(formatCurrency(invoice.taxAmount), totalsX + totalsWidth - 5, y + 12, { align: 'right' });
  ...
  doc.text(formatCurrency(invoice.total), totalsX + totalsWidth - 5, y + 22, { align: 'right' });
  ...
  if (invoice.paymentInfo && (invoice.paymentInfo.totalPaid > 0 || invoice.paymentInfo.outstandingAmount > 0)) {
    ...
    doc.text(formatCurrency(invoice.paymentInfo.totalPaid), totalsX + totalsWidth - 5, y + 5, { align: 'right' });
    
    // Outstanding Amount
    doc.setTextColor(239, 68, 68); // Red color
    doc.text('Outstanding:', totalsX + 5, y + 12);
    doc.text(formatCurrency(invoice.paymentInfo.outstandingAmount), totalsX + totalsWidth - 5, y + 12, { align: 'right' });
  ...
  doc.setFont('helvetica', 'normal');
  let notesText = invoice.notes || 'Thank you for your business!';
  
  // Add payment breakdown for partial invoices
  if (invoice.paymentInfo && invoice.paymentInfo.isPartiallyPaid && invoice.payments && invoice.payments.length > 0) {
    ...
    invoice.payments.forEach((payment, index) => {
      ...
      notesText += `\n${index + 1}. ${formatCurrency(payment.amount)} - ${payment.paymentMethod} (${paymentDate})`;
      if (payment.reference) {
        notesText += ` - Ref: ${payment.reference}`;
      }
    });
    notesText += `\n\nTotal Paid: ${formatCurrency(invoice.paymentInfo.totalPaid)}`;
    notesText += `\nOutstanding: ${formatCurrency(invoice.paymentInfo.outstandingAmount)}`;
  }
  
  doc.text(notesText, x, y + 6);
  
  // Footer
  if (showFooter) {
    doc.setTextColor(120, 120, 120);
    doc.setFontSize(8);
    const footerText = branding?.emailFooter || `${branding?.companyName || 'Your Company'} | ${branding?.email || ''} | ${branding?.phone || ''}`;
