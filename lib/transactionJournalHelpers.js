...
export async function getStandardAccounts(tenantId, tx = prisma) {
  ...
  const [
    ...
    findOrCreateAccount(tenantId, '5000', 'Cost of Goods Sold', 'Expense', 'Debit', tx).catch(async (err) => {
      console.error('❌ Error getting COGS account:', err);
      try {
        return await findOrCreateAccount(tenantId, '5000', 'Cost of Goods Sold', 'Expense', 'Debit', tx);
      } catch (retryErr) {
...
export async function createSaleJournalEntries({
  ...
  if (!normalizedItems.length || totalNet <= 0) {
    throw new Error('Sale items must include valid account allocations.');
  }

  normalizedItems.forEach((item) => {
    if (!item.accountId) {
      throw new Error('Each sale item must reference an income account.');
    }
    revenueBuckets.set(
      item.accountId,
