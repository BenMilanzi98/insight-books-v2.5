...
/**
 * Get the account where tax from outflows (expenses, supplier bills) accumulates.
 * Used so outflow tax can be offset against collected tax for reporting.
...
export async function getTaxOutflowAccount(tenantId, tx = prisma) {
  ...
  if (settings?.taxOutflowAccountId) {
    const account = await tx.account.findFirst({
      where: { id: settings.taxOutflowAccountId, tenantId, isActive: true }
    });
  ...
  if (taxType?.account) return taxType.account;
  return null;
}
...
export async function createExpenseJournalEntry({
  ...
  const netAmount = amount - (taxAmount || 0);
  const taxToPost = taxAmount && taxAmount > 0 ? taxAmount : 0;
  const taxAccount = taxToPost > 0 ? await getTaxOutflowAccount(tenantId, tx) : null;

  const linesToCreate = [];
  if (taxToPost > 0 && taxAccount) {
    // Record tax portion separately and accumulate in tax management account (for offset vs collected tax)
    linesToCreate.push(
      {
        lineNumber: 1,
        ...
        creditAmount: 0,
        description: `Expense (net): ${category}`,
      },
      {
        lineNumber: 2,
