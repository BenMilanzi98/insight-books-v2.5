...
export async function getStandardAccounts(tenantId, tx = prisma) {
  ...
  const [
    ...
    (async () => {
      ...
      return findOrCreateAccount(tenantId, '5100', 'Cost of Goods', 'Expense', 'Debit', tx).catch(async (err) => {
        console.error('❌ Error getting COGS account:', err);
        try {
          return await findOrCreateAccount(tenantId, '5100', 'Cost of Goods', 'Expense', 'Debit', tx);
        } catch (retryErr) {
...
function purposeForPaymentMethod(paymentMethod, paymentAccountType = null) {
  ...
  if (key.includes('petty')) return 'CASH_ON_HAND';
  return 'CASH_ON_HAND';
}
