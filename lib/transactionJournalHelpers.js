...
export async function createInvoiceJournalEntry({
  ...
  const normalizedItems = (items || []).map((item) => {
    ...
    return {
      ...
      netAmount,
      lineTax: Math.max(0, Number(item.taxAmount || 0)),
    };
  });

  const totalNet = normalizedItems.reduce((sum, item) => sum + item.netAmount, 0);
  const totalLineTax = normalizedItems.reduce((sum, item) => sum + item.lineTax, 0);
  const arDebit =
    totalLineTax > 0 && Math.abs(totalAmount - totalNet - totalLineTax) < 0.02
      ? roundMoney(totalNet + totalLineTax)
  ...
  // Prepare revenue transaction lines (AR = net + tax; revenue credits = net only when tax posts separately)
  const revenueLines = [
    {
      lineNumber: 1,
  ...
  const creditTarget = totalLineTax > 0 ? totalNet : arDebit;
