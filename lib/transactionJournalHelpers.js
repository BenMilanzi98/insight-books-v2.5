...
export async function getStandardAccounts(tenantId, tx = prisma) {
  ...
  const resolveBankLikeAsset = async () => {
    const group1120 = await tx.account.findFirst({
      where: { tenantId, accountCode: '1120', isActive: true, accountType: 'Asset' },
      select: { id: true },
    });
    if (group1120?.id) {
      const child = await tx.account.findFirst({
        where: { tenantId, parentAccountId: group1120.id, isActive: true, accountType: 'Asset' },
    ...
    return findOrCreateAccount(tenantId, '1130-04', 'National Bank', 'Asset', 'Debit', tx).catch(() => null);
  };
