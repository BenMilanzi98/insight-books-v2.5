...
export async function createSaleJournalEntries({
  ...
  let revenueEntry;
  try {
    revenueEntry = await postGlEntry({
      tenantId,
      userId,
      entryDate,
  ...
  if (cogsAmount > 0 && accounts.cogs && accounts.inventory) {
    // Re-resolve inside the same DB client as journal + balance updates (pre-fetch may use
    // prisma outside an interactive transaction, or 1300 may only have non-posting children).
    const inventoryForCogs = await resolveOrEnsureInventoryGlAccount(tenantId, tx);
    ...
    const cogsEntry = await postGlEntry({
      tenantId,
      userId,
      entryDate,
    ...
    console.log('✅ COGS entry created successfully:', {
      ...
      cogsAmount,
      cogsAccountId: accounts.cogs.id,
      inventoryAccountId: inventoryForCogs.id,
    });
  } else {
    if (cogsAmount > 0) {
...
export async function createInvoiceJournalEntry({
  ...
  if (!accounts.accountsReceivable) {
    console.warn('⚠️ Accounts Receivable account not found, attempting to resolve...');
    try {
      accounts.accountsReceivable = await findAccountsReceivableGlAccount(tenantId);
      if (accounts.accountsReceivable) {
        console.log('✅ Accounts Receivable resolved:', accounts.accountsReceivable.id);
      }
    } catch (resolveErr) {
      console.error('❌ Failed to resolve Accounts Receivable account:', resolveErr);
    }
  }
  ...
  if (!accounts.accountsReceivable) {
    try {
      ...
      throw new Error('Accounts Receivable account not found and could not be created. Please set up account 1200 in Chart of Accounts.');
    }
  }
  ...
  const normalizedItems = (items || []).map((item) => {
    ...
    if (item.netAmount != null && item.netAmount !== '') {
      netAmount = Math.max(0, Number(item.netAmount) || 0);
    } else {
      const baseAmount =
        typeof item.amount === 'number'
          ? item.amount
          : qty * Number(item.unitPrice || 0);
      const perUnitDiscount = Number(item.discountAmount || 0);
      netAmount = Math.max(0, baseAmount - perUnitDiscount * qty);
    }
    return {
  ...
  // Revenue JE is net-only (Dr AR net / Cr Revenue net). Line tax on items must NOT inflate AR here:
  // VAT is posted separately via autoPostTaxEntry (balanced against AR). Including tax in AR
  // while crediting only net fails validateTransactionBalance and rolls back the invoice.
  // Caller-supplied totalAmount may be gross; this JE always posts net.
  const arDebit = roundMoney(totalNet);
  const creditTarget = roundMoney(totalNet);
  if (totalAmount != null && Number(totalAmount) > 0 && Math.abs(Number(totalAmount) - totalNet) > 0.02) {
    console.log(
      `[Invoice JE] Using net AR ${arDebit} (invoice totalAmount ${totalAmount} includes tax posted separately)`
    );
  }
  ...
  bucketEntries.forEach(([accountId, amount], index) => {
    const ratio = totalNet > 0 ? amount / totalNet : 0;
    const credit =
      index === bucketEntries.length - 1
        ? Number((creditTarget - allocatedTotal).toFixed(2))
        : Number((creditTarget * ratio).toFixed(2));
  ...
  const revenueEntry = await postGlEntry({
    tenantId,
    userId,
    entryDate,
  ...
  if (taxAmount > 0 && taxTypeId) {
    const taxEntry = await autoPostTaxEntry({
      ...
      balancingAccountId: accounts.accountsReceivable.id,
      tx
    });
    if (taxEntry) {
      entries.push(taxEntry);
...
export async function createInvoicePaymentJournalEntry({
  ...
  const entry = await postGlEntry({
    ...
    sourceId: `${invoiceId}-payment-${referenceNumber}`,
    lines: [
      {
        lineNumber: 1,
        accountId: paymentAccount.id,
...
export async function createExpenseJournalEntry({
  ...
  const postingCheckIds = [expenseAccount.id, creditAccount.id];
  if (taxToPost > 0 && taxAccount?.id) postingCheckIds.push(taxAccount.id);
  await assertAccountsAllowDirectPosting(postingCheckIds, tx);
