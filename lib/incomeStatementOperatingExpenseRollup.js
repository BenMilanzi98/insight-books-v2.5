...
const SYSTEM_OPERATING_CODES_ORDER = ['5301', '5200', '5300', '5310', '5320', '5330', '5340', '5400', '5500', '5900'];
...
function rollByName(text) {
  ...
  if (/software|subscription|hosting|saas|cloud|cursor|inmotion|it\s*(?:&|and)\s*hosting|development|developer|insightbooks|programming/.test(n)) return '5702';
  ...
  if (/salari|wage|payroll|paye|staff|labour|labor|employee\s*benefit/.test(n)) return '5301';
...
export function resolveOperatingExpenseRollup(input) {
  ...
  if (rawCode && SALARY_CHILD_CODES.has(rawCode)) {
    return { rollupCode: '5301', exclude: false };
  }
