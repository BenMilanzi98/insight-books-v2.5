/**
 * Roll operating expense amounts into PHINDU Chart of Accounts main lines (structure file:
...
/**
 * Template (ensureExpenseAccountsForTenant) code → PHINDU display code.
 * `null` = omit from operating expenses (parent / COGS template lines).
 */
const TEMPLATE_CODE_TO_PHINDU = {
  5000: null,
  5001: null,
  5002: null,
  5003: null,
...
const PHINDU_OPERATING_CODES_ORDER = ['5200', '5300', '5310', '5320', '5330', '5340', '5400', '5500', '5900'];

const SALARY_CHILD_CODES = new Set(['5201', '5202', '5203', '5210']);

/**
 * @returns {{ code: string, name: string }[]}
 */
export function getPhinduOperatingExpenseDisplayLines() {
  const nameBy = new Map(CHART_OF_ACCOUNTS_BLUEPRINT.map((r) => [r.code, r.name]));
  return PHINDU_OPERATING_CODES_ORDER.map((code) => ({
    code,
...
export function resolveOperatingExpenseRollup(input) {
  ...
  if (typeof key === 'string' && key.startsWith('cat:')) {
    ...
    if (sync && SYNC_STANDARD_TO_PHINDU[sync]) {
      return { rollupCode: SYNC_STANDARD_TO_PHINDU[sync], exclude: false };
    }
  ...
  if (TEMPLATE_CODES.has(rawCode)) {
    const mapped = TEMPLATE_CODE_TO_PHINDU[rawCode];
  ...
  if (bpRow && bpRow.code === '5200' && bpRow.subtype === 'Group') {
    return { rollupCode: '5200', exclude: false };
  }
