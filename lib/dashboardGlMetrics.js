...
/**
 * Dashboard / sales-card revenue: completed invoices + completed POS sales
 * for the local window. Excludes voids/refunds/drafts. Does not add payments
 * on top of invoice totals or other income.
 */
export async function fetchOperationalRevenue({
  prisma,
  tw,
  userQ,
  startDate,
  ...
  const [invoices, sales] = await Promise.all([
    prisma.invoice.aggregate({
      where: {
        ...
        ...(branchIdForPayments ? { branchId: branchIdForPayments } : {}),
      },
      _sum: { total: true },
    }),
  ...
  return addMoney(invoices._sum.total, sales._sum.total);
}
...
async function fetchGlProfitAndLossAggregate({
  ...
  for (const tenantId of tenantIds) {
    try {
      ...
      revenue = addMoney(revenue, pl.totalRevenue);
...
export async function fetchDashboardPeriodMetrics({
  ...
  const [operationalRevenue, operatingExpenses] = await Promise.all([
  ...
  return {
    revenue: roundMoney(operationalRevenue),
    operatingExpenses,
    cogs,
    source: useGlOperatingExpenses ? 'mixed' : 'operational',
  };
}
