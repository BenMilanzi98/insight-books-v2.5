...
export async function hasPremiumAccess(tenantId) {
  ...
  // Defensive checks are important because some legacy trial rows may have isTrial=false.
  const now = new Date();
  const subscription = await prisma.accountSubscription.findFirst({
    where: {
      tenantId,
      ...
      isTrial: false,
      expiresAt: { gt: now },
      // Exclude "trial-like" rows even if isTrial was incorrectly stored as false.
      NOT: [
        { plan: 'trial' },
        { paymentMethod: 'trial' },
        { txRef: { startsWith: 'TRIAL_' } }
      ],
      // Prevent zero-amount / pending rows from unlocking premium access.
      amount: { gt: 0 },
...
export async function getTenantWithSubscription(tenantId) {
  ...
  const isPaidActive = (sub) =>
    ...
    !(typeof sub.txRef === 'string' && sub.txRef.startsWith('TRIAL_'));
