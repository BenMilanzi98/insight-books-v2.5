 ...
 * EIS subscriptions are separate from regular premium - they have their own pricing and features
 */
export async function hasEISAccess(tenantId) {
  ...
  // Check if the subscription plan is an EIS plan
  if (subscription && isEISPlan(subscription.plan)) {
    return true;
  }
...
/**
 * Check if tenant can submit EIS invoices (subscription + quota check)
 */
