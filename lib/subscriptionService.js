...
import { TRIAL_DURATION_DAYS, calculateSubscriptionExpiry, isEISPlan, getEISQuota } from './subscriptionConfig';
...
async function hasStandardAccessImpl(tenantId) {
  ...
  // Active trial: align with isTenantTrialActive / getTenantSubscription — trialEndDate is authoritative
  // (avoids denying access when isTrial row exists but isActive was cleared inconsistently).
...
/**
 * Check if tenant has an active EIS subscription plan (billing layer).
 * Fixed G2-004: query EIS plan IDs explicitly instead of taking the latest
 * non-EIS paid subscription and then testing isEISPlan.
 *
 * Phase 4: operational fiscalization also requires MraEis entitlement +
 * effective capability (see canPerformEisOperation / canSubmitEISInvoice).
 */
export async function hasEISAccess(tenantId) {
  if (!tenantId) return false;
  const now = new Date();
  const { EIS_PLAN_IDS } = await import('@/lib/subscriptionConfig');
  const planIds = Array.isArray(EIS_PLAN_IDS) ? EIS_PLAN_IDS : [];
  ...
  const subscription = await prisma.accountSubscription.findFirst({
    where: {
      ...
      isActive: true,
      isTrial: false,
      plan: { in: planIds.length ? planIds : ['eis-monthly', 'eis-yearly'] },
      // Lifetime / open-ended EIS plans may have null expiresAt
      OR: [{ expiresAt: null }, { expiresAt: { gt: now } }],
    },
    orderBy: { createdAt: 'desc' },
  });

  return Boolean(subscription && isEISPlan(subscription.plan));
}
...
export async function getEISConfiguration(tenantId) {
  try {
    return await prisma.eISConfiguration.findFirst({
      where: { tenantId, isActive: true }
    });
  } catch (err) {
    console.warn('getEISConfiguration error (table may not exist yet):', err.message);
    return null;
  }
}

/**
 * Get EIS monthly usage for the current month
 */
export async function getEISMonthlyUsage(tenantId) {
  const now = new Date();
  ...
  try {
    ...
    console.warn('getEISMonthlyUsage error:', err.message);
    return { invoiceCount: 0, submissionCount: 0, approvedCount: 0, rejectedCount: 0, totalAmount: 0 };
  }
}

/**
 * Check if tenant can submit EIS invoices (subscription + quota + Phase 4 capability).
 * Does not call MRA. Phase 4 effectiveOperational is false until later runtime deps exist.
 */
export async function canSubmitEISInvoice(tenantId) {
  ...
  try {
    const { canPerformEisOperation, EIS_OPERATION } = await import('@/lib/mraEis/index.js');
    const gate = await canPerformEisOperation(tenantId, EIS_OPERATION.TRANSMIT_SALE);
    if (!gate.allowed) {
      const first = gate.result?.blockers?.[0];
      return {
        canSubmit: false,
        reason: first?.message || 'EIS effective capability does not allow transmission',
        blockers: gate.result?.blockers || [],
      };
    }
  } catch (err) {
    // Tables missing pre-migration: keep legacy subscription/quota check only.
    const missing =
      err?.code === 'P2021' ||
      /does not exist|MraEis/i.test(String(err?.message || ''));
    if (!missing) {
      console.warn('canSubmitEISInvoice capability check:', err.message);
      return { canSubmit: false, reason: 'EIS control plane unavailable' };
    }
  }

  const subscription = await getTenantSubscription(tenantId);
  if (!subscription) {
    return { canSubmit: false, reason: 'No active subscription' };
  }

  const quota = getEISQuota(subscription.plan);
  if (quota && quota.monthlyInvoices !== Infinity) {
    const usage = await getEISMonthlyUsage(tenantId);
    if (usage.invoiceCount >= quota.monthlyInvoices) {
      return {
        canSubmit: false,
