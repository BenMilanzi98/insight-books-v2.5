 ...
 * balanceDue = total - totalPaid so the result is accurate regardless of status string
 * (e.g. "Partial" vs "Partially Paid" as used by the payments route).
 */
export async function getClientOutstandingBalance(clientId, tenantId) {
  ...
  const totalBalance = invoicesWithBalance.reduce((sum, inv) => sum + inv.balanceDue, 0);
