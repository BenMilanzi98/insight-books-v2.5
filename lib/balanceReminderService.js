...
export async function getClientOutstandingBalance(clientId, tenantId) {
  ...
  const invoicesWithBalance = invoices.map(invoice => {
    const actualTotalPaid = parseMoney(invoice.totalPaid);
  ...
  const invoiceTotal = invoicesWithBalance.reduce((sum, inv) => addMoney(sum, inv.balanceDue), 0);
  const creditTotal = creditNotes.reduce((sum, n) => addMoney(sum, n.amount), 0);
  const debitTotal = debitNotes.reduce((sum, n) => addMoney(sum, n.amount), 0);
  const totalBalance = addMoney(subtractMoney(invoiceTotal, creditTotal), debitTotal);
