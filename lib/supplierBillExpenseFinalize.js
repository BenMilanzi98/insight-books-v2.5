...
import { assertAccountsAllowDirectPosting } from '@/lib/coaDirectPostingEligibility.js';
...
export async function finalizeExpenseBill(tx, bill, tenantId, userId) {
  ...
  if (lines.length === 0) {
    throw new Error('Cannot post expense bill: no GL lines (missing expense accounts on line items).');
  }
