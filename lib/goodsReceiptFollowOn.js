...
import { isPurchasesGrniEnabled } from '@/lib/purchases/grniPolicy.js';
...
async function allocateGoodsReceiptBillNumber(tx, goodsReceipt) {
  const base = `GRB-${(goodsReceipt.receiptNumber || goodsReceipt.id || 'RECEIPT').trim()}`;
  let candidate = base;
  let suffix = 1;

  while (await tx.supplierBill.findFirst({ where: { billNumber: candidate } })) {
    suffix += 1;
...
export async function autoCreateBillFromReceipt({
  ...
  // When GRNI is on: receipt journal is Dr Inventory / Cr GRNI (2115).
  // Bill must post its own Dr GRNI / Cr AP (2110) journal — never reuse the receipt JE.
  const bill = await tx.supplierBill.create({
    data: {
      ...
      journalEntryId: grniEnabled ? null : journalEntryId || null,
      items: {
  ...
  if (grniEnabled) {
    const { finalizeInventoryBill } = await import('@/lib/purchases/finalizeInventoryBill.js');
    await finalizeInventoryBill(tx, bill, tenantId, userId);
  } else {
