...
export async function autoCreateBillFromReceipt({
  ...
  // billNumber is globally @unique; receipt numbers are only unique per tenant, so
  // GRB-${receiptNumber} collides across tenants. Tie the bill number to the receipt id.
  const billNumber = `GRB-${goodsReceipt.id}`;
