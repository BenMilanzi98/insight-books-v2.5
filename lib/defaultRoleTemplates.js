...
export function getDefaultRoleTemplates() {
  ...
  const admin = {
    name: 'Admin',
    description: 'Full access to all tenant features (same permission map as Owner; business owner flag remains separate).',
    permissions: generateFullPermissions(),
  };
  ...
  const manager = {
    ...
    permissions: pick(null, [
      ...
      'purchases.create', 'purchases.view', 'purchases.update', 'purchases.approve', 'purchases.export',
      'suppliers.create', 'suppliers.view', 'suppliers.update', 'suppliers.export',
      ...
      'openingBalances.view', 'openingBalances.export',
  ...
  const sales = {
    name: 'Sales',
    description: 'Point of sale only (nav and routes restricted to /pos; sales.* grants supporting API access implicitly).',
    permissions: pick(null, [
      'sales.create', 'sales.view', 'sales.update', 'sales.void', 'sales.refund', 'sales.export',
    ]),
  };
  ...
  const accountant = {
    ...
    permissions: pick(null, [
      ...
      'openingBalances.view', 'openingBalances.manage', 'openingBalances.post', 'openingBalances.export',
  ...
  return [owner, admin, manager, sales, inventory, accountant, reportsOnly];
}
