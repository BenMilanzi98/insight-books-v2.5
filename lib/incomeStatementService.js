...
export async function getSalesRevenueForPeriod(tenantId, startDate, endDate, branchId = null) {
  ...
  const [recognizedRevenue, paymentAgg, salesAgg] = await Promise.all([
    sumInvoiceRecognizedRevenueForPeriod(tenantId, start, end, branchFilterId),
  ...
  const invoicePayments =
    recognizedRevenue > 0
      ? recognizedRevenue
      : parseMoney(paymentAgg._sum.amount);
  const posTotals = parseMoney(salesAgg._sum.total);
  return roundReportAmount(addMoney(invoicePayments, posTotals));
}

/**
 * Sum net revenue recognized at payment time (Dr Deferred / Cr Sales) for deferred-revenue invoices.
 * Aligns P&L with GL when VAT-inclusive payments are received on credit invoices.
 */
async function sumInvoiceRecognizedRevenueForPeriod(tenantId, start, end, branchFilterId) {
  const journals = await prisma.journalEntry.findMany({
    ...
    select: { metadata: true, totalCredit: true },
  });
  ...
  for (const journal of journals) {
    ...
    const recognized =
      meta && typeof meta === 'object' && meta.recognizedNet != null
        ? meta.recognizedNet
        : journal.totalCredit;
    total = addMoney(total, parseMoney(recognized));
  }
  return roundReportAmount(total);
}
