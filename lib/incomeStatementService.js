...
export async function generateIncomeStatementFromAccounts(
  ...
  for (const account of revenueAccounts) {
    ...
    const startBalance = await getAccountBalanceDetails(account.id, tenantId, startDateMinusOne, prisma);
  ...
  if (cogsAccount) {
    const endBalance = await getAccountBalanceDetails(cogsAccount.id, tenantId, end, prisma);
    const startDateMinusOne = new Date(start);
    startDateMinusOne.setDate(startDateMinusOne.getDate() - 1);
    const startBalance = await getAccountBalanceDetails(cogsAccount.id, tenantId, startDateMinusOne, prisma);
  ...
  if (taxAccount) {
    const endBalance = await getAccountBalanceDetails(taxAccount.id, tenantId, end, prisma);
    const startDateMinusOne = new Date(start);
    startDateMinusOne.setDate(startDateMinusOne.getDate() - 1);
    const startBalance = await getAccountBalanceDetails(taxAccount.id, tenantId, startDateMinusOne, prisma);
