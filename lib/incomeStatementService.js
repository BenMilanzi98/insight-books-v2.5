 ...
 * 4. Operating Expenses drive the structure — list from user-created expense categories; only categories with transactions.
...
export async function generateIncomeStatementFromAccounts(
  ...
  // Import normalization service for account code grouping
  const { getAccountCodeForCategory } = await import('./expenseCategoryNormalization');
  ...
  // List is built from user-created expense categories only (see below).
  ...
  // User-created expense categories (ExpenseCategory + 5001-5999 under 5000 - Expense, or 6xxx operating)
  const expenseCategoriesList = await getExpenseCategoriesForTenant(tenantId);

  // Merge depreciation (synthetic DEP) into a "Depreciation" category if one exists in the list
  const depBucket = amountsByAccountId['DEP'];
  if (depBucket && (depBucket.amount || 0) > 0) {
    const depCategory = expenseCategoriesList.find(
      (c) => (c.name || '').toLowerCase().includes('depreciation') || (c.account?.accountName || '').toLowerCase().includes('depreciation')
    );
    if (depCategory) {
      if (!amountsByAccountId[depCategory.accountId]) {
        amountsByAccountId[depCategory.accountId] = { amount: 0, details: [], accountCode: depCategory.accountCode, accountName: depCategory.name };
      }
      amountsByAccountId[depCategory.accountId].amount += depBucket.amount;
      amountsByAccountId[depCategory.accountId].details.push(...(depBucket.details || []));
    }
  }

  // OPERATING EXPENSES: same categories as dropdown (expense category selectors). Only include those that have values in the period.
  const operatingExpensesCategories = [];
  const addedAccountIds = new Set();
  for (const cat of expenseCategoriesList) {
    const bucket = amountsByAccountId[cat.accountId];
    const amount = bucket ? Number(bucket.amount) || 0 : 0;
    if (amount <= 0) continue;
    addedAccountIds.add(cat.accountId);
    const displayName = cat.name || cat.account?.accountName || cat.account?.name || cat.accountCode || 'Unnamed';
  ...
  // Include unmatched buckets only if they correspond to a known user-category name.
  // Buckets keyed as "cat:<name>" are expenses without a linked account; try to merge
  // them into an existing category row or create one only when the name matches a
  // user-created ExpenseCategory.
  const knownCategoryNames = new Set(
    expenseCategoriesList.map(c => (c.name || '').toLowerCase())
  );
  Object.entries(amountsByAccountId).forEach(([key, bucket]) => {
    const amount = Number(bucket?.amount) || 0;
    if (amount <= 0) return;
    if (key === 'DEP') return;
    const alreadyAdded = key && addedAccountIds.has(key);
    if (alreadyAdded) return;

    const isCatBucket = typeof key === 'string' && key.startsWith('cat:');
    const rawName = isCatBucket ? key.slice(4) : (bucket.accountName || bucket.accountCode || key);
    const displayName = rawName.replace(/^cat:/, '');

    if (isCatBucket) {
      // Try to merge into an existing category row by name match
      const existing = operatingExpensesCategories.find(
        c => (c.category || '').toLowerCase() === displayName.toLowerCase()
      );
      if (existing) {
        existing.amount += amount;
        existing.details.push(...(bucket.details || []));
        return;
      }
      // Only create a new row if the category name matches a user-created ExpenseCategory
      if (!knownCategoryNames.has(displayName.toLowerCase())) return;
    }

    const matchedCat = expenseCategoriesList.find(
      c => (c.name || '').toLowerCase() === displayName.toLowerCase()
    );
    operatingExpensesCategories.push({
      accountCode: matchedCat?.accountCode || bucket.accountCode || '',
      accountName: displayName,
      accountId: matchedCat?.accountId || (isCatBucket ? null : key),
      category: displayName,
      categoryNames: [displayName],
      amount,
      details: bucket?.details || []
  ...
  // If we have depreciation but no "Depreciation" category in the list, add one line so the total is correct
  if (depBucket && (depBucket.amount || 0) > 0) {
    const hasDepreciation = operatingExpensesCategories.some(
      (c) => (c.category || '').toLowerCase().includes('depreciation')
    );
    if (!hasDepreciation) {
      operatingExpensesCategories.push({
        accountCode: 'DEP',
        ...
        accountId: null,
        category: 'Depreciation',
        categoryNames: ['Depreciation'],
        amount: depBucket.amount,
        details: depBucket.details || []
      });
    }
  }
  operatingExpensesCategories.sort((a, b) => (a.category || '').localeCompare(b.category || ''));
  ...
  const result = {
    ...
    metadata: {
      ...
      dataSource: 'Revenue and COGS system-generated. COGS excludes shipping. Operating expenses from expense tracking + payroll. Same logic for dashboard, Excel, PDF.',
