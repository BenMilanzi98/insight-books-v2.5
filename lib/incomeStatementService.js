...
=======
  await ensureDuplicate5301MergedInto5200(prisma, tenantId);
  await ensureLegacyExpenseAccountMerges(prisma, tenantId);

  console.log('📊 Generating Income Statement (GL-backed revenue when posted):', {
>>>>>>> Stashed changes
    tenantId,
...
=======
  let glProfitAndLoss = null;
  try {
    glProfitAndLoss = await buildProfitAndLossFromGl({
      tenantId,
      startDate: periodStartYmd ?? formatYmdInTimeZone(start, DEFAULT_REPORT_TIMEZONE),
      endDate: periodEndYmd ?? formatYmdInTimeZone(end, DEFAULT_REPORT_TIMEZONE),
      branchId: branchFilterId,
      prisma,
    });
  } catch (glPlErr) {
    console.warn('Income statement: GL P&L fetch failed', glPlErr?.message || glPlErr);
  }

  // Operational revenue (sub-ledger) — used for drill-down and reconciliation
  const salesRevenueOperational = await getSalesRevenueForPeriod(tenantId, start, end, branchFilterId);
  const useGlRevenue =
    glProfitAndLoss?.hasGlActivity &&
    (glProfitAndLoss.totalRevenue > 0 || glProfitAndLoss.revenue.lineItems.length > 0);
  const salesRevenueTotal = useGlRevenue
    ? glProfitAndLoss.totalRevenue
    : salesRevenueOperational;
  ...
  const revenueLineItems = useGlRevenue && glProfitAndLoss.revenue.lineItems.length > 0
    ? glProfitAndLoss.revenue.lineItems.map((li) => ({
        key: li.key,
        label: li.label,
        amount: li.amount,
        accountId: li.accountId,
        accountCode: li.accountCode,
        sourceHref: li.sourceHref,
  ...
  const otherIncomeFromGl = useGlRevenue
    ? (glProfitAndLoss.revenue.otherIncome ?? 0)
    : 0;

>>>>>>> Stashed changes
  const revenue = {
    salesRevenue: useGlRevenue ? glProfitAndLoss.totalRevenue : salesRevenueTotal,
    serviceRevenue: 0,
    otherIncome: otherIncomeFromGl,
    details: [],
<<<<<<< Updated upstream
    lineItems: [{
      key: 'sales-revenue',
...
=======
    ...
    fromGeneralLedger: useGlRevenue,
    operationalTotal: salesRevenueOperational,
>>>>>>> Stashed changes
  };
  const totalRevenue = addMoney(salesRevenueTotal, otherIncomeFromGl);
  ...
  const glEngineCogsTotal = glProfitAndLoss?.cogs?.fromGeneralLedger
    ? roundMoney(glProfitAndLoss.cogs.total)
    : 0;
  const useGlCogs =
    (glProfitAndLoss?.cogs?.fromGeneralLedger && Math.abs(glEngineCogsTotal) > 1e-6) ||
    (cogsAccountIds.length > 0 && Math.abs(glRounded) > 1e-6);
  const costOfGoodsSoldRaw = useGlCogs
    ? (Math.abs(glEngineCogsTotal) > 1e-6 ? glEngineCogsTotal : glRounded)
    : costOfGoodsSoldFromActivity;
...
=======
  ...
  const cogsLineItems =
    useGlCogs && glProfitAndLoss?.cogs?.lineItems?.length > 0
      ? glProfitAndLoss.cogs.lineItems.map((li) => ({
          key: li.key,
          label: li.label,
          amount: li.amount,
          accountId: li.accountId,
...
>>>>>>> Stashed changes
    ...
    operationalTotal: costOfGoodsSoldFromActivity,
  };
  ...
  /** Non-zero expense buckets that count toward operating expenses (same exclude rules as rollup). */
  const operatingExpenseAccountLines = [];
  for (const [key, bucket] of Object.entries(amountsByAccountId)) {
    ...
    const { rollupCode, exclude } = resolveOperatingExpenseRollup({
      key: String(key),
      accountCode: accountCodeRaw || null,
      accountName: bucket.accountName
    });
    if (exclude || !rollupCode) continue;
    operatingExpenseAccountLines.push({
      accountId: bucket.accountId ?? null,
      accountCode: accountCodeRaw || String(bucket.accountCode || key),
      accountName: bucket.accountName || '',
      amount: roundMoney(amount),
      details: bucket.details || []
    });
  }
  operatingExpenseAccountLines.sort((a, b) =>
    String(a.accountCode).localeCompare(String(b.accountCode), undefined, { numeric: true })
  );

  // Operating expense categories: one row per **actual GL** account (matches category-linked postings).
  ...
  // Calculate total operating expenses (operational sub-ledger)
  let totalOperatingExpensesOperational = operatingExpensesCategories.reduce(
    (sum, cat) => addMoney(sum, cat.amount),
    0
  );

  const useGlOperatingExpenses =
    glProfitAndLoss?.operatingExpenses?.lineItems?.length > 0 &&
    glProfitAndLoss.operatingExpenses.total > 0;

  let finalOperatingExpenseAccountLines = operatingExpenseAccountLines;
  let finalOperatingExpensesCategories = operatingExpensesCategories;
  let totalOperatingExpenses = totalOperatingExpensesOperational;

  if (useGlOperatingExpenses) {
    finalOperatingExpenseAccountLines = glProfitAndLoss.operatingExpenses.lineItems.map((li) => {
      const opMatch = operatingExpenseAccountLines.find(
        (l) =>
          (li.accountCode && l.accountCode === li.accountCode) ||
          (li.accountId && l.accountId === li.accountId)
      );
      return {
        accountCode: li.accountCode,
        accountName: li.label || li.accountName,
        accountId: li.accountId,
        amount: li.amount,
        details: opMatch?.details ?? [],
        sourceHref: li.sourceHref,
        fromGeneralLedger: true,
      };
    });
    finalOperatingExpensesCategories = finalOperatingExpenseAccountLines.map((line) => ({
      accountCode: line.accountCode,
      accountName: line.accountName,
      accountId: line.accountId,
      category:
    ...
    totalOperatingExpenses = glProfitAndLoss.operatingExpenses.total;
  }

  // Create operatingExpenses object with dynamic categories
  const operatingExpenses = {
    categories: finalOperatingExpensesCategories,
    accountLines: finalOperatingExpenseAccountLines,
    total: totalOperatingExpenses,
    details: expenseDetails,
    fromGeneralLedger: useGlOperatingExpenses,
    operationalTotal: totalOperatingExpensesOperational,
  };
  ...
  const result = {
    ...
    budget: budgetData,
    metadata: {
      revenueAccounts: useGlRevenue ? glProfitAndLoss.revenue.lineItems.length : 0,
      expenseAccounts: expenseAccounts.length,
      expenseCategories: finalOperatingExpensesCategories.length,
      generatedAt: new Date().toISOString(),
      dataSource:
        'Primary source: posted General Ledger survivor totals classified via Chart of Accounts. Revenue, COGS, and operating expenses prefer GL when posted activity exists; operational sub-ledgers retained for drill-down and reconciliation. Same logic for dashboard, Excel, PDF.',
      ledgerSource: glProfitAndLoss?.source ?? 'general_ledger',
      sourcePolicy: glProfitAndLoss?.sourcePolicy ?? null,
      fromGeneralLedger: {
        revenue: useGlRevenue,
        cogs: useGlCogs,
        operatingExpenses: useGlOperatingExpenses,
      },
      reconciliation: buildReconciliationSummary([
        buildReconciliationItem({
          label: 'Revenue',
          glAmount: glProfitAndLoss?.totalRevenue ?? 0,
          operationalAmount: salesRevenueOperational,
        }),
        buildReconciliationItem({
          label: 'Cost of Goods Sold',
          glAmount: glProfitAndLoss?.cogs?.total ?? glRounded,
          operationalAmount: costOfGoodsSoldFromActivity,
        }),
        buildReconciliationItem({
          label: 'Operating Expenses',
          glAmount: glProfitAndLoss?.operatingExpenses?.total ?? 0,
          operationalAmount: totalOperatingExpensesOperational,
        }),
      ]),
