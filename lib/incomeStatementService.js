 ...
 * 5. Operating Expenses roll up to SYSTEM Chart of Accounts main lines (5200–5900); leaf/custom category accounts merge into those lines.
...
import {
  getSystemOperatingExpenseDisplayLines,
...
/**
 * Sales Revenue — aligned with `/api/dashboard/metrics` (cash received on invoices + POS totals).
 * - Invoices: sum of **completed** `Payment.amount` with `paymentDate` in range (same as dashboard).
 * - POS: sum of `Sale.total` for `status: 'completed'` with `saleDate` in range (same as dashboard).
...
export async function getSalesRevenueForPeriod(tenantId, startDate, endDate, branchId = null) {
  ...
  const invoiceRevenue = invoices.reduce((sum, invoice) => sum + invoiceNetRevenueTotalExTax(invoice), 0);
  const saleRevenue = sales.reduce((sum, sale) => sum + saleNetRevenueTotalExTax(sale), 0);

  return roundReportAmount(invoiceRevenue + saleRevenue);
}
...
export async function generateIncomeStatementFromAccounts(
  ...
  const costOfGoodsSoldFromActivity =
    Math.round(Number(cogsStats?.totalAmount ?? 0) * 100) / 100;
  const glRounded = Math.round(Number(costOfGoodsSoldGl) * 100) / 100;
  const useGlCogs =
    cogsAccountIds.length > 0 && Math.abs(glRounded) > 1e-6;
  const costOfGoodsSoldRaw = useGlCogs ? glRounded : costOfGoodsSoldFromActivity;
  const costOfGoodsSold = Math.round(costOfGoodsSoldRaw * 100) / 100;
  ...
  // OPERATING EXPENSES: roll every bucket into SYSTEM main CoA lines (no one-line-per-custom-category).
  ...
  const systemLines = getSystemOperatingExpenseDisplayLines();
  ...
  for (const [code, data] of rolledOperating.entries()) {
    if (systemLines.some((l) => l.code === code)) continue;
    const rawAmt = Number(data.amount) || 0;
    if (Math.abs(rawAmt) < 1e-6) continue;
    const other = systemLines.find((l) => l.code === '5900');
