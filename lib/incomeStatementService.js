...
export async function getSalesRevenueForPeriod(tenantId, startDate, endDate, branchId = null) {
  ...
  const paymentWhere = {
    ...
    ...(branchId
      ? {
          OR: [{ branchId }, { invoice: { branchId } }]
        }
      : {})
  };
  const invoicePaymentsAgg = await prisma.payment.aggregate({
    where: paymentWhere,
    _sum: { amount: true }
  });
  const invoicePaymentTotal = Number(invoicePaymentsAgg._sum?.amount ?? 0) || 0;
  ...
  return Math.round((invoicePaymentTotal + saleTotal) * 100) / 100;
}
