...
export async function generateIncomeStatementFromAccounts(
  ...
  // Include any bucket with value that wasn't matched to a category (e.g. category-only expenses, or account not in list)
  Object.entries(amountsByAccountId).forEach(([key, bucket]) => {
    const amount = Number(bucket?.amount) || 0;
    if (amount <= 0) return;
    if (key === 'DEP') return; // handled below
    const alreadyAdded = key && addedAccountIds.has(key);
    if (alreadyAdded) return;
    const displayName = (bucket.accountName || bucket.accountCode || key).replace(/^cat:/, '');
    operatingExpensesCategories.push({
      accountCode: bucket.accountCode || key,
      accountName: displayName,
      accountId: key,
      category: displayName,
      categoryNames: [displayName],
  ...
  const calculatedNetIncome = calculatedOperatingIncome - taxExpense;
