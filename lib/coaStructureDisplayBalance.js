...
function hasServerRolledBalance(row) {
  if (!row || typeof row !== 'object') return false;
  return (
    Object.prototype.hasOwnProperty.call(row, 'postedDirectBalance') ||
    Object.prototype.hasOwnProperty.call(row, 'postedGlNet') ||
    Object.prototype.hasOwnProperty.call(row, 'journalEntryBalance') ||
    Object.prototype.hasOwnProperty.call(row, 'balanceSource')
  );
}
...
export function structureNodeBalanceBreakdown(
  ...
  if (kids.length === 0) {
    ...
    display = childrenSum;
  } else if (matches.some(hasServerRolledBalance)) {
    display = leafSelf;
  } else {
    display = roundCents(leafSelf + childrenSum);
  }
