...
export async function calculateBudgetActuals(budgetId, tenantId, asOfDate = null) {
  ...
  const updatedItems = await Promise.all(
    budget.items.map(async (item) => {
      ...
      if (item.category) {
        // Category-based budget: sum expenses by category
        ...
        // Sum expenses for this category within the period
        ...
        // Account-based budget: use account balance changes
        const periodEnd = new Date(item.period);
        periodEnd.setHours(23, 59, 59, 999);
        ...
        actualAmount = Math.abs(endBalance.balance - startBalance.balance);
      }
...
export async function getBudgetVsActual(budgetId, tenantId, asOfDate = null) {
  ...
  budget.items.forEach(item => {
    ...
    // Determine type: if account exists, use account type; otherwise assume expense for category-based
    let accountType = null;
    if (item.account) {
      accountType = item.account.accountType;
    } else if (item.category) {
      // Category-based budgets are typically expenses
      accountType = 'Expense';
    }
