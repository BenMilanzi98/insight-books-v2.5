...
export async function createRevenueBudget(tenantId, userId, budgetData) {
  ...
  const budget = await prisma.legacyBudget.create({
    data: {
      tenantId,
      name,
      description,
...
export async function updateRevenueBudget(budgetId, tenantId, userId, updates) {
  ...
  const budget = await prisma.$transaction(async (tx) => {
    ...
    if (items && Array.isArray(items)) {
      await tx.legacyBudgetItem.deleteMany({ where: { budgetId } });
      updateData.items = {
        create: items.map(item => ({
          accountId: item.accountId || null,
          category: item.accountId ? (accountMap.get(item.accountId) || null) : null,
    ...
    if (breakdowns && Array.isArray(breakdowns)) {
      await tx.legacyRevenueBudgetBreakdown.deleteMany({ where: { budgetId } });
    ...
    return await tx.legacyBudget.update({
      where: { id: budgetId },
      data: updateData,
      include: {
        items: true,
...
export async function deleteRevenueBudget(budgetId, tenantId) {
  const budget = await prisma.legacyBudget.findFirst({
    where: { id: budgetId, tenantId }
  });
  ...
  if (budget.status === 'draft' || budget.status === 'active' || budget.status === 'approved') {
    await prisma.legacyBudget.delete({
      where: { id: budgetId }
    });
...
export async function activateBudget(budgetId, tenantId, userId) {
  ...
  const existingBudgetRecord = await prisma.legacyBudget.update({
    where: { id: budgetId },
    data: {
      status: 'active',
...
export async function approveBudget(budgetId, tenantId, approvedById) {
  ...
  return await prisma.legacyBudget.update({
    where: { id: budgetId },
    data: {
      status: 'approved',
      approvedById,
...
export async function autoCloseExpiredBudgets(tenantId) {
  ...
  const expiredBudgets = await prisma.legacyBudget.findMany({
    where: {
      tenantId,
...
export async function archiveBudgets(tenantId, beforeDate) {
  ...
  const updated = await prisma.legacyBudget.updateMany({
    where: {
      tenantId,
      status: 'closed',
      endDate: { lt: new Date(beforeDate) }
