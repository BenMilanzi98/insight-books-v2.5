 ...
 */
import { addMoney, MONEY_TOLERANCE_CENT, roundMoney, subtractMoney } from '@/lib/money';
...
export function validateTransactionBalance(lines) {
  ...
  const totalDebits = roundMoney(
    addMoney(
      ...lines.map((line) => roundMoney(line.debitAmount || 0))
    )
  );
  const totalCredits = roundMoney(
    addMoney(
      ...lines.map((line) => roundMoney(line.creditAmount || 0))
    )
  );

  const difference = Math.abs(subtractMoney(totalDebits, totalCredits));
  const tolerance = MONEY_TOLERANCE_CENT;
