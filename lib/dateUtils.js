 ...
 * Financial alignment: thisMonth/lastMonth = 1st to last day of month; thisQuarter/lastQuarter = calendar quarters (Q1=1 Jan–31 Mar, Q2=1 Apr–30 Jun, Q3=1 Jul–30 Sep, Q4=1 Oct–31 Dec); thisYear/lastYear = 1 Jan–31 Dec.
...
export function calculateDateRange(timeframe, previous = false, customRange = null) {
    const today = getCurrentDateInAfricaBlantyre();
    ...
    switch (timeframe) {
      ...
      case 'thisWeek':
        if (!previous) {
          ...
          startDate = new Date(thisWeekSun);
          startDate.setDate(thisWeekSun.getDate() - 7);
          endDate = new Date(thisWeekSun);
          endDate.setDate(thisWeekSun.getDate() - 1);
        }
        break;
      ...
      case 'lastWeek':
        if (!previous) {
          ...
          thisWeekSun.setDate(today.getDate() - today.getDay());
          startDate = new Date(thisWeekSun);
          startDate.setDate(thisWeekSun.getDate() - 14);
          endDate = new Date(thisWeekSun);
          endDate.setDate(thisWeekSun.getDate() - 8);
        }
        break;
      ...
      case 'lastQuarter':
        ...
        const lastQYear = currentQ === 0 ? currentYear - 1 : currentYear;
        
        if (!previous) {
          startDate = new Date(lastQYear, lastQ * 3, 1);
          endDate = new Date(lastQYear, (lastQ + 1) * 3, 0);
        } else {
          const prevQ = lastQ === 0 ? 3 : lastQ - 1;
          const prevQYear = lastQ === 0 ? lastQYear - 1 : lastQYear;
          startDate = new Date(prevQYear, prevQ * 3, 1);
          endDate = new Date(prevQYear, (prevQ + 1) * 3, 0);
        }
        break;
...
// Get current date in Africa/Blantyre timezone
export function getCurrentDateInAfricaBlantyre() {
  const now = new Date();
  const africaTimeString = now.toLocaleString("en-US", {timeZone: "Africa/Blantyre"});
  return new Date(africaTimeString);
}
  ...
  export function getDateRange(selectedRange) {
    ...
    switch (selectedRange) {
      ...
      case 'lastQuarter':
        ...
        const prevQuarter = curQ === 0 ? 3 : curQ - 1;
        const prevQuarterYear = curQ === 0 ? today.getFullYear() - 1 : today.getFullYear();
