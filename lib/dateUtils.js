...
/**
 * Parse API `YYYY-MM-DD` bounds as inclusive UTC civil days (same encoding as {@link civilDateToUtcNoon} / month math in {@link calculateDateRange}).
 * Avoids `new Date('YYYY-MM-DD')` + `setHours` mixing UTC and local and shifting the calendar day on export/display.
...
export function parseInclusiveApiYmdRange(startInput, endInput) {
  ...
  if (sHead && eHead) {
    // Same civil YMD encoding as {@link calculateDateRange} / {@link parseYmdToUtcNoon}; end = last ms of end day UTC so the whole last day is included.
    const start = parseYmdToUtcNoon(`${sHead[1]}-${sHead[2]}-${sHead[3]}`);
    const y = Number(eHead[1]);
    const m0 = Number(eHead[2]) - 1;
    const d = Number(eHead[3]);
    const end = new Date(Date.UTC(y, m0, d, 23, 59, 59, 999));
    return { start, end };
  }
