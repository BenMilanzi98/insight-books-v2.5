...
export function calculatePAYE(monthlyIncome) {
  return computeMalawiPayeMonthly(monthlyIncome).payeAmount;
}
...
export function calculateMalawiPayroll(payrollData, applyPAYE = false, applyNPS = false, npsRates = null) {
  ...
  // Tenant NPS % (null = not set). When NPS applies and tenant left rates unset, use statutory default 5%/5%.
  const resolveNpsPct = (raw) => {
  ...
  const employeeRatePercent =
    resolveNpsPct(npsRates?.employeeRatePercent) ??
    (applyNPS ? 5 : 0);
  const employerRatePercent =
    resolveNpsPct(npsRates?.employerRatePercent) ??
    (applyNPS ? 5 : 0);
