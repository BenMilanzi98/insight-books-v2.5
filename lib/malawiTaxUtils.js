 ...
 *
 * PAYE / NPS use **basic salary + overtime** only. **Allowances** (benefit amounts) are
 * not included in that base; they are **added to net pay after** deductions.
 */
 ...
 * @param {number} monthlyTaxableIncome - Income after allowable pre-PAYE deductions (e.g. employee pension)
...
export function calculateMalawiPayroll(payrollData, applyPAYE = false, applyNPS = false, npsRates = null) {
  ...
  // PAYE (Malawi): taxable income = gross − employee pension (NPS employee share) when NPS applies.
  const npsEmployeeForPayeBase = applyNPS ? (Number(npsCalculation.employeeAmount) || 0) : 0;
  const payeTaxableIncome = Math.max(0, taxableGrossPay - npsEmployeeForPayeBase);
  ...
  return {
    ...
    /** Monthly income used for PAYE brackets (after employee NPS when applicable). */
    payeTaxableIncome,
...
export function generatePayrollJournalEntries(payrollCalculation, tenantId) {
  ...
  const salaryExpenseDebit = taxable + allowancesAmt;

  const entries = [];

  // Debit: employment cost (taxable gross + allowances paid via net)
  entries.push({
