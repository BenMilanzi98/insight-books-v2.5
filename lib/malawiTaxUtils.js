...
export function calculateMalawiPayroll(payrollData) {
  ...
  const totalOtherDeductions = Object.values(otherDeductions).reduce((sum, amount) => sum + (amount || 0), 0);
  
  // Calculate net pay
  const totalDeductions = payeAmount + npsCalculation.employeeAmount + totalOtherDeductions;
