 ...
 *
 * PAYE is computed on **taxable income**: monthly gross (incl. overtime) minus the
 * employee’s pension (NPS employee contribution) when NPS applies — not on gross alone.
 */
...
export function calculateMalawiPayroll(payrollData, applyPAYE = false, applyNPS = false, npsRates = null) {
  ...
  const payeTaxableIncome = Math.max(0, totalGrossPay - npsEmployeeForPayeBase);
