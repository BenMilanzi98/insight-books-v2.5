...
export async function generatePaymentReceiptPDF(receiptData) {
  try {
    ...
    console.error('Error generating payment receipt PDF:', error?.message || error);
...
/**
 * PDF for quotations (same engine as invoices; simplified layout).
 */
export async function generateQuotationPdf(quotation, template, branding) {
  ...
  const subtotal = formatCurrencyForExport(quotation.subtotal ?? 0);
  const tax = formatCurrencyForExport(quotation.taxAmount ?? 0);
  const total = formatCurrencyForExport(quotation.total ?? 0);

  let logoHtml = '';
  if (showLogo && branding?.logoUrl) {
    logoHtml = `<img src="${branding.logoUrl}" alt="Logo" style="max-height:72px;max-width:220px;object-fit:contain;">`;
  } else {
    logoHtml = `<div style="font-size:1.35rem;font-weight:bold;">${branding?.companyName || 'Your Company'}</div>`;
  }
  ...
  const rows = items
    .map((item) => {
      ...
      const rate = formatAmountForExport(item.unitPrice);
      const line = formatAmountForExport((item.quantity || 0) * (item.unitPrice || 0));
      return `<tr>
  ...
  const footerBlock =
    showFooter && (branding?.emailFooter || branding?.businessPhone || branding?.defaultBankDetails)
      ? `<div style="margin-top:24px;padding-top:16px;border-top:1px solid #eee;font-size:12px;color:#555;">
          ${branding?.businessPhone ? `<p>Tel: ${branding.businessPhone}</p>` : ''}
          ${branding?.defaultBankDetails ? `<p style="white-space:pre-wrap;">${branding.defaultBankDetails}</p>` : ''}
          ${branding?.emailFooter ? `<p>${branding.emailFooter}</p>` : ''}
        </div>`
      : '';
  ...
  const bodyHtml = `
    <div style="max-width:800px;margin:0 auto;padding:24px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;">
        <div>
          <h1 style="color:${primaryColor};margin:0;font-size:26px;">QUOTATION</h1>
          <p style="color:#71717a;margin-top:6px;">#${qNum}</p>
          <p style="margin-top:8px;font-size:13px;">Issue: ${quotation.issueDate || ''} &nbsp;·&nbsp; Valid until: ${quotation.validUntil || ''}</p>
          <p style="margin-top:4px;font-size:13px;">Status: ${quotation.status || ''}</p>
        </div>
        <div>${logoHtml}</div>
      </div>
      <div style="margin-bottom:20px;">
        <h3 style="color:#4b5563;margin-bottom:8px;">Client</h3>
        <p style="margin:4px 0;font-weight:600;">${(client.name || '').toString()}</p>
        ${client.address ? `<p style="margin:4px 0;font-size:14px;">${(client.address || '').toString()}</p>` : ''}
        ${client.email ? `<p style="margin:4px 0;font-size:14px;">${client.email}</p>` : ''}
        ${client.phone ? `<p style="margin:4px 0;font-size:14px;">${client.phone}</p>` : ''}
      </div>
      <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
        <thead>
          <tr style="background:#f3f4f6;">
            <th style="text-align:left;padding:10px;font-size:12px;color:#6b7280;">Item</th>
            <th style="text-align:right;padding:10px;font-size:12px;color:#6b7280;">Qty</th>
            <th style="text-align:right;padding:10px;font-size:12px;color:#6b7280;">Rate (MWK)</th>
            <th style="text-align:right;padding:10px;font-size:12px;color:#6b7280;">Amount (MWK)</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
      <div style="display:flex;justify-content:flex-end;">
        <div style="width:260px;">
          <div style="display:flex;justify-content:space-between;padding:6px 0;"><span>Subtotal</span><span>${subtotal}</span></div>
          <div style="display:flex;justify-content:space-between;padding:6px 0;"><span>Tax</span><span>${tax}</span></div>
          <div style="display:flex;justify-content:space-between;padding:10px 0;margin-top:6px;border-top:2px solid ${primaryColor};font-weight:bold;color:${primaryColor};">
            <span>Total</span><span>${total}</span>
          </div>
        </div>
      </div>
      ${quotation.notes ? `<div style="margin-top:20px;"><h4 style="color:#4b5563;">Notes</h4><p style="font-size:14px;color:#555;">${(quotation.notes || '').toString()}</p></div>` : ''}
      ${footerBlock}
    </div>
  `;
    ...
    <html>
      <head>
        ...
        <style>
          ...
          body { font-family: system-ui, -apple-system, sans-serif; margin: 0; -webkit-print-color-adjust: exact; }
        </style>
      </head>
      <body>${bodyHtml}</body>
    </html>
  ...
  try {
    ...
    console.error('Error generating quotation PDF:', error?.message || error);
