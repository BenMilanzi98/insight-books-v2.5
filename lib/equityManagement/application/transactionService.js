/**
 * Equity Transaction lifecycle — create and post directly (no approval gate).
 */
...
export async function createEquityTransaction(db, context, input) {
  ...
  // Simplified equity: always ready to post (no approval / SoD gate)
...
/**
 * Create and post in one step (UI happy path).
 */
export async function createAndPostEquityTransaction(db, context, input, { hasPermission } = {}) {
  const tx = await createEquityTransaction(db, context, input);
  return postEquityTransaction(db, context, tx.id, { hasPermission });
}
...
export async function postEquityTransaction(db, context, id, { hasPermission } = {}) {
  ...
  // Direct-post path: APPROVED or NOT_REQUIRED are both allowed
  if (
    tx.status !== EquityTxStatus.APPROVED &&
    tx.approvalStatus !== 'NOT_REQUIRED' &&
    tx.approvalStatus !== 'APPROVED'
  ) {
    // Auto-promote drafts left from older UI
    if ([EquityTxStatus.DRAFT, EquityTxStatus.PENDING_APPROVAL].includes(tx.status)) {
      ...
      throw new AccountingValidationError('Transaction cannot be posted in its current status.');
    }
  }
