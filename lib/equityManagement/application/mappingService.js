...
export async function resolveEquityAccountByPurpose(db, tenantId, purpose) {
  ...
  if (!account) {
    ...
    throw new AccountingValidationError(
      `Map a Chart of Accounts posting account for “${purpose.replace(/_/g, ' ').toLowerCase()}” before posting equity (system purpose ${purpose}).`,
...
export async function resolveBankOrCashAccount(db, tenantId, accountId) {
  ...
  if (!account) {
    throw new AccountingValidationError(
      'Select a valid bank/cash account from your payment accounts (linked to Chart of Accounts).',
