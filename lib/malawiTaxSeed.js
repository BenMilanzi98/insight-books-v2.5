...
export async function ensureMalawiTaxTypesForTenant(tenantId, tx = prisma, options = {}) {
  ...
  for (const def of MALAWI_TAX_CATALOG) {
    ...
    if (applyCatalogRates && def.isSystem) {
  ...
  // Default VAT types on tenant settings when unset
  const vatIn = await tx.taxType.findFirst({
    where: { tenantId, taxId: 'MW-VAT', status: 'Active' },
    select: { id: true, accountId: true },
  });
  const vatOut = await tx.taxType.findFirst({
    where: { tenantId, taxId: 'MW-VAT-IN', status: 'Active' },
    select: { id: true, accountId: true },
  });
  ...
  // Bump legacy default VAT rate on tenant settings (16.5 → current MRA standard)
  await tx.tenantSettings.updateMany({
    where: { tenantId, defaultTaxRate: 16.5 },
