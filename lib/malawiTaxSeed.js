...
export async function resolveTaxGlAccountForPosting(tenantId, taxType, sourceType, tx = prisma) {
  ...
  const taxId = String(taxType?.taxId || '').toUpperCase();
  const taxCode = String(taxType?.taxCode || '').toUpperCase();
  const taxName = String(taxType?.taxName || '').toUpperCase();
  const isVatOutput =
    taxId === 'MW-VAT' ||
    taxId === 'A' ||
    taxCode.startsWith('MW-VAT') ||
    taxCode === 'VAT' ||
    taxCode === 'VAT16.5' ||
    (taxName.includes('VAT') && !taxName.includes('INPUT'));

  // Sales invoices credit VAT_OUTPUT (2120). Settlements must debit the same account.
  if (isVatOutput && (src === 'TaxPayment' || src.includes('TaxSettlement'))) {
    try {
      const { resolvePurposeAccount } = await import('./coaV2/application/accountMappingRegistry.js');
      const vatPayable = await resolvePurposeAccount(
        { tenantId, businessId: tenantId },
        'VAT_OUTPUT',
        {},
        tx
      );
      if (vatPayable?.id) return vatPayable;
    } catch {
      /* fall through to legacy code lookup */
    }
    const acct2120 = await tx.account.findFirst({
      where: { tenantId, accountCode: '2120', isActive: true },
    });
    if (acct2120) return acct2120;
  }
