...
/**
 * Secure cookies only work over HTTPS. NODE_ENV=production alone is wrong for
 * http://localhost and http://VPS-IP:3000 — browsers silently drop the cookie
 * and login appears to "fail" (redirect loop back to /login).
 */
export function shouldUseSecureCookies() {
  ...
  const appUrl =
    ...
    '';
  if (/^https:\/\//i.test(String(appUrl).trim())) return true;
  return false;
}
