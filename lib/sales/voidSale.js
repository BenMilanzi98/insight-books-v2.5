// lib/sales/voidSale.js
// Extracted from the POST body of app/api/sales/[id]/void/route.js so the route and
// the desktop outbox void through the same inventory restore + GL reversal path.
import prisma from '@/lib/prisma';
import { reverseSaleGlForRefundInTx } from '@/lib/transactionReversalService';
import { serviceError } from '@/lib/serviceErrors';

/**
 * Void a completed sale: restore stock, reverse the payment and reverse posted GL.
 *
...
export async function voidSale({ user, saleId, reason }) {
  ...
  if (!reason || reason.trim() === '') {
    throw serviceError('Void reason is required', { status: 400 });
  }
  if (reason.trim().length < 10) {
    throw serviceError(
      'Void reason must be at least 10 characters (audit and GL reversal requirement).',
      { status: 400 }
    );
  }
