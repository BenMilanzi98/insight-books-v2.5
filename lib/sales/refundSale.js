// lib/sales/refundSale.js
// Extracted from the POST body of app/api/sales/[id]/refund/route.js so the route and
// the desktop outbox refund through the same inventory restore + GL reversal path.
import prisma from '@/lib/prisma';
import { reverseSaleGlForRefundInTx } from '@/lib/transactionReversalService';
import { serviceError } from '@/lib/serviceErrors';

/**
 * Refund a completed sale: restore stock, book the negative payment and reverse GL.
 *
 * @returns {Promise<object>} the updated sale row.
 */
export async function refundSale({ user, saleId, body = {} }) {
  if (!saleId) {
    throw serviceError('Sale ID is required', { status: 400 });
  }
  ...
  if (!reason || reason.trim() === '') {
    throw serviceError('Refund reason is required', { status: 400 });
  }
  if (reason.trim().length < 10) {
    throw serviceError(
      'Refund reason must be at least 10 characters (audit and GL reversal requirement).',
      { status: 400 }
    );
  }
