...
/**
 * Guided recon SoD is opt-in. Flip leftover requireSeparateApprover=true rows
 * off when a recon is created or resumed so a solo user can complete.
 */
export async function ensureGuidedSodOptIn(db, context, paymentAccountId) {
  const cfg = await getConfiguration(db, context.businessId, paymentAccountId);
  if (!cfg) {
    return upsertConfiguration(db, context, {
      paymentAccountId,
      requireSeparateApprover: false,
    });
  }
  if (cfg.requireSeparateApprover) {
    await db.bankRecConfiguration.updateMany({
      where: { id: cfg.id, tenantId: context.businessId, requireSeparateApprover: true },
      data: { requireSeparateApprover: false, updatedBy: context.userId ?? null },
    });
    return { ...cfg, requireSeparateApprover: false };
  }
  return cfg;
}
...
export async function createReconciliation(db, context, input) {
  ...
  const cfg = await ensureGuidedSodOptIn(db, context, pa.id);
...
export async function getReconciliationWorkspace(db, context, reconciliationId) {
  const recon = await requireRecon(db, context, reconciliationId);
  // Resume/continue skips createReconciliation — still flip leftover SoD on for this account.
  await ensureGuidedSodOptIn(db, context, recon.paymentAccountId);
