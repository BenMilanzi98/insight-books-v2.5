...
export async function createReconciliation(db, context, input) {
  ...
  if (!cfg) {
    cfg = await upsertConfiguration(db, context, {
      paymentAccountId: pa.id,
      requireSeparateApprover: false,
    });
  } else if (cfg.requireSeparateApprover) {
    // Guided product default: SoD is opt-in so a solo user can complete recon.
    ...
    cfg = { ...cfg, requireSeparateApprover: false };
  }
