...
export function generateInvoiceHtml(invoice, template, branding) {
  const content = typeof template?.content === 'string'
    ...
    : template?.content || {};
  const primaryColor = content.primaryColor || branding?.primaryColor || '#0075be';
  const layoutStyle = String(content.style || 'standard').toLowerCase();
  const showFooter = content.showFooter !== false;
  const boldHeader = ['bold', 'professional', 'modern'].includes(layoutStyle);
...
<div class="page" style="padding:28px">
  <!-- Header -->
  <div style="${boldHeader ? `background:${primaryColor};color:#fff;` : `border-left:4px solid ${primaryColor};`}padding:24px 24px 16px">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;flex-wrap:wrap">
      <div>
        ${logoImg(branding) || `<p style="font-size:18px;font-weight:600;color:${boldHeader ? '#fff' : '#111827'}">${esc(branding?.companyName || 'Company')}</p>`}
