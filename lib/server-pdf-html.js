...
export function generateInvoiceHtml(invoice, template, branding) {
  ...
  const boldHeader = ['modern', 'band-header', 'bold-bar'].includes(layoutId);
  ...
  const headerHtml = (() => {
    const brand = logoImg(branding) || `<p style="font-size:18px;font-weight:600">${esc(branding?.companyName || 'Company')}</p>`;
    if (layoutId === 'band-header') {
      return `<div style="display:flex"><div style="flex:2;background:${primaryColor};color:#fff;padding:20px">${brand}</div><div style="flex:3;background:#1e293b;color:#fff;padding:20px"><p style="font-size:22px;font-weight:700">Invoice</p><p style="opacity:.8">#${esc(invoice.invoiceNumber)}</p></div></div>`;
    }
    if (layoutId === 'bold-bar') {
      return `<div style="border-top:6px solid ${primaryColor};padding:20px 24px"><div style="display:flex;flex-direction:column;align-items:${logoAlign}">${brand}<p style="font-size:28px;font-weight:800;margin-top:8px">Invoice</p><p style="color:#64748b">#${esc(invoice.invoiceNumber)}</p></div></div>`;
    }
    if (layoutId === 'soft-card') {
      return `<div style="padding:16px"><div style="border:1px solid #e5e7eb;border-radius:16px;padding:20px;box-shadow:0 1px 2px rgba(0,0,0,.04);display:flex;flex-direction:column;align-items:${logoAlign}">${brand}<p style="font-size:22px;font-weight:600;margin-top:8px">Invoice</p><p style="color:#64748b">#${esc(invoice.invoiceNumber)}</p></div></div>`;
    }
    if (layoutId === 'ledger') {
      return `<div style="padding:20px 24px;border-bottom:2px solid #1e293b"><div style="display:flex;justify-content:space-between;align-items:flex-end"><div style="display:flex;flex-direction:column;align-items:${logoAlign}">${brand}<p style="font-size:16px;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-top:8px">Invoice</p></div><p style="font-family:ui-monospace,monospace">#${esc(invoice.invoiceNumber)}</p></div></div>`;
    }
    if (layoutId === 'split-brand') {
      return `<div style="display:flex;border-bottom:1px solid #e5e7eb"><div style="flex:1;padding:20px;display:flex;flex-direction:column;align-items:${logoAlign}">${brand}${sellerTpin ? `<p style="font-size:12px;color:#64748b;margin-top:8px">TPIN: ${esc(sellerTpin)}</p>` : ''}</div><div style="flex:1;padding:20px;background:#f8fafc;border-left:1px solid #e5e7eb"><p style="font-size:20px;font-weight:700;color:${primaryColor}">Invoice</p><p style="color:#64748b">#${esc(invoice.invoiceNumber)}</p></div></div>`;
    }
    if (layoutId === 'minimal' || layoutId === 'compact') {
      return `<div style="padding:${layoutId === 'compact' ? '12px 16px' : '24px'};display:flex;justify-content:space-between;align-items:flex-start"><div style="display:flex;flex-direction:column;align-items:${logoAlign}"><p style="font-size:${layoutId === 'compact' ? '14px' : '18px'};color:${primaryColor}">Invoice #${esc(invoice.invoiceNumber)}</p>${brand}</div></div>`;
    }
    if (layoutId === 'editorial') {
      return `<div style="padding:28px 24px 8px"><p style="font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:${primaryColor}">Invoice</p></div><div style="display:flex;flex-direction:column;align-items:${logoAlign};padding:0 24px 16px">${brand}<h1 style="font-size:26px;margin-top:12px">${esc(invoice.title?.trim?.()) || `Invoice ${esc(invoice.invoiceNumber)}`}</h1><p style="color:#64748b;margin-top:8px">#${esc(invoice.invoiceNumber)} · ${formatDate(invoice.issueDate)}</p></div>`;
    }
    // classic / modern default
    return `<div style="${boldHeader ? `background:${primaryColor};color:#fff;` : `border-left:4px solid ${primaryColor};`}padding:24px 24px 16px">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;flex-wrap:wrap">
      <div style="display:flex;flex-direction:column;align-items:${logoAlign}">
        ${brand}
        ${sellerTpin ? `<p style="margin-top:8px;font-size:12px;color:${boldHeader ? '#e0f2fe' : '#4b5563'}"><b>TPIN:</b> ${esc(sellerTpin)}</p>` : ''}
      </div>
      <div style="text-align:right">
        <p style="font-size:24px;font-weight:700;color:${boldHeader ? '#fff' : '#111827'}">Invoice</p>
        <p style="font-size:14px;color:${boldHeader ? '#e0f2fe' : '#6b7280'};margin-top:2px">#${esc(invoice.invoiceNumber)}</p>
  ...
  const showLineTax = documentHasLineTax(items);
  ...
  const itemRows = items.map((it, i) => `
    <tr style="background:${i % 2 === 1 ? '#f9fafb' : '#fff'}">
      ...
      ${showLineTax ? `<td style="padding:10px 8px;text-align:right;color:#4b5563">${it.taxRate > 0 ? `${it.taxRate}%` : '\u2014'}</td>` : ''}
  ...
  const totalPaid = eligiblePayments.reduce((s, p) => s + (parseFloat(p.amount) || 0), 0);
  ...
  <div style="padding:20px 24px">
    ...
    <table style="font-size:14px">
      <thead>
        <tr style="border-bottom:2px solid #e5e7eb">
          ...
          ${showLineTax ? '<th style="text-align:right;padding:12px 8px;font-weight:600;color:#374151">Tax</th>' : ''}
    ...
    <div class="ib-doc-keep" style="display:flex;justify-content:flex-end;margin-top:24px">
      <div style="width:256px;border:1px solid #e5e7eb;border-radius:8px;background:#f9fafb;padding:16px;font-size:14px">
        ...
        ${showDocumentTax ? `<div style="display:flex;justify-content:space-between;padding:6px 0;color:#4b5563"><span>Tax</span><span style="font-weight:500;color:#111827">${fmtCurrency(invoice.taxAmount)}</span></div>` : ''}
        ...
        ${totalPaid > 0 || eligiblePayments.length > 0 ? `
    ...
    <div style="margin-top:20px;padding:16px;border:1px solid #e5e7eb;border-radius:8px;background:#f8fafc">
      ...
      <table style="width:100%;font-size:13px;border-collapse:collapse">
        ...
        <tbody>
          ${eligiblePayments.map((p) => `<tr style="border-bottom:1px solid #f1f5f9">
            <td style="padding:8px 6px;color:#334155">${formatDate(p.paymentDate)}</td>
            <td style="padding:8px 6px;color:#334155">${esc(p.paymentMethod || '')}</td>
            <td style="padding:8px 6px;color:#64748b">${p.reference ? esc(String(p.reference)) : '\u2014'}</td>
            <td style="padding:8px 6px;text-align:right;font-weight:500">${fmtCurrency(parseFloat(p.amount) || 0)}</td>
          </tr>`).join('')}
        </tbody>
      </table>
    </div>` : ''}
  ...
  <div style="padding:20px 24px">
    ...
    <div class="ib-doc-keep" style="display:flex;justify-content:flex-end;margin-top:24px">
      <div style="width:256px;border:1px solid #e5e7eb;border-radius:8px;background:#f9fafb;padding:16px;font-size:14px">
        ...
        ${showDocumentTax ? `<div style="display:flex;justify-content:space-between;padding:6px 0;color:#4b5563"><span>Tax</span><span style="font-weight:500;color:#111827">${fmtCurrency(quotation.taxAmount)}</span></div>` : ''}
