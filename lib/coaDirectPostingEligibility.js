...
import { isPaymentGlParentCode } from './paymentGlChannels.js';
import { OPENING_BALANCE_EQUITY_CODE } from './coaMoney.js';
...
export function accountBlocksDirectPosting(account, overrides = {}) {
  ...
  /** Bank/mobile rollup parents and 1130 group — post only to child accounts. */
  if (glCode === '1130' || isPaymentGlParentCode(glCode)) {
    return {
      blocked: true,
      reason:
        'This bank or mobile money channel is a rollup parent. Post to a sub-account beneath it (e.g. 1131-01), not to the parent line.',
      details: coaAccountDisplayLabel(account) || glCode,
    };
  }

  /** Canonical payroll salary expense (5200) always accepts direct postings. */
  if (glCode === '5200') {
  ...
  /** Opening Balance Equity — system-only counter-account for onboarding (OpeningBalanceService). */
  if (glCode === OPENING_BALANCE_EQUITY_CODE) {
    return { blocked: false };
  }
 ...
 */
export async function assertAccountsAllowDirectPosting(accountIds, tx, options = {}) {
  if (options.allowBlockedAccountForReversal) return;
