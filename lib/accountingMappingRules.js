...
export async function getPostableExpenseAccountOptions(tenantId, db = prisma) {
  try {
    const { ensureMissingBlueprintAccountsForTenant } = await import(
      './chartOfAccountsInitialization.js'
    );
    await ensureMissingBlueprintAccountsForTenant(tenantId, db);
  } catch (err) {
    console.warn(
      'ensureMissingBlueprintAccountsForTenant (expense options):',
      err?.message || err
    );
  }
...
export async function resolveCanonicalSalaryExpenseAccount(tenantId, db = prisma) {
  try {
    ...
    console.warn(
      'ensureMissingBlueprintAccountsForTenant (canonical salary):',
      err?.message || err
    );
  }
  ...
  // Heal common CoA drift so payroll works for every tenant.
  const patch = {};
  if (account.isActive === false) patch.isActive = true;
  if (account.visibleInChart === false) patch.visibleInChart = true;
  if (account.acceptsNewTransactions === false) patch.acceptsNewTransactions = true;
  if (account.mergedIntoAccountId) patch.mergedIntoAccountId = null;
  const typeOk =
    String(account.accountType || '').toLowerCase() === 'expense' ||
    String(account.type || '').toLowerCase() === 'expense';
  if (!typeOk) {
    patch.accountType = 'Expense';
    patch.type = 'EXPENSE';
  }
  if (String(account.accountCode || '').trim() !== CANONICAL_SALARY_ACCOUNT_CODE) {
    patch.accountCode = CANONICAL_SALARY_ACCOUNT_CODE;
  }
  ...
  if (!isCanonicalSalaryExpenseAccount(account)) {
    throw new Error(
      `${CANONICAL_SALARY_ACCOUNT_CODE} - ${CANONICAL_SALARY_ACCOUNT_NAME} cannot receive payroll postings (invalid account classification).`
    );
  }
