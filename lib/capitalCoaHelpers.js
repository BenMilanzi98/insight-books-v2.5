 ...
 * Skips non-postable parents (bank/mobile rollups) and prefers postable leaves.
 */
export async function resolveContributionCashDebitAccount(tenantId, cashAccountId, db = prisma) {
  const { getAccountForPaymentMethod } = await import('./paymentMethodAccountMapping.js');

  if (cashAccountId) {
    try {
      const viaPaymentMap = await getAccountForPaymentMethod(tenantId, cashAccountId, db);
      if (viaPaymentMap && isCoaAssetAccount(viaPaymentMap) && !isInventoryLedgerAccount(viaPaymentMap)) {
        return viaPaymentMap;
      }
    } catch {
      /* fall through */
    }
    ...
    if (paymentAccount?.coaAccountId) {
      try {
        const viaLinked = await getAccountForPaymentMethod(
          tenantId,
          paymentAccount.coaAccountId,
          db
        );
        if (viaLinked && isCoaAssetAccount(viaLinked) && !isInventoryLedgerAccount(viaLinked)) {
          return viaLinked;
        }
      } catch {
        /* fall through */
  ...
  try {
    return await getAccountForPaymentMethod(tenantId, 'cash', db);
  } catch {
    /* fall through */
  }
...
export async function resolveOrEnsureOwnersCapitalGlAccount(tenantId, db = prisma) {
  ...
  // Prefer a light create over full CoA bootstrap (bootstrap can take minutes and
  // time out API requests like POST /api/assets for existing/owner-contributed assets).
  ...
  if (!parent3000) {
    try {
      parent3000 = await db.account.create({
        data: {
          tenantId,
          accountCode: EQUITY_GROUP_GL_CODE,
          code: EQUITY_GROUP_GL_CODE,
          accountName: 'Equity',
          name: 'Equity',
          accountType: 'Equity',
          type: 'Equity',
