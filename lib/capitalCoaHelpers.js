...
/** Owner's Capital — primary pool; contribution detail posts to 3101–3199 children. */
...
/**
 * Roll up child contribution balances into Owner's Capital (3100) display balance.
 * 3100 is a rollup parent — direct postings are blocked; this keeps UI totals in sync.
 */
export async function syncCapitalParentRollupBalance(tenantId, parentAccountId, db = prisma) {
  const children = await db.account.findMany({
    where: { tenantId, parentAccountId: parentAccountId, isActive: true },
    select: { balance: true },
  });
  const rollup = children.reduce((sum, c) => sum + Number(c.balance || 0), 0);
