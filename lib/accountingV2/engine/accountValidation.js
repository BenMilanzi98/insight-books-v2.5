...
import { PAYMENT_CASH_GL_CODE } from '../../paymentGlChannels.js';
...
/**
 * Cash on hand (1110) is the canonical posting leaf even when legacy child rows exist.
 * Matches `canUsePaymentAssetAccount` in paymentMethodAccountMapping.js.
 */
function allowsPostingDespiteActiveChildren(account) {
  const code = String(account?.accountCode ?? account?.code ?? '').trim();
  return code === PAYMENT_CASH_GL_CODE;
}
/**
...
export function validateLineAccount(params) {
  ...
  // (except canonical cash 1110, which remains postable with legacy children).
  const childCount = account._count?.childAccounts ?? 0;
  if (
    account.coaV2Behaviour === AccountBehaviour.HEADER ||
    (childCount > 0 && !allowsPostingDespiteActiveChildren(account))
  ) {
    throw new NonPostingAccountError(ref, ids);
  }
