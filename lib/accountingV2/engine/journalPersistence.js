...
export async function createPostedJournal(tx, context, params) {
  ...
  try {
    ...
    if (err && typeof err === 'object' && err.code === 'P2002') {
      // Unique violation on accountingEventId or journalNumber — concurrency guard fired.
      throw new JournalPersistenceError({
        requestId: context.requestId,
        correlationId: context.correlationId,
        retryable: false,
        diagnostic: { code: 'P2002', target: err.meta?.target },
      });
    }
    throw new JournalPersistenceError({
      requestId: context.requestId,
      correlationId: context.correlationId,
      diagnostic: { cause: err instanceof Error ? err.message : String(err) },
    });
  }
}
