...
export async function runValidationPipeline(db, context, command, options) {
  ...
  try {
    ...
    const approvalOverride = command.metadata?.approvalOverride;
    const requirement = approvalOverride?.allowSelfApproval
      ? { ...approvalRequirement, required: true, separationOfDuties: false, reason: approvalRequirement.reason }
      : approvalRequirement;
    validateApproval({
      context,
      requirement,
      approval: approvalOverride
        ? {
            approvedById: approvalOverride.approvedById,
            approvedAt: approvalOverride.approvedAt,
            createdById: approvalOverride.createdById ?? null,
          }
        : source
          ? {
