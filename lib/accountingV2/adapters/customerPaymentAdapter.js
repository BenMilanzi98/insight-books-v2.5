...
import { resolveWhtReceivableAccount } from '../../invoicePaymentWithholding.js';
...
export async function postCustomerPaymentAccounting({
  ...
  const withholdingAmount = Number(payment?.withholdingAmount || 0);
  ...
  return submitViaCutover({
    ...
    buildEngineInput: async () => {
      ...
      if (withholdingAmount > 0) {
        metadata.withholdingAmount = withholdingAmount;
        const whtAcct = await resolveWhtReceivableAccount(db, tenantId);
        if (whtAcct?.id) metadata.withholdingAccountId = whtAcct.id;
      }
