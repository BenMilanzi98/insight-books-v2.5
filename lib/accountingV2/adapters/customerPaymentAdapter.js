...
export async function postCustomerPaymentAccounting({
  ...
  const payment = await db.payment.findFirst({
    ...
    include: { invoice: { select: { id: true, clientId: true, invoiceNumber: true } } },
  });
  const invoice = payment?.invoice
    || (invoiceId
      ? await db.invoice.findFirst({ where: { id: invoiceId, tenantId }, select: { id: true, clientId: true, invoiceNumber: true } })
      : null);
