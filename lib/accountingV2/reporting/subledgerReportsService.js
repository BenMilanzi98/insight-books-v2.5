...
export async function generateBudgetVsActual(db, context, request) {
  const { resolveBudgetActuals } = await import('@/lib/budgetForecast/application/budgetActualsService.js');
  const { computeVariance, classifyAccountKind } = await import('@/lib/budgetForecast/domain/variance.js');
  ...
  if (typeof db.budgetLine?.findMany === 'function') {
    const budgetLines = await db.budgetLine.findMany({
      where: {
        budget: {
          tenantId: context.businessId,
          status: { in: ['ACTIVE', 'APPROVED', 'LOCKED', 'active', 'approved', 'closed'] },
        },
      },
      include: {
        periodAmounts: true,
    ...
    const to = request.toDate || request.asOfDate ? new Date(request.toDate || request.asOfDate) : null;
    for (const line of budgetLines) {
      let minor = 0;
      const periods = Array.isArray(line.periodAmounts) ? line.periodAmounts : [];
      if (periods.length) {
        for (const p of periods) {
          const ps = new Date(p.periodStart);
          const pe = new Date(p.periodEnd);
          const overlaps = (!from || pe >= from) && (!to || ps <= to);
          if (overlaps) minor += Number(p.plannedAmountMinor || 0);
        }
      } else {
        minor = Number(line.annualAmountMinor || 0);
      }
      budgetByAccount.set(line.accountId, (budgetByAccount.get(line.accountId) || 0) + minor);
      labelByAccount.set(line.accountId, {
        accountCode: line.accountCodeSnapshot || line.account?.accountCode,
        accountName: line.accountNameSnapshot || line.account?.accountName,
      });
    }
  }

  if (db.legacyBudgetItem) {
    const budgetItems = await db.legacyBudgetItem.findMany({
      where: {
        period: {
        ...
        budget: { tenantId: context.businessId, status: { in: ['active', 'approved', 'closed'] } },
      },
    }).catch(() => []);
  ...
  const actualByAccount = new Map((actuals.accounts || []).map((a) => [a.accountId, a]));
  const accountIds = new Set([...budgetByAccount.keys(), ...actualByAccount.keys()]);

  const lines = [];
  for (const accountId of accountIds) {
    const actual = actualByAccount.get(accountId);
    const budgetMinor = budgetByAccount.get(accountId) ?? 0;
    const actualMinor = actual?.actualMinor ?? 0;
    const kind = actual?.kind || classifyAccountKind(actual?.accountType, actual?.accountCategory);
    const variance = computeVariance(kind, budgetMinor, actualMinor);
    const label = labelByAccount.get(accountId);
    const built = buildReportLine({
      lineId: `bva-${accountId}`,
      code: actual?.accountCode || label?.accountCode || null,
      label: actual?.accountName || label?.accountName || accountId,
      lineType: 'VARIANCE',
      ...
      accounts: actual
        ? [{
            ...
            amount: amount(actualMinor),
            lineCount: actual.lineCount,
          }]
        : [],
      metadata: {
        kind,
        varianceMinor: variance.favourableVarianceMinor,
        variancePercent: variance.variancePercent,
        isFavourable: variance.isFavourable,
        status: variance.status,
        percentState: variance.percentState,
      },
    });
    built.budgetVariance = amount(variance.favourableVarianceMinor);
    built.variancePercentage = variance.variancePercent;
    lines.push(built);
  }

  lines.sort((a, b) => String(a.code || a.label).localeCompare(String(b.code || b.label)));
  ...
  const envelope = buildReportEnvelope(context, request, { id: 'BVA-FOUNDATION', name: 'Budget versus Actual', version: '1.0.0' }, {
    ...
    totals: {
      ...
      variance: amount(lines.reduce((s, l) => s + (l.budgetVariance?.minor ?? 0), 0)),
    },
    unresolvedExceptions: [],
    sourceNotes: 'Actuals from canonical General Ledger via budgetForecast; variance sign from computeVariance (expenses favourable when under budget). Unbudgeted actuals are included.',
  });
  envelope.integrityStatus = REPORT_INTEGRITY_STATUS.VERIFIED;
  return envelope;
}
