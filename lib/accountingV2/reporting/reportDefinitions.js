...
export function resolveAccountProfile(account) {
  ...
  const category = explicitCategory || LEGACY_TYPE_CATEGORY[legacyType] || null;

  const subType = String(account.coaV2SubType ?? '').toUpperCase() || null;
  ...
  const assist = {
    ...
    taxExpense: nameHas(account, ['tax expense', 'corporate tax', 'income tax expense']),
 ...
 */
export function accountMatchesRule(profile, rule) {
...
const INCOME_STATEMENT_V1 = Object.freeze({
  ...
  lines: [
    ...
    line({ lineId: 'cost-of-sales', label: 'Cost of Sales', lineType: 'ACCOUNT_GROUP', displaySign: 1, displayOrder: 20, match: { categories: ['COST_OF_SALES'] } }),
    ...
    line({
      ...
      match: {
        categories: ['EXPENSE'],
        excludeSubTypes: ['DEPRECIATION', 'AMORTIZATION', 'FINANCE_COST', 'INTEREST', 'TAX', 'INCOME_TAX'],
    ...
    line({ lineId: 'profit-before-tax', label: 'Profit Before Tax', lineType: 'CALCULATED_TOTAL', displayOrder: 110, formula: [{ op: '+', ref: 'operating-profit' }, { op: '+', ref: 'other-income' }, { op: '-', ref: 'other-expenses' }, { op: '-', ref: 'finance-costs' }] }),
    line({
      lineId: 'tax-expense', label: 'Tax Expense', lineType: 'ACCOUNT_GROUP', displaySign: 1, displayOrder: 120,
      match: { categories: ['EXPENSE', 'OTHER_EXPENSE'], subTypes: ['TAX', 'INCOME_TAX'], assistAny: ['taxExpense'] },
    }),
    line({ lineId: 'net-profit', label: 'Net Profit / (Loss)', lineType: 'GRAND_TOTAL', displayOrder: 130, formula: [{ op: '+', ref: 'profit-before-tax' }, { op: '-', ref: 'tax-expense' }] }),
  ],
});
...
export function assignAccountsToLines(definition, accountRows, inScope = () => true) {
  ...
  for (const row of accountRows) {
    ...
    const target = groupLines.find((l) => accountMatchesRule(profile, l.match));
    if (!target) {
