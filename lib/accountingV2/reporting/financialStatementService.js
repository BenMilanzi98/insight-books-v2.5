...
function displayAccountName(r) {
  ...
  if (code === '5100' && /cost of sales/i.test(name)) return 'Cost of Goods';
...
function slimIncomeStatementEnvelope(envelope) {
  ...
  if (opex) {
    ...
    const periodExtras = new Map();
    for (const id of SLIM_PL_FOLD_INTO_OPEX) {
      const line = byId.get(id);
      extra += line?.currentAmount?.minor ?? 0;
      extraAccounts.push(...(line?.accounts || []));
      for (const pa of line?.periodAmounts || []) {
        periodExtras.set(pa.key, (periodExtras.get(pa.key) || 0) + (pa.amount?.minor || 0));
      }
    }
    ...
    if (opex.periodAmounts?.length) {
      opex.periodAmounts = opex.periodAmounts.map((pa) => {
        let minor = (pa.amount?.minor || 0) + (periodExtras.get(pa.key) || 0);
        const oi = otherIncome?.periodAmounts?.find((x) => x.key === pa.key);
        if (oi) minor -= oi.amount?.minor || 0;
        return { ...pa, amount: amount(minor) };
      });
    }
    if (Array.isArray(opex.children) || Array.isArray(otherIncome?.children)) {
      const foldedChildren = [...(opex.children || [])];
      for (const id of SLIM_PL_FOLD_INTO_OPEX) {
        const line = byId.get(id);
        if (line?.children?.length) foldedChildren.push(...line.children);
      }
      opex.children = foldedChildren;
    }
  }
  envelope.lines = (envelope.lines || []).filter((l) => SLIM_PL_KEEP.has(l.lineId));
  // Gross margin synthetic row for UI
  const gp = envelope.lines.find((l) => l.lineId === 'gross-profit');
  const rev = envelope.lines.find((l) => l.lineId === 'revenue');
  if (gp && rev && !envelope.lines.some((l) => l.lineId === 'gross-margin')) {
    const revM = rev.currentAmount?.minor ?? 0;
    const gpM = gp.currentAmount?.minor ?? 0;
    const pct = revM !== 0 ? Number(((gpM / revM) * 100).toFixed(2)) : null;
    const marginPeriodAmounts = (gp.periodAmounts || []).map((pa, idx) => {
      const rPa = rev.periodAmounts?.[idx];
      const rMin = rPa?.amount?.minor ?? 0;
      const gMin = pa?.amount?.minor ?? 0;
      const p = rMin !== 0 ? Number(((gMin / rMin) * 100).toFixed(2)) : null;
      return {
        key: pa.key,
        amount: { minor: p == null ? 0 : Math.round(p * 100), decimal: p == null ? null : String(p) },
        isPercent: true,
        percent: p,
      };
    });
    const marginLine = {
      ...
      label: 'Gross Margin',
      lineType: 'RATIO',
      displayOrder: (gp.displayOrder ?? 30) + 0.5,
      currentAmount: { minor: pct == null ? 0 : Math.round(pct * 100), decimal: pct == null ? null : String(pct) },
      isPercent: true,
      percent: pct,
      periodAmounts: marginPeriodAmounts,
      children: [],
      accounts: [],
      accountIds: [],
    };
    const gpIdx = envelope.lines.findIndex((l) => l.lineId === 'gross-profit');
    envelope.lines.splice(gpIdx + 1, 0, marginLine);
  }
  envelope.presentation = 'slim';
  return envelope;
}

export { slimIncomeStatementEnvelope };
...
/**
 * Build IS line minors from a precomputed ledger-like accounts array
 * (accrual summary rows or cash-basis synthetic rows).
 */
export function buildIncomeStatementBodyFromAccounts(
  definition,
  accounts,
  includeZero,
  { sourceTypeByAccount = null, summary = { anomalies: [] } } = {}
) {
  ...
  for (const defLine of definition.lines) {
    if (defLine.lineType === 'ACCOUNT_GROUP') {
      ...
      for (const r of rows) {
        ...
        if (sourceTypeByAccount) {
          const perSrc = sourceTypeByAccount.get(r.accountId);
          if (perSrc) {
            for (const [src, t] of perSrc) {
              const srcMinor = sign * (t.debitMinor - t.creditMinor);
              srcMap.set(src, (srcMap.get(src) || 0) + srcMinor);
            }
          }
        }
      }
      ...
      if (srcMap.size) sourceTypeTotals.set(defLine.lineId, srcMap);
    }
  }
  for (const defLine of definition.lines) {
    if (defLine.formula) {
...
export async function buildIncomeStatementBody(db, context, scope, definition, includeZero) {
  ...
  return buildIncomeStatementBodyFromAccounts(definition, summary.accounts, includeZero, { summary });
}
...
export async function generateIncomeStatement(db, context, request) {
  if (request.groupBy === 'MONTH' || request.groupBy === 'QUARTER') {
    const { generatePeriodizedIncomeStatement } = await import('./incomeStatementPeriodService.js');
    return generatePeriodizedIncomeStatement(db, context, request);
  }
  ...
  let current;
  if (reportBasis === 'CASH') {
    const { buildCashBasisAccountMovements } = await import('./incomeStatementCashBasis.js');
    const cash = await buildCashBasisAccountMovements(db, context, request);
    current = buildIncomeStatementBodyFromAccounts(definition, cash.accounts, request.includeZeroBalances, {
      sourceTypeByAccount: cash.sourceTypeByAccount,
    });
  } else {
    current = await buildIncomeStatementBody(db, context, request, definition, request.includeZeroBalances);
  }

  let citMeta = null;
  // Always evaluate CIT when tax is enabled: rate × period NPBT (includes pre-enable activity).
  // Posting to the ledger only when applyCitProvision is true (Accrual).
  try {
    const {
    ...
    const npbtMinor = current.lineMinors.get('profit-before-tax') ?? 0;
    ...
    if (reportBasis === 'ACCRUAL' && (citMeta?.posted || citMeta?.reason === 'UNCHANGED')) {
      current = await buildIncomeStatementBody(db, context, request, definition, request.includeZeroBalances);
    }
    // Always surface calculated CIT on the statement for the selected period.
    if (shouldShowCalculatedCit(citMeta)) {
      applyCitDisplayToBody(current, citMeta.citMinor);
    }
  } catch (err) {
    citMeta = {
      posted: false,
  ...
  const comparative =
    request.comparison && reportBasis !== 'CASH'
  ...
  if (reportBasis === 'ACCRUAL' && !citMeta?.posted && netProfitMinor !== expectedNetProfit) {
  ...
  const envelope = buildReportEnvelope(context, request, definition, {
    ...
    totals: {
      ...
      taxExpense: amount(current.lineMinors.get('tax-expense') ?? 0),
  ...
  envelope.meta = {
    ...
    groupBy: null,
    breakdown: request.breakdown === 'SOURCE_TYPE' ? 'SOURCE_TYPE' : 'ACCOUNT',
    methodLabel:
      reportBasis === 'CASH'
        ? `Income Collected (${request.currency || context.baseCurrency || 'MWK'})`
        : `Income Billed (${request.currency || context.baseCurrency || 'MWK'})`,
    cit: citMeta,
  };
  ...
  if (citMeta?.warning) {
    finalized.warnings = [...(finalized.warnings || []), citMeta.warning];
  }
