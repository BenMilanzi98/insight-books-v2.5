...
export function normalizeReportRequest(context, reportType, raw = {}) {
  ...
  return Object.freeze({
    ...
    reportBasis:
      String(raw.reportBasis || raw.accountingMethod || 'ACCRUAL').toUpperCase() === 'CASH'
        ? 'CASH'
        : 'ACCRUAL',
    groupBy: (() => {
      const g = String(raw.groupBy || '').toUpperCase();
      return g === 'MONTH' || g === 'QUARTER' ? g : null;
    })(),
    breakdown:
      String(raw.breakdown || '').toUpperCase() === 'SOURCE_TYPE' ? 'SOURCE_TYPE' : 'ACCOUNT',
    applyCitProvision:
      raw.applyCitProvision === true ||
      raw.applyCitProvision === 'true' ||
      raw.applyCitProvision === '1',
...
export function hashReportRequest(request) {
  const scope = {
    ...
    reportBasis: request.reportBasis ?? 'ACCRUAL',
    groupBy: request.groupBy ?? null,
    breakdown: request.breakdown ?? 'ACCOUNT',
