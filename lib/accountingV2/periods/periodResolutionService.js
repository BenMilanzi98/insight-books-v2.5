...
export async function resolvePeriodV2(db, context, params) {
  ...
  let years = await db.acctV2FinancialYear.findMany({
    where: {
      tenantId: context.businessId,
  ...
  if (years.length === 0) {
    // Auto-provision OPEN financial year (default Jan–Dec) so posting works for
    // new/existing tenants that never created a calendar year manually.
    try {
      const { ensureOpenFinancialYearForDate } = await import('./ensureFinancialYear.js');
      await ensureOpenFinancialYearForDate(db, {
        tenantId: context.businessId,
        userId: context.userId,
    ...
    years = await db.acctV2FinancialYear.findMany({
      where: {
        tenantId: context.businessId,
        startDate: { lte: postingDateValue },
        endDate: { gte: postingDateValue },
  ...
  if (fy.status === FinancialYearStatus.DRAFT) {
    try {
      const { openFinancialYear } = await import('./financialYearService.js');
      await openFinancialYear(db, context, fy.id);
      const opened = await db.acctV2FinancialYear.findFirst({
        where: { id: fy.id, tenantId: context.businessId },
      });
      if (opened) Object.assign(fy, opened);
    } catch (openErr) {
      throw new InvalidAccountingPeriodError(
        openErr?.message || `Financial year ${fy.code} has not been opened yet.`,
        { ...ids(context), diagnostic: { financialYearId: fy.id } }
      );
    }
  }
  ...
  if (period.status === AccountingPeriodStatus.DRAFT) {
    try {
      await db.acctV2AccountingPeriod.update({
        where: { id: period.id },
