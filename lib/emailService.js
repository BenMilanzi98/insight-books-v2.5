...
import { emailSubject } from '@/lib/i18n/documentTranslate';

// Email templates
const templates = {
  'welcome-email': (data) => ({
    subject:
      emailSubject('welcomeSubject', { locale: data.locale || 'en' }) ||
      `Welcome to ${process.env.COMPANY_NAME || 'Insight Books'}`,
    html: `
...
export async function verifySmtpConnectionOptional(transporter, context = 'email') {
  try {
...
/** From address must match the authenticated SMTP mailbox. */
export function getSmtpFromAddress(displayName = 'InsightBooks') {
  const raw = (process.env.EMAIL_FROM || process.env.EMAIL_USER || '').trim();
  if (!raw) return `"${displayName}" <noreply@example.com>`;
...
export const sendEmail = async ({
  ...
  try {
    ...
    await verifySmtpConnectionOptional(transporter, 'sendEmail');
...
export const sendQuotationEmail = async (quotation, tenant, pdfBuffer, options = {}) => {
  ...
  try {
    ...
    await verifySmtpConnectionOptional(transporter, 'quotation email');
