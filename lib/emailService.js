...
const templates = {
  ...
  'rich-email': (data) => {
    ...
    // Always use /logo.png from public folder
...
export const sendEmail = async ({ to, subject, template, data, from, attachments = [], htmlContent = null }) => {
  try {
    ...
    // Provide more specific error messages and fallback for demo requests
    if (error.code === 'EAUTH') {
      console.error('SMTP Authentication failed. Please check your email credentials.');

      // For demo requests and other non-critical emails, simulate success
      console.log('⚠️ Email sending failed due to authentication, but continuing with process...');
