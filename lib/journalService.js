...
import { validateLineAccountsAllowDirectPosting } from './coaDirectPostingEligibility.js';
...
async function ensureAccountsAccessible(tx, tenantId, lines) {
  ...
  const posting = validateLineAccountsAllowDirectPosting(accounts);
  if (!posting.ok) {
    throw new Error(posting.details ? `${posting.error} (${posting.details})` : posting.error);
  }
...
export async function postEntry(entryId, context = {}) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    // Legacy↔V2 guard: V2-managed drafts post only through the V2 engine, and
    // the legacy path refuses events the V2 engine owns or has already posted.
    if (entry.architectureVersion === 'ACCOUNTING_V2') {
      throw new Error(
        'This journal is managed by the Accounting V2 posting engine and cannot be posted through the legacy path.'
      );
    }
    await assertLegacyPostingAllowed(
      { tenantId: entry.tenantId, sourceType: 'JournalEntry', sourceId: entry.id },
      tx
    );
    ...
    const lineIds = [...new Set(entry.lines.map((l) => l.accountId))];
    const postingAccounts = await tx.account.findMany({
      where: { id: { in: lineIds }, tenantId: entry.tenantId },
      select: {
        id: true,
        accountCode: true,
        code: true,
    ...
    if (postingAccounts.length !== lineIds.length) {
      throw new Error('One or more journal lines reference an unknown account.');
    }
    const postingCheck = validateLineAccountsAllowDirectPosting(postingAccounts);
    if (!postingCheck.ok) {
      throw new Error(
        postingCheck.details
          ? `${postingCheck.error} (${postingCheck.details})`
          : postingCheck.error,
      );
    }
    ...
    // Fresh-books V2: Account.balance is not financial SoT — do not mutate via
    // updateAccountBalanceOnTransaction. JournalEntry lines are authoritative.
...
export async function voidEntry(entryId, reason, context = {}) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    // V2-managed journals follow the V2 lifecycle (immutable once posted;
    // pre-posted rows are cancelled through the V2 service, never voided here).
    if (entry.architectureVersion === 'ACCOUNTING_V2') {
      throw new Error(
        'This journal is managed by the Accounting V2 posting engine and cannot be voided through the legacy path.'
      );
    }

    // Posted entries must be reversed (creates offsetting GL), not voided in place.
    if (entry.status === 'Posted') {
      return createReversalEntry(entryId, reason, context);
    }
...
export async function createReversalEntry(entryId, reason, context = {}) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    // V2-posted journals are reversed through a V2 REVERSAL_POSTED accounting
    // event so the reversal itself carries an accounting identity.
    if (entry.architectureVersion === 'ACCOUNTING_V2') {
      throw new Error(
        'This journal was posted by the Accounting V2 engine. Reverse it through the V2 reversal workflow.'
      );
    }
