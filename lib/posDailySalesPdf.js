...
export function generatePosDailySalesPdfBuffer(report, cashState, opts = {}) {
  ...
  const body =
    txs.length === 0
      ...
      : txs.map((tx) => [
          String(tx.saleNumber ?? ''),
          fmtUtcDateTime(tx.saleDate),
          fmtMoney(tx.total ?? 0, currency),
          String(tx.primaryPaymentLabel || ''),
        ]);

  autoTable(doc, {
    startY: y,
    head: [['Sale #', 'Time (UTC)', 'Total', 'Payment detail']],
    body,
    theme: 'striped',
    headStyles: {
      fillColor: ACCENT,
