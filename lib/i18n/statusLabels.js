...
const STATUS_KEY_MAP = {
  ...
  PENDINGAPPROVAL: 'common.status.pendingApproval',
  ...
  PARTIALLYPAID: 'common.status.partiallyPaid',
  PARTIAL: 'common.status.partiallyPaid',
  ...
  VOID: 'common.status.cancelled',
  REFUNDED: 'common.status.reversed',
  EXPIRED: 'common.status.failed',
};

export function translateStatus(status, { locale, messages, englishMessages } = {}) {
  const raw = String(status || '');
  const code = raw.toUpperCase().replace(/[\s-]+/g, '_');
  const key = STATUS_KEY_MAP[code] || STATUS_KEY_MAP[code.replace(/_/g, '')];
  if (!key) return raw;
