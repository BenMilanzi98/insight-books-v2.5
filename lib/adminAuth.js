...
import { adminHasPermission as checkAdminPermission } from '@/lib/admin/permissions';
export { requireAdminDecision } from '@/lib/admin/authorization/requireAdminDecision';
export { authorizeAdminDecision } from '@/lib/admin/authorization/authorizeAdminDecision';
 ...
 * @param {string} permission - "systemAdmin.*.*" or legacy "category.action"
 * @returns {boolean} Whether the admin has the permission
 */
export function adminHasPermission(admin, permission) {
  return checkAdminPermission(admin, permission);
}
