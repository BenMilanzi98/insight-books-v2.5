...
/** True when this tax type is Corporate Income Tax (MW-CIT). */
export function isCorporateIncomeTaxType(taxType) {
  ...
  return id === 'MW-CIT' || code === 'MW-CIT' || name.includes('corporate income tax');
}
...
export function isCanonicalOwnerOfLinkedTaxAccount(taxType, allTaxTypes = []) {
  ...
  if (catalogByGl) {
    const id = String(taxType.taxId || '').toUpperCase();
    const code = String(taxType.taxCode || '').toUpperCase();
    const catalogId = String(catalogByGl.taxId || '').toUpperCase();
    const catalogCode = String(catalogByGl.taxCode || '').toUpperCase();
    return id === catalogId || (catalogCode && code === catalogCode);
  }
