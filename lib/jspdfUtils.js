...
function createBasicPDF(doc, invoice, branding, colorRGB, style, showFooter) {
  ...
  doc.text('Price', x + 130, y + 7);
  doc.text('Amount', x + contentWidth - 15, y + 7, { align: 'right' });
  ...
  items.forEach(item => {
    ...
    doc.text(formatCurrency(item.unitPrice, '', false), x + 130, y);
    doc.text(formatCurrency(item.quantity * item.unitPrice, '', false), 
  ...
  doc.text(formatCurrency(invoice.subtotal), x + contentWidth - 15, y, { align: 'right' });
  y += 8;
  
  doc.text('Tax:', x + contentWidth - 70, y);
  doc.text(formatCurrency(invoice.taxAmount), x + contentWidth - 15, y, { align: 'right' });
  y += 8;
  
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(colorRGB.r, colorRGB.g, colorRGB.b);
  doc.text('Total:', x + contentWidth - 70, y);
  doc.text(formatCurrency(invoice.total), x + contentWidth - 15, y, { align: 'right' });
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(invoice.notes || 'Thank you for your business!', x, y + 6);
  
  // Footer
  if (showFooter) {
    doc.setTextColor(120, 120, 120);
    doc.setFontSize(8);
    const footerText = branding?.emailFooter || 
      `${branding?.companyName || 'Your Company'} | ${branding?.email || ''} | ${branding?.phone || ''}`;
