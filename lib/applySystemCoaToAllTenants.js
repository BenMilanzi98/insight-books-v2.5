...
export async function applySystemCoaPayloadToTenant(db, tenantId, payload) {
  ...
  for (const row of sorted) {
    // Never provision retired codes (e.g. 1120 Petty Cash).
    if (isRetiredGlCode(row.code)) {
      const targetCode = retiredMergeTargetCode(row.code);
      const targetId = targetCode ? codeToId.get(targetCode) : null;
      const existingRetired = await db.account.findFirst({
        where: { tenantId, accountCode: row.code },
        select: { id: true },
      });
      if (existingRetired && targetId) {
        await db.account.update({
          where: { id: existingRetired.id },
          data: {
            isActive: false,
    ...
    const baseData = {
      ...
      isPaymentGlParentCode(row.code) ||
      row.code === '1130'
        ? { acceptsNewTransactions: false }
        : {}),
    };
