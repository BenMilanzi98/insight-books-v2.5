/**
 * Opening balance posting — legacy Transaction path removed (fresh-books V2-only).
 * Use `/api/accounting-v2/opening-balances` (AcctV2OpeningBalanceBatch) instead.
 */
...
export async function postOpeningBalance(params) {
  ...
  throw new Error(
    'Legacy opening-balance posting is removed. Use the V2 opening-balance API (/api/accounting-v2/opening-balances). Fresh-books cutover starts at zero with no OB seed.'
  );
