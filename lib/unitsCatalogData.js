/**
 * Canonical global unit catalog (BaseUnit has no tenantId — shared by all tenants).
 * Keep seed-units and runtime sync aligned with this single source.
 */
export const UNITS_CATALOG = [
  ...
  },
  {
    ...
    units: [
      ...
      { name: 'Set', symbol: 'set', conversionToBase: 1 },
      { name: 'Pack', symbol: 'pack', conversionToBase: 1 },
      { name: 'Box', symbol: 'box', conversionToBase: 1 },
      { name: 'Bundle', symbol: 'bundle', conversionToBase: 1 },
    ],
  },
  {
    name: 'energy',
  ...
  },
  {
    ...
    units: [
      ...
      { name: 'Fahrenheit', symbol: '°F', conversionToBase: 1 },
      { name: 'Kelvin', symbol: 'K', conversionToBase: 1 },
    ],
  },
  {
    name: 'time',
  ...
  },
  {
    ...
    baseUnit: 'unit',
    units: [{ name: 'Unit', symbol: 'unit', conversionToBase: 1, isBaseUnit: true }],
  },
];

export const EXPECTED_CATALOG_BASE_COUNT = UNITS_CATALOG.length;
export const EXPECTED_CATALOG_UNIT_COUNT = UNITS_CATALOG.reduce(
  (n, b) => n + b.units.length,
  0
);

/** Distinct symbols shipped with the global catalog (sync may create/update only these when isCatalogUnit). */
export const CATALOG_UNIT_SYMBOLS = UNITS_CATALOG.flatMap((b) => b.units.map((u) => u.symbol));
