...
import { resolvePaymentAccount } from './paymentMethodAccountMapping';

async function getAccounts(tenantId, tx = prisma) {
  // Try to find inventory account by code
...
async function getPaymentAccount(tenantId, paymentMethod, tx = prisma) {
  ...
  try {
    return await resolvePaymentAccount(tenantId, paymentMethod, tx);
  } catch (error) {
    console.error('Error resolving payment account:', error);
    // Fallback to old method if resolvePaymentAccount fails
