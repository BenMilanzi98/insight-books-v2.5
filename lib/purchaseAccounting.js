...
async function getPaymentAccount(tenantId, paymentMethod, tx = prisma) {
  ...
  try {
    // Use the centralized mapping implementation
    return await getAccountForPaymentMethod(tenantId, paymentMethod, tx);
  } catch (error) {
    console.error('Error resolving payment account via mapping:', error);
    // Fallback to legacy mapping if centralized mapping fails
