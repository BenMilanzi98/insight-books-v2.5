 ...
 * - Open monthly accounting period for the current month
 *
...
export async function initializeNewTenantFinancialDefaults(tenantId, tx) {
  if (!tenantId || !tx) {
  ...
  try {
    ...
    await ensureChartOfAccountsForTenant(tenantId, tx);
  } catch (coaErr) {
