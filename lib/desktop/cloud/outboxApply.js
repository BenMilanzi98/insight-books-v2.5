...
import { findPrismaCode, getErrorChain } from '../../prismaErrorChain.js';

/** @param {unknown} error */
export function isPrismaUniqueConflict(error) {
  return findPrismaCode(getErrorChain(error)) === 'P2002';
}
...
export async function applyDesktopOutboxItem({
  ...
  try {
    ...
    if (isPrismaUniqueConflict(error)) {
      const raced = await prisma.desktopOutboxReceipt.findUnique({
        where: { tenantId_id: { tenantId, id: item.id } },
      });
      if (raced) {
        return {
          serverId: raced.serverEntityId ?? raced.resultJson?.serverId ?? null,
          result: raced.resultJson ?? null,
          duplicate: true,
        };
      }
    }
...
/**
 * Apply outbox items in order; stop on the first failure.
 *
 * @returns {Promise<{ results: object[], stoppedAt: string | null, error: object | null }>}
 */
export async function applyDesktopOutboxBatch({
  prisma,
  tenantId,
  user,
  deviceId,
  items,
  request,
  applyItem = applyDesktopOutboxItem,
}) {
  const results = [];
