...
import { appendOutbox, listSyncIssues } from '../sqlite/outboxStore.js';
import { assertWritable, touchLocalNow } from './writeGate.js';
import { syncStatusFromDb, runDesktopSync } from '../syncWorker.js';
import { createCloudClient } from '../cloud/createCloudClient.js';
...
function assertTillOpenForLocalSale(db) {
  ...
  if (!cashDay || cashDay.status !== 'OPEN') {
    return {
      status: 409,
      json: {
        error: 'POS till is not open for today. Open the till before making sales.',
        code: 'TILL_NOT_OPEN',
      },
    };
  }
...
function getSaleLocal(db, saleId) {
  const row = db.prepare('SELECT payload FROM sales WHERE id = ?').get(saleId);
  return row ? JSON.parse(row.payload) : null;
}

function restoreSaleItemStock(db, items) {
...
function createSaleLocal({ db, body, now, user }) {
  ...
  const tillError = assertTillOpenForLocalSale(db);
  if (tillError) return tillError;
...
function voidSaleLocal({ db, saleId, body, now, user }) {
  assertWritable(db, now);

  const reason = body?.reason?.trim();
  if (!reason) {
    return { status: 400, json: { error: 'Void reason is required' } };
  }

  const existing = getSaleLocal(db, saleId);
  if (!existing) {
    return { status: 404, json: { error: 'Sale not found' } };
  }
  if (existing.status !== 'completed') {
    return { status: 400, json: { error: 'Only completed sales can be voided' } };
  }
  ...
  const tx = db.transaction(() => {
    restoreSaleItemStock(db, existing.items);
    db.prepare('UPDATE sales SET payload = ? WHERE id = ?').run(JSON.stringify(sale), saleId);
    appendOutbox(db, {
      id: randomUUID(),
      kind: 'pos.void',
  ...
  return {
    ...
    json: {
      success: true,
      message: `Sale ${sale.saleNumber} has been voided successfully`,
      sale: {
        id: sale.id,
        saleNumber: sale.saleNumber,
        status: sale.status,
...
function refundSaleLocal({ db, saleId, body, now, user }) {
  assertWritable(db, now);

  const reason = body?.reason?.trim();
  if (!reason) {
    return { status: 400, json: { error: 'Refund reason is required' } };
  }
  ...
  if (existing.status !== 'completed') {
    return { status: 400, json: { error: 'Only completed sales can be refunded' } };
  }

  const refundedAt = new Date(now).toISOString();
  const sale = {
    ...existing,
    status: 'refunded',
    refundedAt,
  ...
  return {
    ...
    json: {
      success: true,
      message: `Sale ${sale.saleNumber} has been refunded successfully`,
      sale: {
        id: sale.id,
        saleNumber: sale.saleNumber,
        status: sale.status,
...
function getInvoiceLocal(db, invoiceId) {
  const row = db.prepare('SELECT payload FROM invoices WHERE id = ?').get(invoiceId);
  return row ? JSON.parse(row.payload) : null;
}

function updateInvoiceLocal({ db, invoiceId, body, now, user }) {
  assertWritable(db, now);

  const existing = getInvoiceLocal(db, invoiceId);
  if (!existing) {
    return { status: 404, json: { error: 'Invoice not found' } };
  }

  if (existing.status !== 'Draft' && existing.status !== 'Pending') {
    return { status: 400, json: { error: 'Cannot edit invoices that are Paid or Overdue' } };
  }
  ...
  const invoice = {
    ...
    taxAmount: Number(body.taxAmount ?? existing.taxAmount ?? 0),
    total: Number(body.total ?? body.subtotal ?? existing.total ?? 0),
    totalDiscountAmount: Number(body.totalDiscountAmount ?? existing.totalDiscountAmount ?? 0),
    discount: body.discount ?? existing.discount ?? 0,
  ...
  const tx = db.transaction(() => {
    db.prepare('UPDATE invoices SET payload = ? WHERE id = ?').run(
      JSON.stringify(invoice),
      invoiceId
    );
    appendOutbox(db, {
...
function voidInvoiceLocal({ db, body, now, user }) {
  assertWritable(db, now);

  const invoiceId = body?.id ?? body?.invoiceId;
  if (!invoiceId) {
    return { status: 400, json: { success: false, error: 'Invoice ID is required' } };
  }

  const existing = getInvoiceLocal(db, invoiceId);
  if (!existing) {
    return { status: 404, json: { success: false, error: 'Invoice not found' } };
  }

  if (existing.status === 'void') {
    return { status: 400, json: { success: false, error: 'Invoice is already voided' } };
  }
  ...
  if (totalPaid > 0) {
    return {
      status: 400,
      json: { success: false, error: 'Cannot void invoice with payments. Process refund instead.' },
    };
  }
  ...
  const tx = db.transaction(() => {
    ...
    appendOutbox(db, {
      id: randomUUID(),
      kind: 'invoice.void',
      payload: { id: invoiceId, invoiceId, reason: body.reason ?? null },
    });
    touchLocalNow(db, now);
  });
  tx();
...
function partialPaymentInvoiceLocal({ db, body, now, user }) {
  assertWritable(db, now);

  const invoiceId = body?.invoiceId;
  const amount = Number(body?.amount);
  if (!invoiceId) {
    return { status: 400, json: { error: 'Invoice ID is required' } };
  }
  if (!amount || Number.isNaN(amount) || amount <= 0) {
    return { status: 400, json: { error: 'Payment amount must be a valid number greater than 0' } };
  }
  if (!body?.paymentMethod) {
    return { status: 400, json: { error: 'Payment method is required' } };
  }
  ...
  if (['void', 'refunded', 'partially_refunded', 'paid'].includes(statusLower)) {
    return {
      status: 400,
      json: { error: 'Cannot process payment for voided, refunded, or fully paid invoice' },
    };
  }

  const payments = [...(existing.payments || [])];
  const totalPaid = payments.reduce((sum, payment) => sum + (Number(payment.amount) || 0), 0);
  const invTotal = Number(existing.total) || 0;
  const remainingBalance = invTotal - totalPaid;

  if (amount > remainingBalance + 0.005) {
    return {
      status: 400,
      json: { error: `Payment amount exceeds remaining balance of ${remainingBalance}` },
    };
  }

  const paymentId = randomUUID();
  const createdAt = new Date(now).toISOString();
  const paymentDate = body.paymentDate ?? createdAt.split('T')[0];
  const payment = {
    id: paymentId,
    amount,
  ...
  const newRemainingBalance = Math.max(0, invTotal - newTotalPaid);
  ...
  const tx = db.transaction(() => {
    ...
    appendOutbox(db, {
      ...
      payload: { ...body, id: paymentId, invoiceId },
    });
    touchLocalNow(db, now);
  });
  tx();
...
function isImplementedOperationalRoute(method, pathname) {
  ...
  if (method === 'POST' && /^\/api\/sales\/[^/]+\/void$/.test(pathname)) return true;
  if (method === 'POST' && /^\/api\/sales\/[^/]+\/refund$/.test(pathname)) return true;
  ...
  if (method === 'PUT' && /^\/api\/invoices\/[^/]+$/.test(pathname)) return true;
  if (method === 'POST' && pathname === '/api/invoices/void') return true;
  if (method === 'POST' && pathname === '/api/invoices/partial-payment') return true;
...
function getSyncStatusLocal({ db, now }) {
  const status = syncStatusFromDb(db, now);
  const issues = listSyncIssues(db).map(({ id, kind, errorMessage }) => ({
    id,
    kind,
    errorMessage,
  }));
  return { status: 200, json: { ...status, issues } };
}

async function syncNowLocal({ db, now, cloud, requestCookie }) {
  const client =
    cloud ||
    (process.env.DESKTOP_CLOUD_URL
      ? createCloudClient({
          baseUrl: process.env.DESKTOP_CLOUD_URL,
          cookie: requestCookie,
        })
      : null);

  if (!client) {
    return { status: 503, json: { ok: false, error: 'Cloud URL not configured' } };
  }

  try {
    const result = await runDesktopSync({ db, cloud: client, now });
    const status = syncStatusFromDb(db, now);
    return { status: 200, json: { ...result, ...status } };
  } catch (err) {
    return {
      status: err?.status || 500,
      json: { ok: false, error: err?.message || 'Sync failed' },
    };
  }
}

export function handleDesktopLocal({
  db,
  method,
  pathname,
  searchParams = {},
  ...
  if (method === 'GET' && pathname === '/api/sync-status') {
    return getSyncStatusLocal({ db, now });
  }

  if (method === 'POST' && pathname === '/api/sync-now') {
    return syncNowLocal({ db, now, cloud, requestCookie });
  }
  ...
  try {
    ...
    if (method === 'POST' && /^\/api\/sales\/[^/]+\/void$/.test(pathname)) {
      return voidSaleLocal({
        db,
        saleId: extractPathId(pathname.replace(/\/void$/, ''), '/api/sales'),
        body,
        now,
        user,
      });
    }
    if (method === 'POST' && /^\/api\/sales\/[^/]+\/refund$/.test(pathname)) {
      return refundSaleLocal({
        db,
        saleId: extractPathId(pathname.replace(/\/refund$/, ''), '/api/sales'),
        body,
        now,
        user,
      });
    ...
    if (method === 'PUT' && /^\/api\/invoices\/[^/]+$/.test(pathname)) {
      return updateInvoiceLocal({
        db,
        invoiceId: extractPathId(pathname, '/api/invoices'),
        body,
        now,
        user,
      });
    }
    if (method === 'POST' && pathname === '/api/invoices/void') {
      return voidInvoiceLocal({ db, body, now, user });
    }
    if (method === 'POST' && pathname === '/api/invoices/partial-payment') {
      return partialPaymentInvoiceLocal({ db, body, now, user });
    }
