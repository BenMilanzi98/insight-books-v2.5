...
`;function k(a,b,c){let k="string"==typeof b?.content?JSON.parse(b.content):b?.content||{},l=k.primaryColor||c?.primaryColor||"#0075be",m=!1!==k.showFooter,n=c?.tpin&&String(c.tpin).trim()||"",o=a?.footerPhoneOverride!=null&&a?.footerPhoneOverride!==""?a.footerPhoneOverride:c?.businessPhone||"",p=a?.footerBankDetailsOverride!=null&&a?.footerBankDetailsOverride!==""?a.footerBankDetailsOverride:c?.defaultBankDetails||"",q=(a.items||[]).map(a=>{let b=parseFloat(a.quantity)||0,c=parseFloat(a.unitPrice)||0,d=parseFloat(a.discountAmount)||0,e=parseFloat(a.taxRate)||0,f=b*c-d,g=e/100*f;return{...a,qty:b,rate:c,disc:d,taxRate:e,taxAmt:g,net:f,amount:f+g}}),r=(0,e.Vr)(q),s=(0,e.ME)({taxAmount:a.taxAmount,taxLines:q.flatMap(a=>a.itemTaxes||[])}),t=q.map((a,b)=>`
    <tr style="background:${b%2==1?"#f9fafb":"#fff"}">
      ...
      ${r?`<td style="padding:10px 8px;text-align:right;color:#4b5563">${a.taxRate>0?`${a.taxRate}%`:"—"}</td>`:""}
      <td style="padding:10px 8px;text-align:right;font-weight:500;color:#111827">${h(a.amount)}</td>
    </tr>`).join(""),u=(a.payments||[]).filter(a=>a&&!a.isReversal&&(null==a.status||"Completed"===String(a.status))),v=u.reduce((a,b)=>a+(parseFloat(b.amount)||0),0),w=Math.max(0,(parseFloat(a.total)||0)-v);return`<!DOCTYPE html><html><head><meta charset="utf-8"><style>${j}</style></head><body>
<div class="page" style="padding:28px">
  ...
  <div style="border-left:4px solid ${l};padding:24px 24px 16px">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;flex-wrap:wrap">
      <div>
        ...
        ${n?`<p style="margin-top:8px;font-size:12px;color:#4b5563"><b>TPIN:</b> ${f(n)}</p>`:""}
      </div>
      <div style="text-align:right">
        <p style="font-size:24px;font-weight:700;color:#111827">Invoice</p>
        <p style="font-size:14px;color:#6b7280;margin-top:2px">#${f(a.invoiceNumber)}</p>
  ...
  <div style="padding:20px 24px">
    ...
    <table style="font-size:14px">
      <thead>
        <tr style="border-bottom:2px solid #e5e7eb">
          ...
          ${r?'<th style="text-align:right;padding:12px 8px;font-weight:600;color:#374151">Tax</th>':""}
    ...
    <div style="display:flex;justify-content:flex-end;margin-top:24px">
      <div style="width:256px;border:1px solid #e5e7eb;border-radius:8px;background:#f9fafb;padding:16px;font-size:14px">
        ...
        ${s?`<div style="display:flex;justify-content:space-between;padding:6px 0;color:#4b5563"><span>Tax</span><span style="font-weight:500;color:#111827">${g(a.taxAmount)}</span></div>`:""}
        ...
        <div style="margin-top:12px;padding-top:12px;border-top:1px solid #e5e7eb">
          <div style="display:flex;justify-content:space-between;padding:4px 0;color:#4b5563"><span>Paid</span><span style="font-weight:500;color:#059669">${g(v)}</span></div>
          ${w>0?`<div style="display:flex;justify-content:space-between;padding:4px 0;color:#4b5563"><span>Outstanding</span><span style="font-weight:500;color:#dc2626">${g(w)}</span></div>`:""}
        </div>`:""}
      </div>
    </div>
    ${u.length>0?`
    <div style="margin-top:20px;padding:16px;border:1px solid #e5e7eb;border-radius:8px;background:#f8fafc">
      ...
      <table style="width:100%;font-size:13px;border-collapse:collapse">
        ...
        <tbody>
          ${u.map(a=>`<tr style="border-bottom:1px solid #f1f5f9">
  ...
  <footer style="padding:24px;border-top:1px solid #e5e7eb;background:#f9fafb;font-size:14px">
    ${o||p?`<div style="color:#6b7280;margin-bottom:16px">
      ${o?`<p>Tel: ${f(o)}</p>`:""}
      ${p?`<pre style="white-space:pre-wrap;font-family:inherit;margin:0">${f(p)}</pre>`:""}
    </div>`:""}
