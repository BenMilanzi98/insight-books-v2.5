...
import { SETUP_WIZARD_GRADIENT_H } from "@/lib/setupWizard/wizardTheme";
...
function StatusPill({ status }) {
  if (status === "complete") {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full bg-indigo-500/15 px-2.5 py-1 text-xs font-semibold text-indigo-700 ring-1 ring-indigo-500/25">
...
export default function SetupWizard({ embedded = false, onClose, initialStepId = null }) {
  ...
  if (payload.allComplete) {
    return (
      <div className="mx-auto max-w-2xl text-center">
        <div className="relative overflow-hidden rounded-3xl border border-indigo-200/60 bg-gradient-to-br from-indigo-50 via-white to-violet-50 p-10 shadow-2xl shadow-indigo-900/10">
          <div className="pointer-events-none absolute -right-8 -top-8 h-40 w-40 rounded-full bg-violet-400/20 blur-3xl" />
          <div className="pointer-events-none absolute -bottom-10 -left-10 h-48 w-48 rounded-full bg-indigo-400/20 blur-3xl" />
          <div className="relative">
            <div className={`mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-2xl ${SETUP_WIZARD_GRADIENT_H} text-white shadow-lg shadow-indigo-500/30`}>
            ...
            {embedded ? (
              <button
                ...
                className={`mt-8 inline-flex items-center gap-2 rounded-xl ${SETUP_WIZARD_GRADIENT_H} px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-500/25 transition hover:brightness-105`}
              >
                {tt('Back to dashboard')}
  ...
  return (
    <div className={embedded ? "" : "mx-auto max-w-6xl"}>
      {/* Top bar */}
      <div className={`flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between ${embedded ? tt('mb-5') : tt('mb-8')}`}>
        <div>
        ...
        <div className="flex shrink-0 flex-col items-end gap-2">
          ...
          <div className="h-2.5 w-48 overflow-hidden rounded-full bg-slate-200/80 sm:w-56">
            <div
              className={`h-full rounded-full ${SETUP_WIZARD_GRADIENT_H} transition-all duration-500 ease-out`}
      ...
      <div className={`grid gap-6 ${embedded ? "lg:grid-cols-[240px_1fr]" : "lg:grid-cols-[280px_1fr]"}`}>
                ...
                <li key={step.id}>
                  <button
                    ...
                    className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-all ${
                      isActive
                        ? `${SETUP_WIZARD_GRADIENT_H} text-white shadow-md shadow-indigo-500/25`
                  ...
                  >
                    ...
                    >
                      {step.status === "complete" ? (
                        <CheckCircle2 className={`h-5 w-5 ${isActive ? "text-white" : "text-indigo-600"}`} />
                      ) : (
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-2xl shadow-slate-300/30">
          <div
            className={`relative bg-gradient-to-br ${meta.gradient} ${embedded ? tt('px-5 py-6 sm:px-8') : tt('px-6 py-10 sm:px-10 sm:py-12')}`}
          >
            ...
            {activeStep?.status === "pending" && (
              <div className="flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:flex-wrap">
                <button
                  ...
                  className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border-2 border-indigo-200 bg-indigo-50 px-5 py-3 text-sm font-semibold text-indigo-800 transition hover:bg-indigo-100 disabled:opacity-50 sm:flex-none"
                >
                  {busy === "complete" ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}
                  {activeIndex === steps.length - 1 ? tt('Complete Setup') : tt('Mark step as done')}
                </button>
                <button
                  type="button"
                  disabled={!!busy}
