...
import { CS_SECTIONS, isCsSectionActive } from '@/lib/admin/customerSuccessNav';
...
export default function CustomerSuccessSectionNav({ className }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Keep nav empty until admin is known — avoid linking gated sections.
      }
    })();
    return () => {
      cancelled = true;
  ...
  return (
    <nav
      ...
      aria-label={t('admin-pages.customerSuccess.sectionNavLabel')}
    >
      {CS_SECTIONS.map((section) => {
        const active = isCsSectionActive(pathname, section);
        ...
        if (!allowed) {
          return (
            <span
              key={section.id}
              aria-disabled="true"
              title={t('admin-pages.customerSuccess.gated')}
              className={cn(
