...
export default function AdminDataTable({
  ...
  return (
    <div className={cn('min-w-0', className)}>
      {/* Desktop */}
      <div className="hidden overflow-x-auto rounded-[var(--admin-radius)] border border-[var(--admin-border)] bg-[var(--admin-surface)] md:block">
        <table className="min-w-full text-left text-sm">
          <thead className="sticky top-0 z-[1] bg-[var(--admin-surface-muted)] text-xs font-semibold uppercase tracking-wide text-[var(--admin-text-muted)]">
            <tr>
              {columns.map((col) => (
                <th
                  key={col.key}
          ...
          <tbody>
            {rows.map((row, index) => (
              <tr
                ...
                style={{ minHeight: 'var(--admin-row-height)' }}
