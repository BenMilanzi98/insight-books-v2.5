...
import { SUPPORT_SECTIONS, isSupportSectionActive } from '@/lib/admin/supportNav';
...
export default function SupportSectionNav({ className }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Keep nav empty until admin is known
      }
    })();
    return () => {
      cancelled = true;
  ...
  return (
    <nav
      ...
      aria-label={t('admin-pages.support.sectionNavLabel')}
    >
      {SUPPORT_SECTIONS.map((section) => {
        const active = isSupportSectionActive(pathname, section);
        ...
        if (!allowed) {
          return (
            <span
              key={section.id}
              aria-disabled="true"
              title={t('admin-pages.support.gated')}
              className={cn(
