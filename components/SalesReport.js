        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 sm:gap-6 mb-8">
            <div className="bg-gradient-to-br from-emerald-50 to-white p-4 sm:p-5 rounded-2xl border border-emerald-200/80 shadow-sm border-l-4 border-l-emerald-500">
              <h3 className="text-sm font-medium text-emerald-700 mb-1">{tt('Total revenue')}</h3>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-white p-4 sm:p-5 rounded-2xl border border-blue-200/80 shadow-sm border-l-4 border-l-blue-500">
              <h3 className="text-sm font-medium text-blue-700 mb-1">{tt('Average sale value')}</h3>
            ...
            <div className="bg-gradient-to-br from-sky-50 to-white p-4 sm:p-5 rounded-2xl border border-sky-200/80 shadow-sm border-l-4 border-l-sky-500">
              <h3 className="text-sm font-medium text-sky-700 mb-1">{tt('Total tax collected')}</h3>
            ...
            <div className="bg-gradient-to-br from-amber-50 to-white p-4 sm:p-5 rounded-2xl border border-amber-200/80 shadow-sm border-l-4 border-l-amber-500">
              <h3 className="text-sm font-medium text-amber-700 mb-1">{tt('Active customers')}</h3>
              <p className="text-2xl font-semibold text-slate-800">{data.salesByCustomer?.length || 0}</p>
            </div>
          </div>

          <div className="mb-6">
            <h3 className="text-lg font-semibold text-slate-800 mb-4 border-l-4 border-emerald-500 pl-3">{tt('Sales trend')}</h3>
            ...
            {data.salesByDate?.length > 0 ? (
                ...
                <table className="min-w-full">
                  <thead>
                    <tr className="bg-slate-50">
                      ...
                      <th className="px-4 py-2.5 text-right text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Sales')}</th>
                      <th className="px-4 py-2.5 text-right text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Invoices')}</th>
                      <th className="px-4 py-2.5 text-right text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Revenue')}</th>
                      <th className="px-4 py-2.5 text-right text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Tax')}</th>
                    </tr>
                  </thead>
              ...
              <div className="text-center p-8 bg-slate-50 rounded-2xl border border-slate-200">
                <p className="text-slate-500">{tt('No sales data available for the selected period.')}</p>
              </div>
            )}
          </div>
              ...
              {data.salesByProduct?.length > 0 ? (
                <div className="overflow-x-auto bg-white rounded-2xl border border-slate-200 shadow-sm">
                  <table className="min-w-full">
                    <thead>
                      <tr className="bg-slate-50">
                        <th className="px-4 py-2.5 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Product')}</th>
                        <th className="px-4 py-2.5 text-right text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Quantity sold')}</th>
                        <th className="px-4 py-2.5 text-right text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Revenue')}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200">
                ...
                <div className="text-center p-8 bg-slate-50 rounded-2xl border border-slate-200">
                  <p className="text-slate-500">{tt('No product data available.')}</p>
                </div>
              )}
            </div>
            <div>
              ...
              {data.salesByCustomer?.length > 0 ? (
                <div className="overflow-x-auto bg-white rounded-2xl border border-slate-200 shadow-sm">
                  <table className="min-w-full">
                    <thead>
                      <tr className="bg-slate-50">
                        <th className="px-4 py-2.5 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Customer')}</th>
                        <th className="px-4 py-2.5 text-right text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Sales')}</th>
                        <th className="px-4 py-2.5 text-right text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Invoices')}</th>
                        <th className="px-4 py-2.5 text-right text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Total spent')}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200">
                ...
                <div className="text-center p-8 bg-slate-50 rounded-2xl border border-slate-200">
                  <p className="text-slate-500">{tt('No customer data available.')}</p>
                </div>
              )}
            </div>
          </div>
