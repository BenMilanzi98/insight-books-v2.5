...
export default function ConsolidationDisclosure({ consolidation, className = '' }) {
  ...
  return (
    <div
      className={`rounded-xl border border-indigo-100 bg-indigo-50/60 px-4 py-3 text-sm text-indigo-950 ${className}`}
      role="note"
      aria-label="Group consolidation disclosure"
    >
      <div className="flex gap-2">
        <Info className="w-4 h-4 shrink-0 mt-0.5 text-indigo-600" aria-hidden />
        <div className="space-y-2 min-w-0">
          ...
          {notes.length > 0 ? (
            <ul className="list-disc pl-4 space-y-1 text-indigo-800/90">
              {notes.map((note) => (
                <li key={note}>{note}</li>
              ))}
            </ul>
