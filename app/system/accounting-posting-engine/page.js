...
export default function PostingEngineDiagnosticsPage() {
  ...
  if (loading) return <div className="p-8 text-slate-500">{tt('Loading posting engine diagnostics…')}</div>;
  if (error)
    return (
      <div className="p-8">
        <h1 className="mb-2 text-xl font-bold">{tt('Accounting Posting Engine')}</h1>
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-8">
      <header>
        <h1 className="text-2xl font-bold">{tt('Accounting Posting Engine')}</h1>
        <p className="text-sm text-slate-500">
          {tt('Internal Phase 4 diagnostics — read-only. Posting mode is resolved server-side and cannot be changed from this page.')}
        </p>
      </header>
      ...
      <section className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <div className="rounded border p-4">
          <div className="text-xs uppercase text-slate-500">{tt('Shadow exact-match rate')}</div>
          <div className="mt-1">
            {exactRate == null
              ? badge('NO DATA', 'muted')
        ...
        <div className="rounded border p-4">
          <div className="text-xs uppercase text-slate-500">{tt('Templates registered')}</div>
          <div className="mt-1 font-semibold">{templates.length}</div>
        </div>
      </section>

      <section className="rounded border p-4">
        <h2 className="mb-2 font-semibold">{tt('Accounting events')}</h2>
      ...
      <section className="rounded border p-4">
        ...
        {(shadowComparisons.total ?? 0) === 0 ? (
          <p className="text-sm text-slate-500">{tt('No shadow comparisons recorded for this business.')}</p>
        ) : (
          <ul className="flex flex-wrap gap-3 text-sm">
      ...
      <section className="rounded border p-4">
        <h2 className="mb-2 font-semibold">{tt('Recent posting attempts')}</h2>
        {recentAttempts.length === 0 ? (
          <p className="text-sm text-slate-500">{tt('No posting attempts recorded.')}</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b text-xs uppercase text-slate-500">
                  <th className="py-1 pr-4">{tt('Started')}</th>
                  <th className="py-1 pr-4">{tt('Event')}</th>
                  <th className="py-1 pr-4">{tt('Source')}</th>
                  <th className="py-1 pr-4">{tt('Attempt')}</th>
                  <th className="py-1 pr-4">{tt('Status')}</th>
                  <th className="py-1 pr-4">{tt('Failure')}</th>
                  <th className="py-1 pr-4">{tt('Duration')}</th>
                  <th className="py-1">{tt('Request ID')}</th>
                </tr>
              </thead>
              <tbody>
      ...
      <section className="rounded border p-4">
        <h2 className="mb-2 font-semibold">{tt('Posting template catalogue')}</h2>
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b text-xs uppercase text-slate-500">
              <th className="py-1 pr-4">{tt('Template')}</th>
              <th className="py-1 pr-4">{tt('Version')}</th>
              <th className="py-1 pr-4">{tt('Event type')}</th>
              <th className="py-1">{tt('Status')}</th>
            </tr>
          </thead>
          <tbody>
            {templates.map((t) => (
