...
export default function AccountingArchitecturePage() {
  ...
  if (loading) return <div className="p-8 text-slate-500">Loading architecture status…</div>;
  if (error)
    return (
      <div className="p-8">
        <h1 className="mb-2 text-xl font-bold">Accounting Architecture</h1>
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-8">
      <header>
        <h1 className="text-2xl font-bold">Accounting Architecture</h1>
        <p className="text-sm text-slate-500">
          Internal console — Phase 9: NEW_ENGINE is authoritative.
        </p>
      </header>
      ...
      <section className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <div className="rounded border p-4">
          <div className="text-xs uppercase text-slate-500">Integrity monitoring</div>
          <div className="mt-1">{badge(data.integrityMonitoring ? 'ENABLED' : 'OFF', data.integrityMonitoring ? 'ok' : 'muted')}</div>
        </div>
        <div className="rounded border p-4">
          <div className="text-xs uppercase text-slate-500">Cutover readiness</div>
          <div className="mt-1 text-sm">{data.cutoverReadiness}</div>
        </div>
      </section>

      <section className="rounded border p-4">
        <h2 className="mb-2 font-semibold">Event registry</h2>
        {Object.keys(events).length === 0 ? (
          <p className="text-sm text-slate-500">No accounting events registered yet.</p>
        ) : (
      ...
      <section className="rounded border p-4">
        ...
        {Object.keys(comparisons).length === 0 ? (
          <p className="text-sm text-slate-500">No shadow comparisons recorded.</p>
        ) : (
          <ul className="flex flex-wrap gap-3 text-sm">
      ...
      <section className="rounded border p-4">
        ...
        {(!data.flags || data.flags.length === 0) ? (
          <p className="text-sm text-slate-500">No accounting flags configured — full legacy behaviour.</p>
        ) : (
