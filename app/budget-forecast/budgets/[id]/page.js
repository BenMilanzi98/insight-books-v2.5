...
export default function BfBudgetDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/bf/expense-budgets/${id}`);
      const j = await res.json();
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/bf/expense-budgets/${id}/lines`, {
        method: 'PUT',
  ...
  const del = async () => {
    if (!window.confirm('Delete this expense budget?')) return;
    const res = await fetch(`/api/bf/expense-budgets/${id}`, { method: 'DELETE' });
    if (res.ok) window.location.href = '/budget-forecast/budgets';
    else {
      const j = await res.json();
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-bold text-slate-900">{header?.name || 'Expense budget'}</h2>
            {header && (
              <p className="mt-1 text-sm text-slate-600">
                {header.periodType} · {new Date(header.startDate).toLocaleDateString()} –{' '}
                {new Date(header.endDate).toLocaleDateString()} ·{' '}
                <span className="font-semibold capitalize">{header.status}</span>
              </p>
            )}
            <p className="mt-3 max-w-2xl text-sm text-slate-600">
              Add each expense account once, fill the grid, then <strong>Save lines</strong>. Zeros are stored so the
              plan matches your periods exactly.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <>
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <label className="text-xs font-semibold uppercase tracking-wide text-slate-500">Add expense account</label>
              <select
            ...
            <section className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              {selectedAccountIds.length === 0 && (
                <p className="p-6 text-sm text-slate-600">Add at least one expense account to build your plan.</p>
              )}
            </section>
          </>
        )}
