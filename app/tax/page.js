...
export default function TaxManagement() {
  ...
  // Tax settlement state
  const [isSettlingTax, setIsSettlingTax] = useState(false);
  ...
  // Handle tax settlement (Development branch approach - prioritized)
  ...
  // Alternative settlement method (Main branch approach - kept for compatibility)
  const handleSettleTax = () => {
    if (!taxData || taxData.netTaxLiability <= 0) {
      alert("No tax liability to settle for this period.");
      return;
    }
    
    setSettlementAmount(taxData.netTaxLiability);
    setSettlementDescription(`Tax Settlement - ${formatDateForDisplay(dateRange.startDate)} to ${formatDateForDisplay(dateRange.endDate)}`);
    setShowSettlementModal(true);
  };

  // Process tax settlement (Main branch approach)
    ...
    try {
      const response = await fetch('/api/expenses', {
        ...
        body: JSON.stringify({
          ...
          status: 'Completed',
          notes: `Tax settlement for period ${formatDateForDisplay(dateRange.startDate)} to ${formatDateForDisplay(dateRange.endDate)}`
        })
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to create tax settlement expense');
      }

      const result = await response.json();
      console.log('Tax settlement expense created:', result);
      
      setSettlementSuccess(true);
      setShowSettlementModal(false);
      
      // Refresh tax data to show updated paid taxes
      setTimeout(() => {
        refreshData();
        setSettlementSuccess(false);
      }, 2000);
      ...
      alert(`Failed to settle tax: ${error.message}`);
    } finally {
      setIsSettlingTax(false);
    }
  };
  ...
  return (
    ...
    <div className="container mx-auto px-4 py-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <h1 className="text-2xl font-bold">{tt('Tax Management')}</h1>
        <div className="flex flex-wrap gap-2">
          {taxData && taxData.netTaxLiability > 0 && (
            <button
              onClick={handleSettleTax}
              disabled={isSettlingTax}
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <StatCard
                      ...
                      icon={DollarSign}
                      helper={`On ${formatCurrency(taxData.collectedTaxes.totalTaxableAmount)} taxable amount`}
                    ...
                    <StatCard
                      ...
                      icon={DollarSign}
                      helper={`From ${taxData.paidTaxes.expenses.length} expense transactions`}
                    ...
                    <StatCard
                      ...
                      icon={PercentIcon}
                      helper="Due to tax authority for this period"
      ...
      {/* Tax Settlement Modal - Development branch approach (prioritized) */}
      <TaxSettlementModal
      ...
      {/* Alternative Settlement Modal - Main branch approach (kept for compatibility) */}
      {showSettlementModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">{tt('Settle Tax Liability')}</h3>
              <button
            ...
            <div className="space-y-4">
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div className="flex items-center">
                  ...
                  <span className="text-sm font-medium text-yellow-800">
                    Net Tax Liability: {formatCurrency(settlementAmount)}
                  </span>
                </div>
                <p className="text-xs text-yellow-700 mt-1">
                  {tt('This will create an expense entry for the tax payment')}
                </p>
              </div>
              ...
              <div className="flex space-x-3 pt-4">
                ...
                <button
                  onClick={processTaxSettlement}
                  disabled={isSettlingTax || settlementAmount <= 0}
      ...
      {settlementSuccess && (
        <div className="fixed top-4 right-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded shadow-lg z-50">
          <div className="flex items-center">
            <FaHandHoldingUsd className="h-5 w-5 mr-2" />
