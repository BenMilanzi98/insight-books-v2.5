  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      <header className="mb-6">
        ...
        <h1 className="mt-1 text-3xl font-semibold tracking-tight">MRA EIS configuration</h1>
        <p className="mt-2 text-sm text-slate-600">
          Cross-tenant configuration freshness. No raw credential or sensitive response content.
        </p>
      </header>
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.terminalId} className="border-b border-slate-100">
                  ...
                  <td className="px-3 py-2 font-medium">{r.freshnessStatus || '—'}</td>
                  <td className="px-3 py-2">{r.processingPaused ? 'Yes' : 'No'}</td>
                  <td className="px-3 py-2 text-xs">{r.lastConfigurationSyncAt || '—'}</td>
                </tr>
              ))}
              {rows.length === 0 && (
                <tr>
