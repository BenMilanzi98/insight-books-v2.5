        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 py-6 lg:py-8 pb-12">
          {roleDenied && (
            <div className="rounded-2xl bg-rose-50 border border-rose-200 p-6 sm:p-8 text-center shadow-sm">
              ...
              <p className="text-rose-600">{tt('You need journal entry access to manage accounting periods.')}</p>
            </div>
          )}
            ...
            <>
              <div className="rounded-2xl bg-gradient-to-r from-blue-600 via-sky-500 to-indigo-500 shadow-xl shadow-blue-200/50 p-6 sm:p-8 mb-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h1 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">{tt('Accounting Periods')}</h1>
                      <p className="text-indigo-100 text-sm mt-0.5">{tt('Open, close, and reopen fiscal periods')}</p>
                    </div>
                  </div>
                  {currentPeriod && (
              ...
              <div className="rounded-2xl bg-white shadow-lg shadow-slate-200/50 border border-slate-100 p-6 sm:p-8 mb-6">
                ...
                <p className="text-sm text-slate-600 mb-4">
                  {tt('The financial period always begins')} <strong>{tt('1 January')}</strong>. Yearly periods run 1 Jan – 31 Dec; reports align to the calendar year.
                </p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">{tt('Period Type')}</label>
                    <select
              ...
              <div className="rounded-2xl bg-white shadow-lg shadow-slate-200/50 border border-slate-100 overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white">
                  <h2 className="text-lg font-semibold text-slate-800">{tt('Period History')}</h2>
                </div>
                {loading ? (
                  ...
                  <div className="py-16 text-center text-slate-500">{tt('No accounting periods configured yet.')}</div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-gradient-to-r from-slate-50 to-slate-100/80 border-b border-slate-200">
                          <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Name')}</th>
                          <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Type')}</th>
                          <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Start')}</th>
                          <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('End')}</th>
                          <th className="px-4 py-3.5 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Status')}</th>
                          <th className="px-4 py-3.5 text-right text-xs font-semibold text-slate-600 uppercase tracking-wider">{tt('Actions')}</th>
                        </tr>
                      </thead>
