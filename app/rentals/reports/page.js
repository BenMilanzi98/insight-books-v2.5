    ...
    <main className="min-h-full p-4 sm:p-6 lg:p-8">
        ...
        <form className="flex flex-wrap items-end gap-3" onSubmit={submitFilters}>
          ...
          <label className="flex flex-col gap-1 text-sm font-medium text-gray-700">
            ...
            <select className="rounded-lg border border-gray-300 bg-white px-3 py-2" name="type" value={filters.type} onChange={updateFilter}>
              ...
              <option value="customer_hire">Customer hire</option>
              <option value="supplier_hire">Supplier hire</option>
            </select>
          </label>
      ...
      <section className="mb-6 grid gap-4 md:grid-cols-3">
        <PosStylePanel className="p-4" accent="blue">
          <h2 className="font-semibold text-[var(--text-primary)]">Revenue source</h2>
        ...
        <PosStylePanel className="p-4" accent="purple">
          <h2 className="font-semibold text-[var(--text-primary)]">Utilization</h2>
        ...
        <PosStylePanel className="p-4">
          <h2 className="font-semibold text-[var(--text-primary)]">Repair entry convention</h2>
          <p className="mt-2 text-sm text-gray-600">
            Tag rental repair expenses with <code>source=REPAIR</code> or <code>RENTAL_REPAIR</code> in Notes to include them here.
          </p>
        </PosStylePanel>
      </section>
      ...
      <PosStylePanel className="overflow-hidden" accent={false}>
        <div className="border-b border-gray-200 px-4 py-4 sm:px-5">
          <h2 className="font-semibold text-[var(--text-primary)]">Report activity</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
              <tr><th className="px-4 py-3 sm:px-5">Date</th><th className="px-4 py-3">Type</th><th className="px-4 py-3">Description</th><th className="px-4 py-3 text-right sm:px-5">Amount</th></tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              ...
              {!loading && report.rows.length === 0 ? <tr><td className="px-4 py-8 text-center text-gray-500 sm:px-5" colSpan="4">No report activity in this period.</td></tr> : null}
            </tbody>
          </table>
        </div>
      </PosStylePanel>
