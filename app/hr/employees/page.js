  ...
  return (
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* Deductions Section */}
        <div className="mb-6">
          <h3 className="text-lg font-medium text-gray-900 mb-4">Salary Deductions</h3>
        ...
        {formData.grossSalary && (
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-gray-900">Salary Calculation</h3>
              <button
                type="button"
...
const DeductionModal = ({ deduction, onClose, onSave, deductionType = 'custom', setDeductionType }) => {
  const [formData, setFormData] = useState({
    name: '',
    description: '',
    type: 'percentage',
  ...
  useEffect(() => {
    if (deduction) {
      setFormData({
        ...
        value: deduction.percentage !== null && deduction.percentage !== undefined ? deduction.percentage : deduction.amount || 0,
        isStatutory: deduction.isStatutory || false,
        isActive: deduction.isActive !== undefined ? deduction.isActive : true
      });
    } else if (deductionType === 'paye') {
      setFormData({
        name: 'PAYE (Pay As You Earn)',
        description: 'Malawi PAYE Tax - Progressive rates based on salary brackets',
        type: 'percentage',
        value: 0, // Will be calculated automatically
        isStatutory: true,
        isActive: true
      });
    } else if (deductionType === 'nps') {
      setFormData({
        name: 'NPS (National Pension Scheme)',
        description: 'Malawi NPS - 5% employee contribution',
        type: 'percentage',
        value: 5,
        isStatutory: true,
        isActive: true
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (response.status === 401) {
          alert('Please log in to save deductions. Authentication required.');
          return;
        }
        const error = await response.json();
        alert(`Failed to save deduction: ${error.error}`);
      }
    } catch (error) {
        ...
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="text-lg font-semibold">
                {deduction ? 'Edit Deduction' : deductionType === 'paye' ? 'Add PAYE Deduction' : deductionType === 'nps' ? 'Add NPS Deduction' : 'Add New Deduction'}
              </h2>
              {deductionType === 'paye' && (
                <p className="text-sm text-gray-600 mt-1">Malawi PAYE Tax with progressive rates</p>
              )}
              {deductionType === 'nps' && (
                <p className="text-sm text-gray-600 mt-1">Malawi National Pension Scheme</p>
              )}
            </div>
            <button
              onClick={onClose}
          ...
          {deductionType === 'paye' && (
            <div className="mb-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
              <h3 className="text-sm font-semibold text-gray-900 mb-2">Malawi PAYE Tax Brackets (2025)</h3>
              <div className="text-xs text-gray-700 space-y-1">
                <div className="flex justify-between">
                  <span>First MK 150,000:</span>
                  <span className="font-medium">0%</span>
                </div>
                <div className="flex justify-between">
                  <span>Next MK 350,000 (150,001 - 500,000):</span>
                  <span className="font-medium">25%</span>
                </div>
                <div className="flex justify-between">
                  <span>Next MK 2,050,000 (500,001 - 2,550,000):</span>
                  <span className="font-medium">30%</span>
                </div>
                <div className="flex justify-between">
                  <span>Excess of MK 2,550,000:</span>
                  <span className="font-medium">35%</span>
                </div>
              </div>
              <p className="text-xs text-gray-600 mt-2 italic">
                This deduction will automatically calculate PAYE based on the employee's gross salary using the above brackets.
              </p>
            </div>
          )}
          ...
          {deductionType === 'nps' && (
            <div className="mb-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
              <h3 className="text-sm font-semibold text-gray-900 mb-2">Malawi NPS Contribution</h3>
              <div className="text-xs text-gray-700 space-y-1">
                <div className="flex justify-between">
                  <span>Employee Contribution:</span>
                  <span className="font-medium">5% of gross salary</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <span>Total NPS:</span>
                  <span className="font-medium">10% of gross salary</span>
                </div>
              </div>
              <p className="text-xs text-gray-600 mt-2 italic">
                This deduction represents the employee's 5% contribution. The employer's 5% is handled separately in payroll processing.
              </p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {!deduction && (
              <div>
                ...
                >
                  <option value="custom">Other Deduction</option>
                  <option value="paye">PAYE (Malawi Tax)</option>
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                required
                disabled={deductionType === 'paye' || deductionType === 'nps'}
                className={`w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-gray-500 focus:border-transparent ${
                  (deductionType === 'paye' || deductionType === 'nps') ? 'bg-gray-100' : ''
                }`}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                {formData.type === 'percentage' ? 'Percentage (%)' : 'Amount (MWK)'} *
                {deductionType === 'paye' && (
                  <span className="text-xs text-gray-600 ml-2">(Calculated automatically)</span>
                )}
              </label>
              <input
                type="number"
                ...
                placeholder={formData.type === 'percentage' ? '5' : '10000'}
              />
            </div>
            ...
            <div className="flex items-center space-x-4">
              <label className="flex items-center">
                <input
                  ...
                  className="mr-2"
                />
                <span className="text-sm text-gray-700">Statutory Deduction</span>
              </label>
