...
/**
 * Generate next debit note number: DN-YYYYMM-00001
 */
async function generateDebitNoteNumber(tenantId, tx = prisma) {
  const now = new Date();
...
export async function GET(request) {
  try {
    ...
    console.error('Debit notes list error:', e);
    return NextResponse.json(
      { error: e.message || 'Failed to list debit notes' },
      { status: 500 }
    );
  }
}
...
/**
 * POST - Create debit note (Draft or Posted)
...
export async function POST(request) {
  try {
    ...
    const debitNote = await prisma.$transaction(async (tx) => {
      const noteNumber = await generateDebitNoteNumber(user.tenantId, tx);
    ...
    console.error('Create debit note error:', e);
    return NextResponse.json(
      { error: e.message || 'Failed to create debit note' },
      { status: 500 }
    );
  }
}
