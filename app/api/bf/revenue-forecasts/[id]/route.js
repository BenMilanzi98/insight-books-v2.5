...
export async function GET(request, { params }) {
  try {
    ...
    const row = await getRevenueForecast(params.id, user.tenantId);
    if (!row) {
    ...
    console.error('bf revenue-forecast GET:', error);
...
export async function PATCH(request, { params }) {
  try {
    ...
    const row = await updateRevenueForecast(params.id, user.tenantId, body);
    if (!row) {
      return NextResponse.json({ error: 'Not found' }, { status: 404 });
    }
    return NextResponse.json({ success: true, data: row });
  } catch (error) {
    console.error('bf revenue-forecast PATCH:', error);
...
export async function DELETE(request, { params }) {
  try {
    ...
    const ok = await deleteRevenueForecast(params.id, user.tenantId);
    if (!ok) {
      return NextResponse.json({ error: 'Not found' }, { status: 404 });
    }
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('bf revenue-forecast DELETE:', error);
