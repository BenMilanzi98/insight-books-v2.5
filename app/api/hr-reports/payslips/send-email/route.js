// app/api/hr-reports/payslips/send-email/route.js
...
/**
 * POST - Send payslip to employee via email
 */
export async function POST(request) {
  try {
    ...
    if (!employee.email) {
      return NextResponse.json(
        { error: 'Employee email address is not available' },
        { status: 400 }
      );
    }
    ...
    if (!payroll) {
      return NextResponse.json(
        { error: 'No payroll record found for the selected period' },
        { status: 404 }
      );
    }
    ...
    const payslip = {
      ...
      deductions: {
        ...
        deductionNames: additionalInfo.deductionNames || {}, // Include deduction names
    ...
    // Generate PDF
    const pdfBuffer = await generatePayslipPDF(payslip, employee.tenant);
    
    // Convert ArrayBuffer to Buffer for nodemailer
    const pdfBufferForEmail = Buffer.from(pdfBuffer);

    // Prepare email content
    const monthName = new Date(startDateObj).toLocaleString('default', { month: 'long' });
    const year = startDateObj.getFullYear();
    const companyName = employee.tenant?.name || 'Company';

    // Send email with payslip PDF attachment
    console.log('Sending payslip email to:', employee.email);
    console.log('PDF buffer size:', pdfBufferForEmail.length, 'bytes');
    ...
    try {
      await sendEmail({
        ...
        data: {
          ...
          tenantLogoUrl: employee.tenant?.logoUrl || null
        },
        attachments: [
          {
      ...
      return NextResponse.json({
        success: true,
        message: 'Payslip sent successfully to employee email'
      });
    } catch (emailError) {
