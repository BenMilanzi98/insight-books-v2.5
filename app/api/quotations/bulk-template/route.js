...
/**
 * PATCH /api/quotations/bulk-template
 * Body: { quotationIds: string[], templateId?: string, appearance?: object, setAsDefault?: boolean }
 */
export async function PATCH(request) {
  try {
    const perm = await requirePermission(request, 'quotations.update');
    if (perm) return perm;
    ...
    const quotationIds = Array.isArray(body.quotationIds)
      ? body.quotationIds.filter(Boolean)
      : [];
    if (quotationIds.length === 0) {
      return NextResponse.json({ error: 'quotationIds required' }, { status: 400 });
    }
    ...
    const result = await prisma.quotation.updateMany({
      where: { tenantId: user.tenantId, id: { in: quotationIds } },
      data: { templateId },
    });
    ...
    console.error('bulk-template quotations error:', error);
