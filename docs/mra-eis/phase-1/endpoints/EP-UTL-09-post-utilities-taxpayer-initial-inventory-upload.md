...
If not the last batch, stores items in staging table. If last batch, processes all items from staging table.
...
## Purpose
Handles the initial inventory upload for a taxpayer, supporting phased uploads (up to 50 products per batch) and a last batch indicator.
If not the last batch, stores items in staging table. If last batch, processes all items from staging table.
...
- Schema: `#/components/schemas/TaxpayerInitialInventoryUploadRequest`

## Response schema
- Declared HTTP responses: 200, 400, 500
...
OpenAPI HTTP codes: 200, 400, 500. Application error catalogue: ERROR_CODE_CATALOGUE.md + guide error_codes.

## Authentication
JWT in Authorization (guide; Bearer vs raw CONFLICT)
